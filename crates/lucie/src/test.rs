...
/// This is intended for use with the `lucie::test` macro
...
fn calculate_seeds(iterations: u64, explicit_seeds: &[u64]) -> (impl Iterator<Item = u64> + '_, bool) {
	...
	let env_num = env::var("SEED").map(|seed| seed.parse().expect("invalid SEED variable as integer")).ok();

	let empty_range = || 0..0;

	let iter = {
		let env_range = if let Some(env_num) = env_num { env_num..env_num + 1 } else { empty_range() };
