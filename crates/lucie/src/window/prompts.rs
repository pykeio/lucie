...
impl PromptHandle {
	...
	pub fn with_view<V: Prompt + Render>(self, view: Entity<V>, window: &mut Window, cx: &mut App) -> RenderablePromptHandle {
...
/// Lucie to always use the fallback prompt renderer.
...
/// The default fallback for rendering prompts, when the platform doesn't support it.
...
pub(crate) enum PromptBuilder {
	Default,
	Custom(Box<dyn Fn(PromptLevel, &str, Option<&str>, &[PromptButton], PromptHandle, &mut Window, &mut App) -> RenderablePromptHandle>)
}

impl Deref for PromptBuilder {
	type Target = dyn Fn(PromptLevel, &str, Option<&str>, &[PromptButton], PromptHandle, &mut Window, &mut App) -> RenderablePromptHandle;
