//! KeyDispatch is where Lucie deals with binding actions to key events.
//!
...
//! ```
//!
//! With all of this in place, Lucie will ensure that if you have an Editor that contains
...
//! bubbles up from the bottom. For example in Zed, the Workspace is the top-level view, which contains Pane's, which
//! contain Editors. If there are conflicting keybindings defined then the Editor's bindings take precedence over the
//! Pane's bindings, which take precedence over the Workspace.
//!
//! In Lucie, keybindings are not limited to just single keystrokes, you can define
...
use crate::{Action, ActionRegistry, App, DispatchPhase, EntityId, FocusId, KeyBinding, KeyContext, Keymap, Keystroke, ModifiersChangedEvent, Window};
...
pub(crate) struct DispatchTree {
	...
	focusable_node_ids: RapidHashMap<FocusId, DispatchNodeId>,
	view_node_ids: RapidHashMap<EntityId, DispatchNodeId>,
...
impl ReusedSubtree {
	pub fn refresh_node_id(&self, node_id: DispatchNodeId) -> DispatchNodeId {
		debug_assert!(self.old_range.contains(&node_id.0), "node {} was not part of the reused subtree {:?}", node_id.0, self.old_range);
...
impl DispatchTree {
	pub fn new(keymap: Rc<RefCell<Keymap>>, action_registry: Rc<ActionRegistry>) -> Self {
		Self {
			...
			view_stack: Vec::new(),
			nodes: Vec::new(),
			focusable_node_ids: RapidHashMap::default(),
			view_node_ids: RapidHashMap::default(),
			keymap,
			action_registry
		}
	}
	...
	pub fn push_node(&mut self) -> DispatchNodeId {
		...
		self.nodes.push(DispatchNode { parent, ..Default::default() });
	...
	pub fn reuse_subtree(&mut self, old_range: Range<usize>, source: &mut Self, focus: Option<FocusId>) -> ReusedSubtree {
		let new_range = self.nodes.len()..self.nodes.len() + old_range.len();

		let mut contains_focus = false;
		let mut source_stack = vec![];
		for (source_node_id, source_node) in source.nodes.iter_mut().enumerate().skip(old_range.start).take(old_range.len()) {
		...
		ReusedSubtree { old_range, new_range, contains_focus }
	}
	...
	pub fn on_action(&mut self, action_type: TypeId, listener: Rc<dyn Fn(&dyn Any, DispatchPhase, &mut Window, &mut App)>) {
		self.active_node().action_listeners.push(DispatchActionListener { action_type, listener });
	}
	...
	pub fn highest_precedence_binding_for_action(&self, action: &dyn Action, context_stack: &[KeyContext]) -> Option<KeyBinding> {
		let keymap = self.keymap.borrow();
		keymap
			.bindings_for_action(action)
			.rev()
	...
	fn binding_matches_predicate_and_not_shadowed(keymap: &Keymap, binding: &KeyBinding, context_stack: &[KeyContext]) -> bool {
	...
	fn bindings_for_input(&self, input: &[Keystroke], dispatch_path: &SmallVec<[DispatchNodeId; 32]>) -> (SmallVec<[KeyBinding; 1]>, bool, Vec<KeyContext>) {
		let context_stack: Vec<KeyContext> = dispatch_path.iter().filter_map(|node_id| self.node(*node_id).context.clone()).collect();
	...
	pub fn possible_next_bindings_for_input(&self, input: &[Keystroke], context_stack: &[KeyContext]) -> Vec<KeyBinding> {
		self.keymap.borrow().possible_next_bindings_for_input(input, context_stack)
	}
	...
	/// - to_replay: any keystroke that had been pushed to pending, but are no-longer matched, these should be replayed
	///   first.
	pub fn dispatch_key(
		&mut self,
		...
		if pending {
			...
			return DispatchResult { context_stack, ..Default::default() };
		}
		input.pop();
	...
	pub fn flush_dispatch(&mut self, input: SmallVec<[Keystroke; 1]>, dispatch_path: &SmallVec<[DispatchNodeId; 32]>) -> SmallVec<[Replay; 1]> {
		let (suffix, mut to_replay) = self.replay_prefix(input, dispatch_path);
	...
	pub fn view_path_reversed(&self, view_id: EntityId) -> impl Iterator<Item = EntityId> {
		...
		std::iter::successors(current_node_id.map(|node_id| self.node(node_id)), |node_id| Some(self.node(node_id.parent?))).filter_map(|node| node.view_id)
	}
...
mod tests {
	...
	use crate::{
		self as lucie, Action, ActionRegistry, App, AppContext, Context, DispatchTree, Element, ElementId, FocusHandle, GlobalElementId, InputHandler,
		IntoElement, KeyBinding, KeyContext, Keymap, Keystroke, LayoutId, Render, Subscription, TestAppContext, UTF16Selection, Window
	};
	...
	impl Action for TestAction {
		...
		fn build(_value: Option<&mut [u8]>) -> anyhow::Result<Box<dyn Action>>
		where
			Self: Sized
		{
	...
	fn test_keybinding_for_action_bounds() {
		...
		let contexts = vec![KeyContext::parse("Workspace").unwrap(), KeyContext::parse("ProjectPanel").unwrap()];
	...
	fn test_pending_has_binding_state() {
		...
		fn dispatch(tree: &mut DispatchTree, pending: SmallVec<[Keystroke; 1]>, key: &str, path: &DispatchPath) -> DispatchResult {
	...
	fn test_pending_input_observers_notified_on_focus_change(cx: &mut TestAppContext) {
		...
		impl InputHandler for CustomElement {
			...
			fn text_for_range(&mut self, _: Range<usize>, _: &mut Option<Range<usize>>, _: &mut Window, _: &mut App) -> Option<String> {
				None
			}
	...
	fn test_focus_preserved_across_window_activation(cx: &mut TestAppContext) {
		...
		cx.update(|window, _| {
			...
			assert!(focus_handle.is_focused(window), "Element should be focused after window.focus() call");
		});
		...
		cx.update(|window, _| {
			...
			assert!(!focus_handle.is_focused(window), "Element should not appear focused when window is inactive");
		});
		...
		cx.update(|window, _| {
			...
			assert!(focus_handle.is_focused(window), "Element should be focused after window reactivation");
		});
	}
}
