...
pub trait EntityInputHandler: 'static + Sized {
	...
	fn text_for_range(&mut self, range: Range<usize>, adjusted_range: &mut Option<Range<usize>>, window: &mut Window, cx: &mut Context<Self>)
	-> Option<String>;
...
impl<V: EntityInputHandler> InputHandler for ElementInputHandler<V> {
	...
	fn text_for_range(&mut self, range_utf16: Range<usize>, adjusted_range: &mut Option<Range<usize>>, window: &mut Window, cx: &mut App) -> Option<String> {
		self.view
			.update(cx, |view, cx| view.text_for_range(range_utf16, adjusted_range, window, cx))
	}

	fn replace_text_in_range(&mut self, replacement_range: Option<Range<usize>>, text: &str, window: &mut Window, cx: &mut App) {
		self.view
			.update(cx, |view, cx| view.replace_text_in_range(replacement_range, text, window, cx));
	}
	...
	fn replace_and_mark_text_in_range(
		...
		self.view
			.update(cx, |view, cx| view.replace_and_mark_text_in_range(range_utf16, new_text, new_selected_range, window, cx));
	}
