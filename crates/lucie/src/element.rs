//! Elements are the workhorses of Lucie. They are responsible for laying out and painting all of
...
//! own APIs and Lucie, or other element implementation, uses the APIs in this module to convert
...
//! tree and any callbacks they have registered with Lucie are dropped and the process repeats.
//!
//! But some state is too simple and voluminous to store in every view that needs it, e.g.
//! whether a hover has been started or not. For this, Lucie provides the [`Element::PrepaintState`], associated type.
//!
//! # Implementing your own elements
//!
//! Elements are intended to be the low level, imperative API to Lucie. They are responsible for upholding,
//! or breaking, Lucie's features as they deem necessary. As an example, most Lucie elements are expected
...
//!
//! However, most of the time, you won't need to implement your own elements. Lucie provides a number of
...
trait ElementObject {
	...
	fn layout_as_root(&mut self, available_space: Size<AvailableSpace>, window: &mut Window, cx: &mut App) -> Size<Pixels>;
}
...
impl<E: Element> Drawable<E> {
	...
	fn request_layout(&mut self, window: &mut Window, cx: &mut App) -> LayoutId {
		match mem::take(&mut self.phase) {
			ElementDrawPhase::Start => {
				...
				self.phase = ElementDrawPhase::RequestLayout { layout_id, global_id, request_layout };
				layout_id
			}
	...
	pub(crate) fn prepaint(&mut self, window: &mut Window, cx: &mut App) {
		match mem::take(&mut self.phase) {
			...
			| ElementDrawPhase::LayoutComputed {
				...
				let prepaint = self.element.prepaint(global_id.as_ref(), bounds, &mut request_layout, window, cx);
	...
	pub(crate) fn paint(&mut self, window: &mut Window, cx: &mut App) -> (E::RequestLayoutState, E::PrepaintState) {
		match mem::take(&mut self.phase) {
			ElementDrawPhase::Prepaint {
				...
				self.element
					.paint(global_id.as_ref(), bounds, &mut request_layout, &mut prepaint, window, cx);
	...
	pub(crate) fn layout_as_root(&mut self, available_space: Size<AvailableSpace>, window: &mut Window, cx: &mut App) -> Size<Pixels> {
		if matches!(&self.phase, ElementDrawPhase::Start) {
			self.request_layout(window, cx);
		}

		let layout_id = match mem::take(&mut self.phase) {
			ElementDrawPhase::RequestLayout { layout_id, global_id, request_layout } => {
...
impl Element for AnyElement {
	...
	fn prepaint(&mut self, _: Option<&GlobalElementId>, _: Bounds<Pixels>, _: &mut Self::RequestLayoutState, window: &mut Window, cx: &mut App) {
...
impl Element for Empty {
	...
	fn prepaint(&mut self, _id: Option<&GlobalElementId>, _bounds: Bounds<Pixels>, _state: &mut Self::RequestLayoutState, _window: &mut Window, _cx: &mut App) {
	}
