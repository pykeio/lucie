...
struct WindowInvalidatorInner {
	...
	pub dirty_views: RapidHashSet<EntityId>
}
...
impl WindowInvalidator {
	pub fn new() -> Self {
		WindowInvalidator {
			inner: Rc::new(RefCell::new(WindowInvalidatorInner {
				...
				dirty_views: RapidHashSet::default()
			}))
		}
	}
	...
	pub fn take_views(&self) -> RapidHashSet<EntityId> {
		mem::take(&mut self.inner.borrow_mut().dirty_views)
	}

	pub fn replace_views(&self, views: RapidHashSet<EntityId>) {
	...
	pub fn debug_assert_paint(&self) {
		debug_assert!(matches!(self.inner.borrow().draw_phase, DrawPhase::Paint), "this method can only be called during paint");
	}

	#[track_caller]
	pub fn debug_assert_prepaint(&self) {
		debug_assert!(matches!(self.inner.borrow().draw_phase, DrawPhase::Prepaint), "this method can only be called during request_layout, or prepaint");
	}
...
/// Emitted by implementers of [`ManagedView`] to indicate the view should be dismissed, such as when a view is
...
impl TooltipId {
	...
	pub fn is_hovered(&self, window: &Window) -> bool {
		window
			.tooltip_bounds
			.as_ref()
			.is_some_and(|tooltip_bounds| tooltip_bounds.id == *self && tooltip_bounds.bounds.contains(&window.mouse_position()))
	}
}
...
pub(crate) struct Frame {
	...
	pub(crate) element_states: RapidHashMap<(GlobalElementId, TypeId), ElementStateBox>,
	...
	pub(crate) debug_bounds: RapidHashMap<String, Bounds<Pixels>>,
...
impl Frame {
	pub(crate) fn new(dispatch_tree: DispatchTree) -> Self {
		Frame {
			focus: None,
			window_active: false,
			element_states: RapidHashMap::default(),
			...
			#[cfg(any(test, feature = "test-support"))]
			debug_bounds: RapidHashMap::default(),
...
pub struct Window {
	...
	pub(crate) dirty_views: RapidHashSet<EntityId>,
...
fn default_bounds(display_id: Option<DisplayId>, cx: &mut App) -> WindowBounds {
	...
	let active_window_bounds = cx.active_window().and_then(|w| w.update(cx, |_, window, _| window.window_bounds()).ok());
	...
	let display_bounds = display.as_ref().map(|d| d.visible_bounds()).unwrap_or_else(default_placement);

	let (Bounds { origin: base_origin, size: base_size }, window_bounds_ctor): (_, fn(Bounds<Pixels>) -> WindowBounds) = match active_window_bounds {
		...
		None => (display.as_ref().map(|d| d.default_bounds()).unwrap_or_else(default_placement), WindowBounds::Windowed)
	};
	...
	pub(crate) fn new(handle: AnyWindowHandle, options: WindowOptions, cx: &mut App) -> Result<Self> {
		...
		platform_window.on_request_frame(Box::new({
			...
			move |request_frame_options| {
				...
				let needs_present =
					request_frame_options.require_presentation || needs_present.get() || (active.get() && input_rate_tracker.borrow_mut().is_high_rate());
		...
		platform_window.on_appearance_changed(Box::new({
			let mut cx = cx.to_async();
			move || {
				handle.update(&mut cx, |_, window, cx| window.appearance_changed(cx)).log_err();
			}
		}));
			...
			move |active| {
				handle
					.update(&mut cx, |_, window, cx| {
						...
						window.activation_observers.clone().retain(&(), |callback| callback(window, cx));
		...
		platform_window.on_select_previous_tab({
			...
			Box::new(move || {
				handle
					.update(&mut cx, |_, _window, cx| SystemWindowTabController::select_previous_tab(cx, handle.window_id()))
					.log_err();
			})
		});
...
impl Window {
	...
	pub fn replace_root<E>(&mut self, cx: &mut App, build_view: impl FnOnce(&mut Window, &mut Context<E>) -> E) -> Entity<E>
	where
		E: 'static + Render
	{
	...
	pub(crate) fn dispatch_keystroke_observers(&mut self, event: &dyn Any, action: Option<Box<dyn Action>>, context_stack: Vec<KeyContext>, cx: &mut App) {
	...
	pub(crate) fn dispatch_keystroke_interceptors(&mut self, event: &dyn Any, context_stack: Vec<KeyContext>, cx: &mut App) {
		let Some(key_down_event) = event.downcast_ref::<KeyDownEvent>() else {
			return;
		};
	...
	/// The callback will be invoked a handle to the emitting entity, the event, and a window context for the current
	/// window.
	...
	pub fn observe_release<T>(&self, entity: &Entity<T>, cx: &mut App, mut on_release: impl FnOnce(&mut T, &mut Window, &mut App) + 'static) -> Subscription
	where
		T: 'static
	{
		let entity_id = entity.entity_id();
	...
	///
	/// If called from within a view, it will notify that view on the next frame. Otherwise, it will refresh the entire
	/// window.
	...
	/// Returns the bounds of the current window in the global coordinate space, which could span across multiple
	/// displays.
		...
		for tooltip_request_index in (0..self.next_frame.tooltip_requests.len()).rev() {
			...
			if tooltip_bounds.right() > window_bounds.right() {
				...
				if new_x >= Pixels::ZERO {
					...
					tooltip_bounds.origin.x = cmp::max(Pixels::ZERO, tooltip_bounds.origin.x - tooltip_bounds.right() - window_bounds.right());
				}
			}
			...
			if tooltip_bounds.bottom() > window_bounds.bottom() {
				...
				if new_y >= Pixels::ZERO {
					...
					tooltip_bounds.origin.y = cmp::max(Pixels::ZERO, tooltip_bounds.origin.y - tooltip_bounds.bottom() - window_bounds.bottom());
				}
			}
	...
	fn prepaint_deferred_draws(&mut self, deferred_draw_indices: &[usize], cx: &mut App) {
		...
		for deferred_draw_ix in deferred_draw_indices {
			...
			if let Some(element) = deferred_draw.element.as_mut() {
				self.with_rendered_view(deferred_draw.current_view, |window| {
					window.with_absolute_element_offset(deferred_draw.absolute_offset, |window| element.prepaint(window, cx));
				})
			} else {
		...
		assert_eq!(self.next_frame.deferred_draws.len(), 0, "cannot call defer_draw during deferred drawing");
	...
	pub(crate) fn reuse_prepaint(&mut self, range: Range<PrepaintStateIndex>) {
		...
		self.next_frame.accessed_element_states.extend(
			self.rendered_frame.accessed_element_states[range.start.accessed_element_states_index..range.end.accessed_element_states_index]
				.iter()
	...
	pub(crate) fn reuse_paint(&mut self, range: Range<PaintIndex>) {
		...
		self.next_frame
			.tab_stops
			.replay(&self.rendered_frame.tab_stops.insertion_history[range.start.tab_handle_index..range.end.tab_handle_index]);
	...
	pub fn use_keyed_state<S: 'static>(&mut self, key: impl Into<ElementId>, cx: &mut App, init: impl FnOnce(&mut Self, &mut Context<S>) -> S) -> Entity<S> {
			...
			.or_else(|| self.rendered_frame.element_states.remove(&key))
		{
			...
			let mut state_box = inner
				...
				.map_err(|_| {
					#[cfg(debug_assertions)]
					{
						anyhow::anyhow!("invalid element state type for id, requested {:?}, actual: {:?}", std::any::type_name::<S>(), type_name)
					}

					#[cfg(not(debug_assertions))]
					{
						anyhow::anyhow!("invalid element state type for id, requested {:?}", std::any::type_name::<S>(),)
					}
				})
				.unwrap();
		...
		S: 'static
	{
		...
		if let Some(global_id) = global_id {
			...
			let (result, state) = f(None, self);
			debug_assert!(state.is_none(), "you must not return an element state when passing None for the global id");
			result
		}
	}
	...
	pub fn paint_svg(
		...
		let Some(AtlasTileWithMetadata { tile, draw_offset }) = self.sprite_atlas.get_or_insert_with(&params.clone().into(), &mut || {
			...
			Ok(Some(AtlasTileData {
				...
				texture_kind: AtlasTextureKind::Monochrome
			}))
		})?
		else {
			return Ok(());
		...
		let svg_bounds = Bounds {
			...
			size: tile.bounds.size.map(|value| ScaledPixels(value.0 as f32 / SMOOTH_SVG_SCALE_FACTOR))
		};

		self.next_frame.scene.insert_primitive(MonochromeSprite {
			order: 0,
			pad: 0,
			bounds: svg_bounds.map_origin(|origin| origin.round()).map_size(|size| size.ceil()),
			content_mask,
	...
	pub fn paint_image(
		...
		let scale_factor = self.scale_factor();
		let mut bounds = bounds.scale(scale_factor);
		...
		let tile = self
			...
			.get_or_insert_with(&params.into(), &mut || {
				Ok(Some(AtlasTileData {
					size: data.size(frame_index),
					draw_offset: Point::default(),
					data: Cow::Borrowed(data.as_bytes(frame_index).expect("It's the caller's job to pass a valid frame index")),
					texture_kind: AtlasTextureKind::Polychrome
				}))
			})?
			.expect("Callback above only returns Some");
		bounds.origin += tile.draw_offset;
		let content_mask = self.content_mask().scale(scale_factor);
	...
	/// Lucie itself automatically in order to pass your element its `Bounds` automatically.
	///
	...
	///
	/// This method should only be called as part of the paint phase of element drawing.
	pub fn on_mouse_event<Event: MouseEvent>(&mut self, mut listener: impl FnMut(&Event, DispatchPhase, &mut Window, &mut App) + 'static) {
		self.invalidator.debug_assert_paint();

		self.next_frame
			.mouse_listeners
			.push(Some(Box::new(move |event: &dyn Any, phase: DispatchPhase, window: &mut Window, cx: &mut App| {
	...
	pub fn on_key_event<Event: KeyEvent>(&mut self, listener: impl Fn(&Event, DispatchPhase, &mut Window, &mut App) + 'static) {
		...
		self.next_frame
			.dispatch_tree
			.on_key_event(Rc::new(move |event: &dyn Any, phase, window: &mut Window, cx: &mut App| {
	...
	pub fn on_modifiers_changed(&mut self, listener: impl Fn(&ModifiersChangedEvent, &mut Window, &mut App) + 'static) {
		...
		self.next_frame
			.dispatch_tree
			.on_modifiers_changed(Rc::new(move |event: &ModifiersChangedEvent, window: &mut Window, cx: &mut App| listener(event, window, cx)));
	}
	...
	pub fn on_focus_in(&mut self, handle: &FocusHandle, cx: &mut App, mut listener: impl FnMut(&mut Window, &mut App) + 'static) -> Subscription {
	...
	pub fn keystroke_text_for(&self, action: &dyn Action) -> String {
		self.highest_precedence_binding_for_action(action)
			.map(|binding| binding.keystrokes().iter().map(ToString::to_string).collect::<Vec<_>>().join(" "))
	...
	fn dispatch_key_event(&mut self, event: &dyn Any, cx: &mut App) {
		...
		if let Some(event) = event.downcast_ref::<ModifiersChangedEvent>() {
			if event.modifiers.number_of_modifiers() == 0 && self.pending_modifier.modifiers.number_of_modifiers() == 1 && !self.pending_modifier.saw_keystroke
			{
	...
	fn finish_dispatch_key_event(&mut self, event: &dyn Any, dispatch_path: SmallVec<[DispatchNodeId; 32]>, context_stack: Vec<KeyContext>, cx: &mut App) {
	...
	fn dispatch_key_down_up_event(&mut self, event: &dyn Any, dispatch_path: &SmallVec<[DispatchNodeId; 32]>, cx: &mut App) {
		// Capture phase
	...
	fn dispatch_modifiers_changed_event(&mut self, event: &dyn Any, dispatch_path: &SmallVec<[DispatchNodeId; 32]>, cx: &mut App) {
	...
	pub fn pending_input_keystrokes(&self) -> Option<&[Keystroke]> {
		self.pending_input.as_ref().map(|pending_input| pending_input.keystrokes.as_slice())
	}
	...
	fn dispatch_action_on_node(&mut self, node_id: DispatchNodeId, action: &dyn Action, cx: &mut App) {
		...
		if let Some(mut global_listeners) = cx.global_action_listeners.remove(&action.as_any().type_id()) {
			...
			global_listeners.extend(cx.global_action_listeners.remove(&action.as_any().type_id()).unwrap_or_default());
	...
	pub fn observe_global<G: Global>(&mut self, cx: &mut App, f: impl Fn(&mut Window, &mut App) + 'static) -> Subscription {
		let window_handle = self.handle;
		let (subscription, activate) = cx
			.global_observers
			.insert(TypeId::of::<G>(), Box::new(move |cx| window_handle.update(cx, |_, window, cx| f(window, cx)).is_ok()));
		cx.defer(move |_| activate());
		subscription
	}
	...
	pub fn prompt<T>(&mut self, level: PromptLevel, message: &str, detail: Option<&str>, answers: &[T], cx: &mut App) -> oneshot::Receiver<usize>
	where
		T: Clone + Into<PromptButton>
	{
		let prompt_builder = cx.prompt_builder.take();
		let Some(prompt_builder) = prompt_builder else {
			unreachable!("Re-entrant window prompting is not supported");
		};
	...
	/// Returns a generic event listener that invokes the given listener with the view and context associated with the
	/// given view handle.
	...
	/// Returns a generic handler that invokes the given handler with the view and context associated with the given
	/// view handle.
...
impl lucie_text::TextPainter for Window {
	...
	fn paint_strikethrough(&mut self, origin: Point<ScaledPixels>, width: ScaledPixels, style: &StrikethroughStyle) {
		...
		let bounds = Bounds {
			origin,
			size: size(width, height.scale(scale_factor))
		};
		let content_mask = self.content_mask();
		let opacity = self.element_opacity();
	...
	fn paint_background(&mut self, bounds: Bounds<ScaledPixels>, background: Hsla) {
		...
		self.next_frame.scene.insert_primitive(Quad {
			...
			bounds,
			content_mask: content_mask.scale(scale_factor),
			background: background.opacity(opacity).into(),
			..Default::default()
		});
	}
	...
	fn paint_glyph(
		&mut self,
		glyph: lucie_text::PositionedGlyph<'_>,
		font: &lucie_text::LoadedFont,
		run_data: &lucie_text::RunData<'_>,
		color: Hsla
	) -> Result<(), Self::Error> {
		self.invalidator.debug_assert_paint();
		...
		if let Some(AtlasTileWithMetadata { tile, draw_offset }) = self.sprite_atlas.get_or_insert_with(&glyph.key(font.handle()).into(), &mut || {
			...
			let element_opacity = self.element_opacity();
			let bounds = Bounds {
				origin: glyph.origin.map(|x| ScaledPixels(x.0 as f32)) + draw_offset,
...
impl AnyWindowHandle {
	...
	where
		...
		T: 'static
	{
		let view = self.downcast::<T>().context("the type of the window's root view has changed")?;
