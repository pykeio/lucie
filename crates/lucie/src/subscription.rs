	...
	Callback: 'static
{
	...
	/// `[SubscriberSet::retain]`. This method returns a tuple of a [`Subscription`] and an `impl FnOnce`, and you can
	/// use the latter to activate the [`Subscription`].
	pub fn insert(&self, emitter_key: EmitterKey, callback: Callback) -> (Subscription, impl FnOnce() + use<EmitterKey, Callback>) {
		...
		lock.subscribers
			...
			.insert(subscriber_id, Subscriber { active: active.clone(), callback });
	...
	pub fn remove(&self, emitter: &EmitterKey) -> impl IntoIterator<Item = Callback> + use<EmitterKey, Callback> {
		...
		subscribers
			...
			.into_iter()
			.flatten()
			.filter_map(|subscriber| if subscriber.active.get() { Some(subscriber.callback) } else { None })
	}
		...
		F: FnMut(&mut Callback) -> bool
	{
		...
		subscribers.retain(|_, subscriber| if subscriber.active.get() { f(&mut subscriber.callback) } else { true });
		let mut lock = self.0.borrow_mut();
...
/// A handle to a subscription created by Lucie. When dropped, the subscription
