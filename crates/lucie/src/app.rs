...
/// A reference to a Lucie application, typically constructed in the `main` function of your app.
...
pub struct SystemWindowTabController {
	visible: Option<bool>,
	tab_groups: RapidHashMap<usize, Vec<SystemWindowTab>>
}
...
impl SystemWindowTabController {
	...
	pub fn new() -> Self {
		Self {
			visible: None,
			tab_groups: RapidHashMap::default()
		}
	}
	...
	/// Get all tab groups.
	pub fn tab_groups(&self) -> &RapidHashMap<usize, Vec<SystemWindowTab>> {
		&self.tab_groups
	}
	...
	pub fn tabs(&self, id: WindowId) -> Option<&Vec<SystemWindowTab>> {
		self.tab_groups.values().find(|tabs| tabs.iter().any(|tab| tab.id == id))
	}
	...
	pub fn update_tab_title(cx: &mut App, id: WindowId, title: SharedString) {
		let controller = cx.global::<SystemWindowTabController>();
		let tab = controller.tab_groups.values().flat_map(|windows| windows.iter()).find(|tab| tab.id == id);
	...
	pub fn add_tab(cx: &mut App, id: WindowId, tabs: Vec<SystemWindowTab>) {
		...
		let mut expected_tab_ids: Vec<_> = tabs.iter().filter(|tab| tab.id != id).map(|tab| tab.id).sorted().collect();
	...
	pub fn select_previous_tab(cx: &mut App, id: WindowId) {
		...
		let current_index = tabs.iter().position(|tab| tab.id == id).unwrap();
		let previous_index = if current_index == 0 { tabs.len() - 1 } else { current_index - 1 };
...
impl RuntimeMode {
	#[cfg(any(test, feature = "test-support"))]
	pub fn test() -> Self {
		RuntimeMode::Test { skip_drawing: false }
	}
	...
	pub(crate) fn skip_drawing(&self) -> bool {
		match self {
			#[cfg(any(test, feature = "test-support"))]
			RuntimeMode::Test { skip_drawing } => *skip_drawing,
...
pub struct App {
	...
	pub(crate) globals_by_type: RapidHashMap<TypeId, Box<dyn Any>>,
	...
	pub(crate) window_handles: RapidHashMap<WindowId, AnyWindowHandle>,
	pub(crate) focus_handles: Arc<FocusMap>,
	pub(crate) keymap: Rc<RefCell<Keymap>>,
	pub(crate) keyboard_layout: Box<dyn PlatformKeyboardLayout>,
	pub(crate) keyboard_mapper: Rc<dyn PlatformKeyboardMapper>,
	pub(crate) global_action_listeners: RapidHashMap<TypeId, Vec<Rc<dyn Fn(&dyn Any, DispatchPhase, &mut Self)>>>,
	pending_effects: VecDeque<Effect>,
	pub(crate) pending_notifications: RapidHashSet<EntityId>,
	pub(crate) pending_global_notifications: RapidHashSet<TypeId>,
	...
	pub(crate) window_invalidators_by_entity: RapidHashMap<EntityId, RapidHashMap<WindowId, WindowInvalidator>>,
	pub(crate) tracked_entities: RapidHashMap<WindowId, RapidHashSet<EntityId>>,
...
impl App {
	...
	pub(crate) fn new_app(platform: Rc<dyn Platform>, liveness: Arc<()>, asset_source: Arc<dyn AssetSource>, http_client: Arc<dyn HttpClient>) -> Rc<AppCell> {
		...
		let app = Rc::new_cyclic(|this| AppCell {
			app: RefCell::new(App {
				...
				asset_source,
				http_client,
				globals_by_type: RapidHashMap::default(),
				entities,
				new_entity_observers: SubscriberSet::new(),
				windows: SlotMap::with_key(),
				window_update_stack: Vec::new(),
				window_handles: RapidHashMap::default(),
				focus_handles: Arc::new(RwLock::new(SlotMap::with_key())),
				keymap: Rc::new(RefCell::new(Keymap::default())),
				keyboard_layout,
				keyboard_mapper,
				global_action_listeners: RapidHashMap::default(),
				pending_effects: VecDeque::new(),
				pending_notifications: RapidHashSet::default(),
				pending_global_notifications: RapidHashSet::default(),
				observers: SubscriberSet::new(),
				tracked_entities: RapidHashMap::default(),
				window_invalidators_by_entity: RapidHashMap::default(),
		...
		platform.on_keyboard_layout_change(Box::new({
			...
			move || {
				if let Some(app) = app.upgrade() {
					...
					cx.keyboard_layout_observers.clone().retain(&(), move |callback| (callback)(cx));
				}
			}
		}));
	...
	pub fn observe<W>(&mut self, entity: &Entity<W>, mut on_notify: impl FnMut(Entity<W>, &mut App) + 'static) -> Subscription
	where
		W: 'static
	{
	...
	pub(crate) fn detect_accessed_entities<R>(&mut self, callback: impl FnOnce(&mut App) -> R) -> (R, RapidHashSet<EntityId>) {
		...
		let entities_accessed_in_callback = accessed_entities_end
			...
			.copied()
			.collect::<RapidHashSet<EntityId>>();
		(result, entities_accessed_in_callback)
	}

	pub(crate) fn record_entities_accessed(&mut self, window_handle: AnyWindowHandle, invalidator: WindowInvalidator, entities: &RapidHashSet<EntityId>) {
	...
	pub(crate) fn observe_internal<W>(&mut self, entity: &Entity<W>, mut on_notify: impl FnMut(Entity<W>, &mut App) -> bool + 'static) -> Subscription
	where
		W: 'static
	{
		let entity_id = entity.entity_id();
		let handle = entity.downgrade();
		self.new_observer(entity_id, Box::new(move |cx| if let Some(entity) = handle.upgrade() { on_notify(entity, cx) } else { false }))
	}
	...
	pub fn subscribe<T, Event>(&mut self, entity: &Entity<T>, mut on_event: impl FnMut(Entity<T>, &Event, &mut App) + 'static) -> Subscription
	where
		...
		self.new_subscription(
			...
			(
				...
				Box::new(move |event, cx| {
					...
					if let Some(entity) = handle.upgrade() { on_event(entity, event, cx) } else { false }
				})
			)
		)
	}
		...
		loop {
			...
			if let Some(effect) = self.pending_effects.pop_front() {
				...
				for window in self
					...
					.collect::<Vec<_>>()
				{
					self.update_window(window, |_, window, cx| window.draw(cx).clear()).unwrap();
				}
	...
	fn apply_notify_global_observers_effect(&mut self, type_id: TypeId) {
		...
		self.global_observers.clone().retain(&type_id, |observer| observer(self));
	}
		...
		R: 'static
	{
		...
		self.foreground_executor.spawn_with_priority(priority, async move { f(&mut cx).await })
	}
	...
	pub fn observe_new<T: 'static>(&self, on_new: impl 'static + Fn(&mut T, Option<&mut Window>, &mut Context<T>)) -> Subscription {
		self.new_entity_observer(
			TypeId::of::<T>(),
			Box::new(move |any_entity: AnyEntity, window: &mut Option<&mut Window>, cx: &mut App| {
				any_entity
					.downcast::<T>()
					.unwrap()
	...
	pub fn observe_keystrokes(&mut self, mut f: impl FnMut(&KeystrokeEvent, &mut Window, &mut App) + 'static) -> Subscription {
		fn inner(keystroke_observers: &SubscriberSet<(), KeystrokeObserver>, handler: KeystrokeObserver) -> Subscription {
	...
	pub fn intercept_keystrokes(&mut self, mut f: impl FnMut(&KeystrokeEvent, &mut Window, &mut App) + 'static) -> Subscription {
		fn inner(keystroke_interceptors: &SubscriberSet<(), KeystrokeObserver>, handler: KeystrokeObserver) -> Subscription {
	...
	pub fn build_action(&self, name: &str, data: Option<&mut [u8]>) -> std::result::Result<Box<dyn Action>, ActionBuildError> {
	...
	pub fn deprecated_actions_to_preferred_actions(&self) -> &RapidHashMap<&'static str, &'static str> {
		self.actions.deprecated_aliases()
	}

	/// Get a map from an action name to the deprecation messages.
	pub fn action_deprecation_messages(&self) -> &RapidHashMap<&'static str, &'static str> {
		self.actions.deprecation_messages()
	}

	/// Get a map from an action name to the documentation.
	pub fn action_documentation(&self) -> &RapidHashMap<&'static str, &'static str> {
	...
	pub fn is_action_available(&mut self, action: &dyn Action) -> bool {
		...
		if let Some(window) = self.active_window()
			&& let Ok(window_action_available) = window.update(self, |_, window, cx| window.is_action_available(action, cx))
		{
	...
	fn dispatch_global_action(&mut self, action: &dyn Action) {
		...
		if let Some(mut global_listeners) = self.global_action_listeners.remove(&action.as_any().type_id()) {
			...
			global_listeners.extend(self.global_action_listeners.remove(&action.as_any().type_id()).unwrap_or_default());

			self.global_action_listeners.insert(action.as_any().type_id(), global_listeners);
		}
	...
	/// Set the prompt renderer. This will replace the default or platform specific
	...
	/// Tell the entity system that an entity has changed and observers of it should be notified.
	pub fn notify(&mut self, entity_id: EntityId) {
		...
		self.window_invalidators_by_entity.insert(entity_id, window_invalidators);
	}
	...
	///
	/// If the current window is being updated, it will be removed from `App.windows`, you can use `current_window` to
	/// specify the current window. This is a no-op if the image is not in the sprite atlas.
	...
	/// Initializes default colors for the application.
	///
...
impl AppContext for App {
	...
	fn insert_entity<T: 'static>(&mut self, reservation: Reservation<T>, build_entity: impl FnOnce(&mut Context<T>) -> T) -> Entity<T> {
		self.update(|cx| {
	...
	fn as_mut<'a, T>(&'a mut self, handle: &Entity<T>) -> BorrowedEntity<'a, T>
	where
		T: 'static
	{
		BorrowedEntity::new(handle.clone(), self)
	}
		...
		T: 'static
	{
		...
		let root_view = window.root.clone().unwrap();
		let view = root_view.downcast::<T>().map_err(|_| anyhow!("root view's type has changed"))?;
...
impl<G: Global> GlobalLease<G> {
	fn new(global: Box<dyn Any>) -> Self {
		GlobalLease { global, global_type: PhantomData }
	}
}
...
impl HttpClient for NullHttpClient {
	fn send(&self, _req: http::Request<AsyncBody>) -> BoxFuture<'static, anyhow::Result<http::Response<AsyncBody>>> {
		async move {
...
/// A mutable reference to an entity owned by Lucie
pub struct BorrowedEntity<'a, T> {
	inner: Option<Lease<T>>,
	app: &'a mut App
}

impl<'a, T: 'static> BorrowedEntity<'a, T> {
	fn new(inner: Entity<T>, app: &'a mut App) -> Self {
		app.start_update();
		let lease = app.entities.lease(&inner);
		Self { inner: Some(lease), app }
	}
}

impl<'a, T: 'static> std::borrow::Borrow<T> for BorrowedEntity<'a, T> {
	fn borrow(&self) -> &T {
		self.inner.as_ref().unwrap().borrow()
	}
}

impl<'a, T: 'static> std::borrow::BorrowMut<T> for BorrowedEntity<'a, T> {
	fn borrow_mut(&mut self) -> &mut T {
		self.inner.as_mut().unwrap().borrow_mut()
	}
}

impl<'a, T: 'static> std::ops::Deref for BorrowedEntity<'a, T> {
	type Target = T;
...
impl<'a, T: 'static> std::ops::DerefMut for BorrowedEntity<'a, T> {
	fn deref_mut(&mut self) -> &mut T {
		self.inner.as_mut().unwrap()
	}
}

impl<'a, T> Drop for BorrowedEntity<'a, T> {
	fn drop(&mut self) {
