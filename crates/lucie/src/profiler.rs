	...
	pub(crate) fn convert(timings: &[GlobalThreadTimings]) -> Vec<Self> {
		timings
			...
			.map(|(thread_id, timings)| {
				...
				ThreadTaskTimings { thread_name, thread_id, timings: vec }
			})
			.collect()
	}
}
...
pub(crate) type GuardedTaskTimings = parking_lot::Mutex<ThreadTimings>;
...
pub(crate) static GLOBAL_THREAD_TIMINGS: parking_lot::Mutex<Vec<GlobalThreadTimings>> = parking_lot::Mutex::new(Vec::new());
...
impl Drop for ThreadTimings {
	fn drop(&mut self) {
		...
		let Some((index, _)) = thread_timings.iter().enumerate().find(|(_, t)| t.thread_id == self.thread_id) else {
			return;
		};
