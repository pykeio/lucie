...
impl<'a, T: 'static> Context<'a, T> {
	...
	pub fn observe<W>(&mut self, entity: &Entity<W>, mut on_notify: impl FnMut(&mut T, Entity<W>, &mut Context<T>) + 'static) -> Subscription
	where
		T: 'static,
		W: 'static
	{
		...
		T: 'static
	{
		...
		self.app.observe(&this, move |this, cx| this.update(cx, |this, cx| on_event(this, cx)))
	}

	/// Subscribe to an event type from another entity
	pub fn subscribe<T2, Evt>(&mut self, entity: &Entity<T2>, mut on_event: impl FnMut(&mut T, Entity<T2>, &Evt, &mut Context<T>) + 'static) -> Subscription
	where
		T: 'static,
	...
	pub fn subscribe_self<Evt>(&mut self, mut on_event: impl FnMut(&mut T, &Evt, &mut Context<T>) + 'static) -> Subscription
	where
		...
		Evt: 'static
	{
		let this = self.entity();
		self.app
			.subscribe(&this, move |this, evt, cx| this.update(cx, |this, cx| on_event(this, evt, cx)))
	}

	/// Register a callback to be invoked when Lucie releases this entity.
	...
	pub fn observe_release<T2>(&self, entity: &Entity<T2>, on_release: impl FnOnce(&mut T, &mut T2, &mut Context<T>) + 'static) -> Subscription
	where
		T: Any,
		T2: 'static
	{
		...
		T: 'static
	{
		...
		let (subscription, activate) = self
			.global_observers
			.insert(TypeId::of::<G>(), Box::new(move |cx| handle.update(cx, |view, cx| f(view, cx)).is_ok()));
	...
	/// The future returned from this callback will be polled for up to [crate::SHUTDOWN_TIMEOUT] until the app fully
	/// quits.
	...
	/// Tell Lucie that this entity has changed and observers of it should be notified.
	...
	///
	/// Many Lucie callbacks take the form of `Fn(&E, &mut Window, &mut App)`,
	...
	/// The callback will be invoked with a reference to the current view, a handle to the emitting `Entity`, the event,
	/// a mutable reference to the `Window`, and the context for the entity.
	...
	{
		let observer = self.weak_entity();
		self.app.observe_release_in(observed, window, move |observed, window, cx| {
			observer.update(cx, |observer, cx| on_release(observer, observed, window, cx)).ok();
		})
	}

	/// Register a callback to be invoked when the window is resized.
	pub fn observe_window_bounds(&self, window: &mut Window, mut callback: impl FnMut(&mut T, &mut Window, &mut Context<T>) + 'static) -> Subscription {
		let view = self.weak_entity();
		let (subscription, activate) = window
			.bounds_observers
			.insert((), Box::new(move |window, cx| view.update(cx, |view, cx| callback(view, window, cx)).is_ok()));
		activate();
		subscription
	}

	/// Register a callback to be invoked when the window is activated or deactivated.
	pub fn observe_window_activation(&self, window: &mut Window, mut callback: impl FnMut(&mut T, &mut Window, &mut Context<T>) + 'static) -> Subscription {
		let view = self.weak_entity();
		let (subscription, activate) = window
	...
	pub fn observe_window_appearance(&self, window: &mut Window, mut callback: impl FnMut(&mut T, &mut Window, &mut Context<T>) + 'static) -> Subscription {
		let view = self.weak_entity();
		let (subscription, activate) = window
	...
	pub fn observe_keystrokes(&mut self, mut f: impl FnMut(&mut T, &KeystrokeEvent, &mut Window, &mut Context<T>) + 'static) -> Subscription {
	...
	pub fn observe_pending_input(&self, window: &mut Window, mut callback: impl FnMut(&mut T, &mut Window, &mut Context<T>) + 'static) -> Subscription {
		let view = self.weak_entity();
		let (subscription, activate) = window
	...
	pub fn on_focus(
		...
		let (subscription, activate) = window.new_focus_listener(Box::new(move |event, window, cx| {
			view.update(cx, |view, cx| {
				if event.previous_focus_path.last() != Some(&focus_id) && event.current_focus_path.last() == Some(&focus_id) {
	...
	/// Returns a subscription and persists until the subscription is dropped.
	pub fn on_focus_lost(&mut self, window: &mut Window, mut listener: impl FnMut(&mut T, &mut Window, &mut Context<T>) + 'static) -> Subscription {
		let view = self.weak_entity();
		let (subscription, activate) = window
			.focus_lost_listeners
			.insert((), Box::new(move |window, cx| view.update(cx, |view, cx| listener(view, window, cx)).is_ok()));
		self.defer(|_| activate());
		subscription
	}
	...
	pub fn observe_global_in<G: Global>(&mut self, window: &Window, mut f: impl FnMut(&mut T, &mut Window, &mut Context<T>) + 'static) -> Subscription {
		let window_handle = window.handle;
		let view = self.weak_entity();
		let (subscription, activate) = self.global_observers.insert(
			TypeId::of::<G>(),
			Box::new(move |cx| {
				// remove observer if the entity has been dropped
				...
				window_handle
					.update(cx, |_, window, cx| view.update(cx, |view, cx| f(view, window, cx)).is_ok())
					.unwrap_or(true)
			})
		);
		self.defer(move |_| activate());
...
impl<T> Context<'_, T> {
	/// Emit an event of the specified type, which can be handled by other entities that have subscribed via `subscribe`
	/// methods on their respective contexts.
	pub fn emit<Evt>(&mut self, event: Evt)
	where
		T: EventEmitter<Evt>,
		Evt: 'static
	{
		let event = self.event_arena.alloc(|| event).map(|it| it as &mut dyn Any);
...
impl<T> AppContext for Context<'_, T> {
	...
	#[inline]
	fn update_entity<U: 'static, R>(&mut self, handle: &Entity<U>, update: impl FnOnce(&mut U, &mut Context<U>) -> R) -> R {
		self.app.update_entity(handle, update)
	}

	#[inline]
	fn as_mut<'a, E>(&'a mut self, handle: &Entity<E>) -> super::BorrowedEntity<'a, E>
	where
		E: 'static
	{
