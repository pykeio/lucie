...
impl TestAppContext {
	...
	pub fn build(dispatcher: TestDispatcher, fn_name: Option<&'static str>) -> Self {
		...
		app.borrow_mut().mode = RuntimeMode::test();
	...
	pub fn skip_drawing(&mut self) {
		self.app.borrow_mut().mode = RuntimeMode::Test { skip_drawing: true };
	}
	...
	pub fn events<Evt, T: 'static + EventEmitter<Evt>>(&mut self, entity: &Entity<T>) -> mpsc::UnboundedReceiver<Evt>
	where
		Evt: 'static + Clone
	{
		let (tx, rx) = mpsc::unbounded();
...
impl<V> Entity<V> {
	...
	pub fn condition<Evt>(&self, cx: &TestAppContext, mut predicate: impl FnMut(&V, &App) -> bool) -> impl Future<Output = ()>
	where
		Evt: 'static,
		V: EventEmitter<Evt>
	{
...
impl VisualTestContext {
	...
	pub fn update<R>(&mut self, f: impl FnOnce(&mut Window, &mut App) -> R) -> R {
		self.cx.update_window(self.window, |_, window, cx| f(window, cx)).unwrap()
	}
	...
	pub fn simulate_mouse_move(&mut self, position: Point<Pixels>, button: impl Into<Option<MouseButton>>, modifiers: Modifiers) {
	...
	pub fn simulate_close(&mut self) -> bool {
		let handler = self
			.cx
			.update_window(self.window, |_, window, _| window.platform_window.as_test().unwrap().0.lock().should_close_handler.take())
			.unwrap();
...
impl AppContext for VisualTestContext {
	...
	fn as_mut<'a, T>(&'a mut self, handle: &Entity<T>) -> super::BorrowedEntity<'a, T>
	where
		T: 'static
	{
...
impl VisualContext for VisualTestContext {
	...
	fn update_window_entity<V: 'static, R>(&mut self, view: &Entity<V>, update: impl FnOnce(&mut V, &mut Window, &mut Context<V>) -> R) -> Self::Result<R> {
		self.window
			.update(&mut self.cx, |_, window, cx| view.update(cx, |v, cx| update(v, window, cx)))
			.unwrap()
	}
	...
	fn focus<V: crate::Focusable>(&mut self, view: &Entity<V>) -> Self::Result<()> {
		self.window
			.update(&mut self.cx, |_, window, cx| view.read(cx).focus_handle(cx).focus(window, cx))
			.unwrap()
	}
}
...
impl AnyWindowHandle {
	...
	pub fn build_entity<V: Render + 'static>(&self, cx: &mut TestAppContext, build_view: impl FnOnce(&mut Window, &mut Context<V>) -> V) -> Entity<V> {
		self.update(cx, |_, window, cx| cx.new(|cx| build_view(window, cx))).unwrap()
	}
}
