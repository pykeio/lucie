...
use super::Context;
use crate::{App, AppContext, BorrowedEntity, VisualContext, Window, seal::Sealed};
...
pub(crate) struct EntityMap {
	...
	pub accessed_entities: RefCell<RapidHashSet<EntityId>>,
...
impl EntityMap {
	pub fn new() -> Self {
		Self {
			...
			accessed_entities: RefCell::new(RapidHashSet::default()),
			ref_counts: Arc::new(RwLock::new(EntityRefCounts {
				...
				leak_detector: LeakDetector {
					next_handle_id: 0,
					entity_handles: RapidHashMap::default()
				}
			}))
		}
	}
	...
	fn assert_valid_context(&self, entity: &AnyEntity) {
		debug_assert!(Weak::ptr_eq(&entity.entity_map, &Arc::downgrade(&self.ref_counts)), "used a entity with the wrong context");
	}

	pub fn extend_accessed(&mut self, entities: &RapidHashSet<EntityId>) {
		self.accessed_entities.get_mut().extend(entities.iter().copied());
	}
...
impl AnyEntity {
	fn new(id: EntityId, entity_type: TypeId, entity_map: Weak<RwLock<EntityRefCounts>>) -> Self {
		Self {
			...
			entity_type,
			#[cfg(any(test, feature = "leak-detection"))]
			handle_id: entity_map.clone().upgrade().unwrap().write().leak_detector.handle_created(id),
			entity_map
		}
	}
...
impl Clone for AnyEntity {
	fn clone(&self) -> Self {
		if let Some(entity_map) = self.entity_map.upgrade() {
			...
			let count = entity_map.counts.get(self.entity_id).expect("detected over-release of a entity");
		...
		Self {
			...
			#[cfg(any(test, feature = "leak-detection"))]
			handle_id: self.entity_map.upgrade().unwrap().write().leak_detector.handle_created(self.entity_id)
		}
	}
}
...
impl Drop for AnyEntity {
	fn drop(&mut self) {
		if let Some(entity_map) = self.entity_map.upgrade() {
			...
			let count = entity_map.counts.get(self.entity_id).expect("detected over-release of a handle.");
		...
		#[cfg(any(test, feature = "leak-detection"))]
		if let Some(entity_map) = self.entity_map.upgrade() {
			entity_map.write().leak_detector.handle_released(self.entity_id, self.handle_id)
		}
	}
}
...
impl std::fmt::Debug for AnyEntity {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("AnyEntity").field("entity_id", &self.entity_id.as_u64()).finish()
	}
}
...
impl<T: 'static> Entity<T> {
	...
	/// Updates the entity referenced by this handle with the given function.
	#[inline]
	pub fn as_mut<'a, C: AppContext>(&self, cx: &'a mut C) -> BorrowedEntity<'a, T> {
		cx.as_mut(self)
	}
	...
	#[inline]
	pub fn update_in<R, C: VisualContext>(&self, cx: &mut C, update: impl FnOnce(&mut T, &mut Window, &mut Context<T>) -> R) -> C::Result<R> {
...
impl AnyWeakEntity {
	...
	/// - Panics if the entity was recently dropped but cleanup hasn't completed yet (resources are retained until the
	///   end of the effect cycle).
	#[cfg(any(test, feature = "leak-detection"))]
...
#[cfg(any(test, feature = "leak-detection"))]
static LEAK_BACKTRACE: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| std::env::var("LEAK_BACKTRACE").is_ok_and(|b| !b.is_empty()));
...
/// # How It Works
///
/// - When an entity handle is created (via `Entity::new`, `Entity::clone`, or `WeakEntity::upgrade`), `handle_created`
///   is called to register the handle.
...
pub(crate) struct LeakDetector {
	next_handle_id: u64,
	entity_handles: RapidHashMap<EntityId, RapidHashMap<HandleId, Option<backtrace::Backtrace>>>
}
