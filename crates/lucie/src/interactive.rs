...
impl ClickEvent {
	...
	pub fn is_right_click(&self) -> bool {
		match self {
			...
			ClickEvent::Mouse(event) => event.down.button == MouseButton::Right && event.up.button == MouseButton::Right
		}
	}
	...
	pub fn is_middle_click(&self) -> bool {
		match self {
			ClickEvent::Keyboard(_) => false,
			ClickEvent::Mouse(event) => event.down.button == MouseButton::Middle && event.up.button == MouseButton::Middle
		}
	}
...
mod test {
	...
	use crate::{
		self as lucie, AppContext as _, Context, FocusHandle, InteractiveElement, IntoElement, KeyBinding, Keystroke, ParentElement, Render, TestAppContext,
		Window, div
	};
		...
		fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
			div().id("testview").child(
				div()
					...
					.child(div().key_context("nested").track_focus(&self.focus_handle).into_element())
			)
		}
	}
