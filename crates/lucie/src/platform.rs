...
pub(crate) fn current_platform(headless: bool, liveness: std::sync::Weak<()>) -> Rc<dyn Platform> {
	...
	match guess_compositor() {
		...
		#[cfg(feature = "x11")]
		"X11" => Rc::new(X11Client::new(liveness).context("Failed to initialize X11 client.").unwrap()),
...
pub struct RunnableMeta {
	...
	pub liveness: Option<std::sync::Weak<()>>
}
...
impl ClipboardItem {
	...
	pub fn new_string_with_metadata(text: String, metadata: String) -> Self {
		Self {
			entries: vec![ClipboardEntry::String(ClipboardString { text, metadata: Some(metadata) })]
		}
	}
	...
	pub fn metadata(&self) -> Option<&String> {
		match self.entries().first() {
			Some(ClipboardEntry::String(clipboard_string)) if self.entries.len() == 1 => clipboard_string.metadata.as_ref(),
			_ => None
		}
	}
...
impl Image {
	...
	/// Use the `use_asset` API to make this image renderable
	...
	/// Use the `get_asset` API to make this image renderable
	...
	/// Use the `remove_asset` API to drop this image, if possible.
