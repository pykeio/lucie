...
pub trait AsKeystroke {
	/// Returns the Lucie-native representation of the keystroke.
...
pub struct KeybindingKeystroke {
	/// The Lucie-native representation of the keystroke.
	inner: Keystroke,
...
impl Display for InvalidKeystrokeError {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		write!(f, "Invalid keystroke \"{}\". {}", self.keystroke, KEYSTROKE_PARSE_EXPECTED_MESSAGE)
	}
}
...
impl Keystroke {
	...
	pub fn parse(source: &str) -> std::result::Result<Self, InvalidKeystrokeError> {
		...
		while let Some(component) = components.next() {
			...
			if let Some(next) = components.peek() {
				if next.is_empty() && source.ends_with('-') {
					...
					return Err(InvalidKeystrokeError { keystroke: source.to_owned() });
				}
				continue;
			}
		...
		let key = key.ok_or_else(|| InvalidKeystrokeError { keystroke: source.to_owned() })?;

		Ok(Keystroke { modifiers, key, key_char })
	}
	...
	pub fn with_simulated_ime(mut self) -> Self {
		if self.key_char.is_none() && !self.modifiers.platform && !self.modifiers.control && !self.modifiers.function && !self.modifiers.alt {
...
impl KeybindingKeystroke {
	...
	pub fn new_with_mapper(inner: Keystroke, use_key_equivalents: bool, keyboard_mapper: &dyn PlatformKeyboardMapper) -> Self {
	...
	/// - On macOS and Linux, this modifiers is the same as `inner.modifiers`, which is the Lucie-native representation
	///   of the keystroke.
	/// - On Windows, this modifiers is the display modifiers, for example, a `ctrl-@` keystroke will have
	///   `inner.modifiers` as
	...
	///
	/// Platform-specific behavior:
	/// - On macOS and Linux, this key is the same as `inner.key`, which is the Lucie-native representation of the
	///   keystroke.
	/// - On Windows, this key is the display key, for example, a `ctrl-@` keystroke will have `inner.key` as `@` and
	///   `display_key` as `2`.
	pub fn key(&self) -> &str {
		#[cfg(target_os = "windows")]
		{
