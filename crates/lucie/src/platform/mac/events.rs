...
impl PlatformInput {
	pub(crate) unsafe fn from_native(native_event: id, window_height: Option<Pixels>) -> Option<Self> {
		unsafe {
			...
			match event_type {
				NSEventType::NSFlagsChanged => Some(Self::ModifiersChanged(ModifiersChangedEvent {
					...
					capslock: Capslock {
						on: native_event.modifierFlags().contains(NSEventModifierFlags::NSAlphaShiftKeyMask)
					}
				})),
				...
				NSEventType::NSScrollWheel => window_height.map(|window_height| {
					...
					let raw_data = point(native_event.scrollingDeltaX() as f32, native_event.scrollingDeltaY() as f32);
				...
				NSEventType::NSLeftMouseDragged | NSEventType::NSRightMouseDragged | NSEventType::NSOtherMouseDragged => {
...
unsafe fn parse_keystroke(native_event: id) -> Keystroke {
	unsafe {
		...
		let mut characters = native_event.charactersIgnoringModifiers().to_str().to_string();
...
fn chars_for_modified_key(code: CGKeyCode, modifiers: u32) -> String {
	...
	let layout_data = unsafe { TISGetInputSourceProperty(keyboard, kTISPropertyUnicodeKeyLayoutData as *const c_void) as CFDataRef };
