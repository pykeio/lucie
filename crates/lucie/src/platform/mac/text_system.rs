...
use core_text::{
	font::CTFont,
	font_descriptor::{kCTFontSlantTrait, kCTFontSymbolicTrait, kCTFontWeightTrait, kCTFontWidthTrait},
	line::CTLine,
...
struct MacTextSystemState {
	...
	font_ids_by_postscript_name: RapidHashMap<String, FontId>,
	font_ids_by_font_key: RapidHashMap<FontKey, SmallVec<[FontId; 4]>>,
	postscript_names_by_font_id: RapidHashMap<FontId, String>
}
...
impl MacTextSystem {
	pub(crate) fn new() -> Self {
		Self(RwLock::new(MacTextSystemState {
			...
			font_ids_by_postscript_name: RapidHashMap::default(),
			font_ids_by_font_key: RapidHashMap::default(),
			postscript_names_by_font_id: RapidHashMap::default()
		}))
	}
}
...
impl PlatformTextSystem for MacTextSystem {
	...
	fn font_id(&self, font: &Font) -> Result<FontId> {
		...
		if let Some(font_id) = lock.font_selections.get(font) {
			...
			let candidates = if let Some(font_ids) = lock.font_ids_by_font_key.get(&font_key) {
				...
				let font_ids = lock.load_family(&font.family, &font.features, font.fallbacks.as_ref())?;
	...
	fn typographic_bounds(&self, font_id: FontId, glyph_id: GlyphId) -> Result<Bounds<f32>> {
		Ok(self.0.read().fonts[font_id.0].typographic_bounds(glyph_id.0)?.into())
	}
...
impl MacTextSystemState {
		...
		let fonts = fonts
			...
			.map(|bytes| match bytes {
				Cow::Borrowed(embedded_font) => {
					let data_provider = unsafe { core_graphics::data_provider::CGDataProvider::from_slice(embedded_font) };
					let font = core_graphics::font::CGFont::from_data_provider(data_provider).map_err(|()| anyhow!("Could not load an embedded font."))?;
	...
	fn load_family(&mut self, name: &str, features: &FontFeatures, fallbacks: Option<&FontFallbacks>) -> Result<SmallVec<[FontId; 4]>> {
		...
		for font in family.fonts() {
			...
			if unsafe {
				!(traits.get(kCTFontSymbolicTrait).downcast::<CFNumber>().is_some()
					&& traits.get(kCTFontWidthTrait).downcast::<CFNumber>().is_some()
			...
			self.font_ids_by_postscript_name.insert(postscript_name.clone(), font_id);
			self.postscript_names_by_font_id.insert(font_id, postscript_name);
	...
	fn id_for_native_font(&mut self, requested_font: CTFont) -> FontId {
		...
		if let Some(font_id) = self.font_ids_by_postscript_name.get(&postscript_name) {
			*font_id
			...
			self.postscript_names_by_font_id.insert(font_id, postscript_name);
			self.fonts
				.push(font_kit::font::Font::from_core_graphics_font(requested_font.copy_to_CGFont()));
			font_id
		}
	}
	...
	fn is_emoji(&self, font_id: FontId) -> bool {
		self.postscript_names_by_font_id
			.get(&font_id)
			.is_some_and(|postscript_name| postscript_name == "AppleColorEmoji" || postscript_name == ".AppleColorEmojiUI")
	}
	...
	fn raster_bounds(&self, params: &RenderGlyphParams) -> Result<Bounds<DevicePixels>> {
		...
		Ok(font
			.raster_bounds(params.glyph_id.0, params.font_size.into(), scale, HintingOptions::None, font_kit::canvas::RasterizationOptions::GrayscaleAa)?
			.into())
	}
	...
	fn rasterize_glyph(&self, params: &RenderGlyphParams, glyph_bounds: Bounds<DevicePixels>) -> Result<(Size<DevicePixels>, Vec<u8>)> {
		if glyph_bounds.size.width.0 == 0 || glyph_bounds.size.height.0 == 0 {
			...
			cx.translate(-glyph_bounds.origin.x.0 as CGFloat, (glyph_bounds.origin.y.0 + glyph_bounds.size.height.0) as CGFloat);
			...
			self.fonts[params.font_id.0]
				...
				.draw_glyphs(
					...
					&[CGPoint::new((subpixel_shift.x / params.scale_factor) as CGFloat, (subpixel_shift.y / params.scale_factor) as CGFloat)],
					cx
				);
	...
	fn layout_line(&mut self, text: &str, font_size: Pixels, font_runs: &[FontRun]) -> LineLayout {
...

		{
			...
			for run in font_runs {
				...
				let font_size = if break_ligature { px(font_size.0.next_up()) } else { font_size };
				unsafe {
					string.set_attribute(cf_range, kCTFontAttributeName, &font.native_font().clone_with_font_size(font_size.into()));
				}
		...
		for run in glyph_runs.into_iter() {
			...
			let font = unsafe { attributes.get(kCTFontAttributeName).downcast::<CTFont>().unwrap() };
			...
			for ((&glyph_id, position), &glyph_utf16_ix) in run.glyphs().iter().zip(run.positions().iter()).zip(run.string_indices().iter()) {
...
mod lenient_font_attributes {
	...
	use core_text::font_descriptor::{CTFontDescriptor, CTFontDescriptorCopyAttribute, kCTFontFamilyNameAttribute};
	...
	fn get_string_attribute(descriptor: &CTFontDescriptor, attribute: CFStringRef) -> Option<String> {
		unsafe {
...
mod tests {
	...
	fn test_layout_line_bom_char() {
		...
		let line = "\u{feff}";
		let mut style = FontRun { font_id, len: line.len() };
		...
		let line = "\u{feff}ab";
		let font_runs = &[FontRun { len: "\u{feff}".len(), font_id }, FontRun { len: "ab".len(), font_id }];
	...
	fn test_layout_line_zwnj_insertion() {
		...
		for run in &layout.runs {
			for glyph in &run.glyphs {
				assert!(glyph.index < text.len(), "Glyph index {} is out of bounds for text length {}", glyph.index, text.len());
			}
		}
		...
		let font_runs_different = &[
			FontRun { font_id, len: 5 }, // "hello"
			// " world"
			FontRun { font_id: font_id2, len: 6 }
		];
