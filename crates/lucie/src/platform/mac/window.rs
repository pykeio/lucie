...
use cocoa::{
	appkit::{
		NSAppKitVersionNumber, NSAppKitVersionNumber12_0, NSApplication, NSBackingStoreBuffered, NSColor, NSEvent, NSEventModifierFlags, NSFilenamesPboardType,
		NSPasteboard, NSScreen, NSView, NSViewHeightSizable, NSViewWidthSizable, NSVisualEffectMaterial, NSVisualEffectState, NSVisualEffectView, NSWindow,
		NSWindowButton, NSWindowCollectionBehavior, NSWindowOcclusionState, NSWindowOrderingMode, NSWindowStyleMask, NSWindowTitleVisibility
	},
	base::{id, nil},
	foundation::{
		NSArray, NSAutoreleasePool, NSDictionary, NSFastEnumeration, NSInteger, NSNotFound, NSOperatingSystemVersion, NSPoint, NSProcessInfo, NSRect, NSSize,
...
const NSWindowStyleMaskNonactivatingPanel: NSWindowStyleMask = NSWindowStyleMask::from_bits_retain(1 << 7);
#[allow(non_upper_case_globals)]
...
unsafe extern "C" {
	...
	fn CGSSetWindowBackgroundBlurRadius(connection_id: id, window_id: NSInteger, radius: i64) -> i32;
}

#[ctor]
unsafe fn build_classes() {
	unsafe {
		WINDOW_CLASS = build_window_class("LucieWindow", class!(NSWindow));
		PANEL_CLASS = build_window_class("LuciePanel", class!(NSPanel));
		VIEW_CLASS = {
			let mut decl = ClassDecl::new("LucieView", class!(NSView)).unwrap();
			decl.add_ivar::<*mut c_void>(WINDOW_STATE_IVAR);
			unsafe {
				decl.add_method(sel!(dealloc), dealloc_view as extern "C" fn(&Object, Sel));

				decl.add_method(sel!(performKeyEquivalent:), handle_key_equivalent as extern "C" fn(&Object, Sel, id) -> BOOL);
				...
				decl.add_method(sel!(rightMouseDown:), handle_view_event as extern "C" fn(&Object, Sel, id));
				decl.add_method(sel!(rightMouseUp:), handle_view_event as extern "C" fn(&Object, Sel, id));
				...
				decl.add_method(sel!(makeBackingLayer), make_backing_layer as extern "C" fn(&Object, Sel) -> id);

				decl.add_protocol(Protocol::get("CALayerDelegate").unwrap());
				decl.add_method(sel!(viewDidChangeBackingProperties), view_did_change_backing_properties as extern "C" fn(&Object, Sel));
				decl.add_method(sel!(setFrameSize:), set_frame_size as extern "C" fn(&Object, Sel, NSSize));
				...
				decl.add_method(sel!(setMarkedText:selectedRange:replacementRange:), set_marked_text as extern "C" fn(&Object, Sel, id, NSRange, NSRange));
				decl.add_method(sel!(unmarkText), unmark_text as extern "C" fn(&Object, Sel));
				decl.add_method(
					sel!(attributedSubstringForProposedRange:actualRange:),
					attributed_substring_for_proposed_range as extern "C" fn(&Object, Sel, NSRange, *mut c_void) -> id
				);
				decl.add_method(sel!(viewDidChangeEffectiveAppearance), view_did_change_effective_appearance as extern "C" fn(&Object, Sel));
				...
				decl.add_method(sel!(characterIndexForPoint:), character_index_for_point as extern "C" fn(&Object, Sel, NSPoint) -> u64);
			}
			decl.register()
		};
		BLURRED_VIEW_CLASS = {
			...
			unsafe {
				decl.add_method(sel!(initWithFrame:), blurred_view_init_with_frame as extern "C" fn(&Object, Sel, NSRect) -> id);
...
unsafe fn build_window_class(name: &'static str, superclass: &Class) -> *const Class {
	unsafe {
		...
		decl.add_method(sel!(windowDidChangeOcclusionState:), window_did_change_occlusion_state as extern "C" fn(&Object, Sel, id));
		...
		decl.add_method(sel!(windowDidBecomeKey:), window_did_change_key_status as extern "C" fn(&Object, Sel, id));
		decl.add_method(sel!(windowDidResignKey:), window_did_change_key_status as extern "C" fn(&Object, Sel, id));
		decl.add_method(sel!(windowShouldClose:), window_should_close as extern "C" fn(&Object, Sel, id) -> BOOL);

		decl.add_method(sel!(close), close_window as extern "C" fn(&Object, Sel));

		decl.add_method(sel!(draggingEntered:), dragging_entered as extern "C" fn(&Object, Sel, id) -> NSDragOperation);
		decl.add_method(sel!(draggingUpdated:), dragging_updated as extern "C" fn(&Object, Sel, id) -> NSDragOperation);
		...
		decl.add_method(sel!(addTitlebarAccessoryViewController:), add_titlebar_accessory_view_controller as extern "C" fn(&Object, Sel, id));

		decl.add_method(sel!(moveTabToNewWindow:), move_tab_to_new_window as extern "C" fn(&Object, Sel, id));

		decl.add_method(sel!(mergeAllWindows:), merge_all_windows as extern "C" fn(&Object, Sel, id));

		decl.add_method(sel!(selectNextTab:), select_next_tab as extern "C" fn(&Object, Sel, id));

		decl.add_method(sel!(selectPreviousTab:), select_previous_tab as extern "C" fn(&Object, Sel, id));

		decl.add_method(sel!(toggleTabBar:), toggle_tab_bar as extern "C" fn(&Object, Sel, id));
...
impl MacWindowState {
	fn move_traffic_light(&self) {
		if let Some(traffic_light_position) = self.traffic_light_position {
			...
			unsafe {
				...
				let mut origin = point(traffic_light_position.x, titlebar_height - traffic_light_position.y - px(close_button_frame.size.height as f32));
	...
	fn start_display_link(&mut self) {
		...
		if let Some(mut display_link) = DisplayLink::new(display_id, self.native_view.as_ptr() as *mut c_void, step).log_err() {
	...
	fn bounds(&self) -> Bounds<Pixels> {
		...
		Bounds::new(
			point(px((window_frame.origin.x - screen_frame.origin.x) as f32), px((window_frame.origin.y + screen_frame.origin.y) as f32)),
...
impl MacWindow {
	pub fn open(
		...
		unsafe {
			...
			if let Some(titlebar) = titlebar.as_ref() {
				style_mask = NSWindowStyleMask::NSClosableWindowMask | NSWindowStyleMask::NSTitledWindowMask;
				...
				style_mask = NSWindowStyleMask::NSTitledWindowMask | NSWindowStyleMask::NSFullSizeContentViewWindowMask;
			}
			...
			let native_window =
				native_window.initWithContentRect_styleMask_backing_defer_screen_(window_rect, style_mask, NSBackingStoreBuffered, NO, target_screen);
			...
			let mut window = Self(Arc::new(Mutex::new(MacWindowState {
				...
				blurred_view: None,
				display_link: None,
				renderer: renderer::new_renderer(renderer_context, native_window as *mut _, native_view as *mut _, bounds.size.map(|pixels| pixels.0), false),
			...
			(*native_window).set_ivar(WINDOW_STATE_IVAR, Arc::into_raw(window.0.clone()) as *const c_void);
			native_window.setDelegate_(native_window);
			(*native_view).set_ivar(WINDOW_STATE_IVAR, Arc::into_raw(window.0.clone()) as *const c_void);
			...
			if allows_automatic_window_tabbing && !main_window.is_null() && main_window != native_window {
				let main_window_is_fullscreen = main_window.styleMask().contains(NSWindowStyleMask::NSFullScreenWindowMask);
				let user_tabbing_preference = Self::get_user_tabbing_preference().unwrap_or(UserTabbingPreference::InFullScreen);
				...
				if should_add_as_tab {
					let main_window_can_tab: BOOL = msg_send![main_window, respondsToSelector: sel!(addTabbedWindow:ordered:)];
					...
					if main_window_can_tab == YES && main_window_visible == YES {
						...
						// Ensure the window is visible immediately after adding the tab, since the tab bar is updated with a new entry at this
						// point. Note: Calling orderFront here can break fullscreen mode (makes fullscreen windows exit fullscreen), so
						// only do this if the main window is not fullscreen.
	...
	pub fn get_user_tabbing_preference() -> Option<UserTabbingPreference> {
		unsafe {
			...
			let value: id = if !dict.is_null() { msg_send![dict, objectForKey: key] } else { nil };
...
impl PlatformWindow for MacWindow {
	...
	fn merge_all_windows(&self) {
		...
		unsafe {
			dispatch_async_f(dispatch_get_main_queue(), native_window as *mut std::ffi::c_void, Some(merge_windows_async));
		}
	}
	...
	fn move_tab_to_new_window(&self) {
		...
		unsafe {
			dispatch_async_f(dispatch_get_main_queue(), native_window as *mut std::ffi::c_void, Some(move_tab_async));
		}
	}
	...
	fn display(&self) -> Option<Rc<dyn PlatformDisplay>> {
		unsafe {
			...
			let screen_number: id = NSDictionary::valueForKey_(device_description, NSString::alloc(nil).init_str("NSScreenNumber"));
	...
	fn mouse_position(&self) -> Point<Pixels> {
		let position = unsafe { self.0.lock().native_window.mouseLocationOutsideOfEventStream() };
	...
	fn prompt(&self, level: PromptLevel, msg: &str, detail: Option<&str>, answers: &[PromptButton]) -> Option<oneshot::Receiver<usize>> {
				...
				.filter(|&(ix, _)| Some(ix) != latest_non_cancel_label.map(|(ix, _)| ix))
			{
				...
				if answer.is_cancel() {
					...
					if let Some(key) = std::char::from_u32(super::events::ESCAPE_KEY as u32) {
						let _: () = msg_send![button, setKeyEquivalent: ns_string(&key.to_string())];
					}
				}
			}
	...
	fn get_title(&self) -> String {
		unsafe {
			...
			if title.is_null() { "".to_string() } else { title.to_str().to_string() }
		}
	}
	...
	fn set_background_appearance(&self, background_appearance: WindowBackgroundAppearance) {
		...
		unsafe {
			...
			if NSAppKitVersionNumber < NSAppKitVersionNumber12_0 {
				...
				let blur_radius = if background_appearance == WindowBackgroundAppearance::Blurred { 80 } else { 0 };
	...
	fn is_fullscreen(&self) -> bool {
		...
		unsafe { window.styleMask().contains(NSWindowStyleMask::NSFullScreenWindowMask) }
	}
		...
		this.executor
			.spawn(async move {
				unsafe {
					...
					let dict: id = msg_send![defaults, persistentDomainForName: domain];
					let action: id = if !dict.is_null() { msg_send![dict, objectForKey: key] } else { nil };
...
impl rwh::HasWindowHandle for MacWindow {
	fn window_handle(&self) -> Result<rwh::WindowHandle<'_>, rwh::HandleError> {
		...
		unsafe { Ok(rwh::WindowHandle::borrow_raw(rwh::RawWindowHandle::AppKit(rwh::AppKitWindowHandle::new(self.0.lock().native_view.cast())))) }
	}
}
...
//  U.S. layout:
//   - The IME consumes characters like 'j' and 'k', which makes paging through `less` in the terminal behave
//     incorrectly by default. This behavior should be patched by our IME integration
//   - `alt-t` should open the tasks menu
//   - In vim mode, this keybinding should work: ``` { "context": "Editor && vim_mode == insert", "bindings": {"j j":
//     "vim::NormalBefore"} } ``` and typing 'j k' in insert mode with this keybinding should insert the two characters
//  Brazilian layout:
...
extern "C" fn handle_key_event(this: &Object, native_event: id, key_equivalent: bool) -> BOOL {
	...
	let run_callback = |event: PlatformInput| -> BOOL {
		...
		let handled: BOOL = if let Some(callback) = callback.as_mut() { !callback(event).propagate as BOOL } else { NO };
	...
	match event {
		PlatformInput::KeyDown(mut key_down_event) => {
			...
			// If that event isn't handled, it will then dispatch a "key down" event. Lucie
			...
			if is_composing
				|| (key_down_event.keystroke.key_char.is_none() && !key_down_event.keystroke.modifiers.control && !key_down_event.keystroke.modifiers.function)
			{
				{
					let mut lock = window_state.as_ref().lock();
...
extern "C" fn handle_view_event(this: &Object, _: Sel, native_event: id) {
	...
	if let Some(mut event) = event {
		match &mut event {
			PlatformInput::MouseDown(
				...
				*event = MouseDownEvent {
					...
					modifiers: Modifiers { control: false, ..event.modifiers },
					click_count: 1,
					..*event
				};
			}
			...
			PlatformInput::MouseDown(event @ MouseDownEvent { button: MouseButton::Left, .. }) if (lock.first_mouse) => {
				*event = MouseDownEvent { first_mouse: true, ..*event };
		...
		match &event {
			...
			PlatformInput::MouseMove(event @ MouseMoveEvent { pressed_button: Some(_), .. }) => {
				...
				if !lock.external_files_dragged {
					...
					executor
						.spawn(synthetic_drag(weak_window_state, lock.synthetic_drag_counter, event.clone()))
						.detach();
				}
			}
...
extern "C" fn has_marked_text(this: &Object, _: Sel) -> BOOL {
	let has_marked_text_result = with_input_handler(this, |input_handler| input_handler.marked_text_range()).flatten();

	has_marked_text_result.is_some() as BOOL
}

extern "C" fn marked_range(this: &Object, _: Sel) -> NSRange {
	let marked_range_result = with_input_handler(this, |input_handler| input_handler.marked_text_range()).flatten();

	marked_range_result.map_or(NSRange::invalid(), |range| range.into())
}

extern "C" fn selected_range(this: &Object, _: Sel) -> NSRange {
	let selected_range_result = with_input_handler(this, |input_handler| input_handler.selected_text_range(false)).flatten();

	selected_range_result.map_or(NSRange::invalid(), |selection| selection.range.into())
}

extern "C" fn first_rect_for_character_range(this: &Object, _: Sel, range: NSRange, _: id) -> NSRect {
	let frame = get_frame(this);
	with_input_handler(this, |input_handler| input_handler.bounds_for_range(range.to_range()?))
		.flatten()
		.map_or(NSRect::new(NSPoint::new(0., 0.), NSSize::new(0., 0.)), |bounds| {
			NSRect::new(
				NSPoint::new(
					frame.origin.x + bounds.origin.x.0 as f64,
					frame.origin.y + frame.size.height - bounds.origin.y.0 as f64 - bounds.size.height.0 as f64
				),
				NSSize::new(bounds.size.width.0 as f64, bounds.size.height.0 as f64)
			)
		})
...
extern "C" fn insert_text(this: &Object, _: Sel, text: id, replacement_range: NSRange) {
	unsafe {
		...
		let text: id = if is_attributed_string == YES { msg_send![text, string] } else { text };

		let text = text.to_str();
		let replacement_range = replacement_range.to_range();
		with_input_handler(this, |input_handler| input_handler.replace_text_in_range(replacement_range, text));
	}
}

extern "C" fn set_marked_text(this: &Object, _: Sel, text: id, selected_range: NSRange, replacement_range: NSRange) {
	unsafe {
		let is_attributed_string: BOOL = msg_send![text, isKindOfClass: [class!(NSAttributedString)]];
		let text: id = if is_attributed_string == YES { msg_send![text, string] } else { text };
		...
		let replacement_range = replacement_range.to_range();
		let text = text.to_str();
		with_input_handler(this, |input_handler| input_handler.replace_and_mark_text_in_range(replacement_range, text, selected_range));
	}
}
...
extern "C" fn attributed_substring_for_proposed_range(this: &Object, _: Sel, range: NSRange, actual_range: *mut c_void) -> id {
...
extern "C" fn character_index_for_point(this: &Object, _: Sel, position: NSPoint) -> u64 {
	let position = screen_point_to_lucie_point(this, position);
	with_input_handler(this, |input_handler| input_handler.character_index_for_point(position))
		.flatten()
		.map(|index| index as u64)
		.unwrap_or(NSNotFound as u64)
}

fn screen_point_to_lucie_point(this: &Object, position: NSPoint) -> Point<Pixels> {
	let frame = get_frame(this);
...
extern "C" fn dragging_updated(this: &Object, _: Sel, dragging_info: id) -> NSDragOperation {
	let window_state = unsafe { get_window_state(this) };
	let position = drag_event_position(&window_state, dragging_info);
	if send_new_event(&window_state, PlatformInput::FileDrop(FileDropEvent::Pending { position })) {
		NSDragOperationCopy
	} else {
		NSDragOperationNone
	}
...
extern "C" fn perform_drag_operation(this: &Object, _: Sel, dragging_info: id) -> BOOL {
	let window_state = unsafe { get_window_state(this) };
	let position = drag_event_position(&window_state, dragging_info);
	send_new_event(&window_state, PlatformInput::FileDrop(FileDropEvent::Submit { position })).to_objc()
}
...
async fn synthetic_drag(window_state: Weak<Mutex<MacWindowState>>, drag_id: usize, event: MouseMoveEvent) {
	loop {
	...
	unsafe {
		...
		if !filters.is_null() {
			...
			for i in 0..count {
				...
				let all_indices = NSRange { location: 0, length: count };
