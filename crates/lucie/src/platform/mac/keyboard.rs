...
use super::{TISCopyCurrentKeyboardLayoutInputSource, TISGetInputSourceProperty, kTISPropertyInputSourceID, kTISPropertyLocalizedName};
...
pub(crate) struct MacKeyboardMapper {
	key_equivalents: Option<RapidHashMap<char, char>>
}
...
impl MacKeyboardLayout {
	pub(crate) fn new() -> Self {
		unsafe {
			...
			let id: *mut Object = TISGetInputSourceProperty(current_keyboard, kTISPropertyInputSourceID as *const c_void);
			let id: *const std::os::raw::c_char = msg_send![id, UTF8String];
			let id = CStr::from_ptr(id).to_str().unwrap().to_string();

			let name: *mut Object = TISGetInputSourceProperty(current_keyboard, kTISPropertyLocalizedName as *const c_void);
...
// output to remove languages with no mappings and other oddities, and converting it to a less verbose representation
// with:  jq -s 'map(to_entries | map({key: .key, value: [(.value | to_entries | map(.key) | join("")), (.value |
// to_entries | map(.value) | join(""))]}) | from_entries) | add' From there I used multi-cursor to produce this match
// statement.
fn get_key_equivalents(layout_id: &str) -> Option<RapidHashMap<char, char>> {
	let mappings: &[(char, char)] = match layout_id {
		...
		"com.apple.keylayout.Maltese" => &[('#', '£'), ('[', 'ġ'), (']', 'ħ'), ('`', 'ż'), ('{', 'Ġ'), ('}', 'Ħ'), ('~', 'Ż')],
	...
	Some(RapidHashMap::from_iter(mappings.iter().cloned()))
}
