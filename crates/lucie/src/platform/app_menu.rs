...
impl MenuItem {
	...
	pub fn os_submenu(name: impl Into<SharedString>, menu_type: SystemMenuType) -> Self {
		Self::SystemMenu(OsMenu { name: name.into(), menu_type })
	}
	...
	pub fn owned(self) -> OwnedMenuItem {
		match self {
			...
			MenuItem::Action { name, action, os_action, checked } => OwnedMenuItem::Action {
				name: name.into(),
				action,
				os_action,
				checked
	...
	pub fn checked(mut self, checked: bool) -> Self {
		match self {
			MenuItem::Action { action, os_action, name, .. } => MenuItem::Action { name, action, os_action, checked },
			_ => self
		}
	}
}
...
impl Clone for OwnedMenuItem {
	fn clone(&self) -> Self {
		match self {
			...
			OwnedMenuItem::Action { name, action, os_action, checked } => OwnedMenuItem::Action {
				name: name.clone(),
...
// be moved to Lucie-provided actions that make this association
// without leaking the platform details to Lucie users
