...
use windows::{
	Win32::{
		Foundation::HMODULE,
		Graphics::{
			Direct3D::{D3D_DRIVER_TYPE_UNKNOWN, D3D_FEATURE_LEVEL, D3D_FEATURE_LEVEL_10_1, D3D_FEATURE_LEVEL_11_0, D3D_FEATURE_LEVEL_11_1},
			Direct3D11::{
				D3D11_CREATE_DEVICE_BGRA_SUPPORT, D3D11_CREATE_DEVICE_DEBUG, D3D11_FEATURE_D3D10_X_HARDWARE_OPTIONS,
				D3D11_FEATURE_DATA_D3D10_X_HARDWARE_OPTIONS, D3D11_SDK_VERSION, D3D11CreateDevice, ID3D11Device, ID3D11DeviceContext
			},
			Dxgi::{CreateDXGIFactory2, DXGI_CREATE_FACTORY_DEBUG, DXGI_CREATE_FACTORY_FLAGS, IDXGIAdapter1, IDXGIFactory6}
		}
	},
	core::Interface
};
...
impl DirectXDevices {
	pub(crate) fn new() -> Result<Self> {
		...
		let (adapter, device, device_context, feature_level) = get_adapter(&dxgi_factory, debug_layer_available).context("Getting DXGI adapter")?;
		match feature_level {
...
#[inline]
fn get_adapter(dxgi_factory: &IDXGIFactory6, debug_layer_available: bool) -> Result<(IDXGIAdapter1, ID3D11Device, ID3D11DeviceContext, D3D_FEATURE_LEVEL)> {
	for adapter_index in 0.. {
		...
		if let Some(device) = get_device(&adapter, Some(&mut context), Some(&mut feature_level), debug_layer_available).log_err() {
...
fn get_device(
	...
	if data.ComputeShaders_Plus_RawAndStructuredBuffers_Via_Shader_4_x.as_bool() {
		Ok(device)
	} else {
		Err(anyhow::anyhow!("Required feature StructuredBuffer is not supported by GPU/driver"))
	}
}
