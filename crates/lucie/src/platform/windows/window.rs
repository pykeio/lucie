...
impl WindowsWindowState {
	fn new(
		...
		let renderer = DirectXRenderer::new(hwnd, directx_devices, disable_direct_composition).context("Creating DirectX renderer")?;
	...
	///
	/// Currently, Lucie uses the logical size of the app to handle mouse interactions (such as
	/// whether the mouse collides with any elements).
			...
			.spawn(async move {
				let StyleAndBounds { style, x, y, cx, cy } = match this.state.fullscreen.take() {
					...
					None => {
						...
						let style = style & !(WS_THICKFRAME | WS_SYSMENU | WS_MAXIMIZEBOX | WS_MINIMIZEBOX | WS_CAPTION);
				...
				unsafe { SetWindowPos(this.hwnd, None, x, y, cx, cy, SWP_FRAMECHANGED | SWP_NOACTIVATE | SWP_NOZORDER) }.log_err();
			})
			.detach();
	}
	...
	fn set_window_placement(self: &Rc<Self>) -> Result<()> {
		...
		match open_status.state {
			...
			WindowOpenState::Fullscreen => {
				unsafe { SetWindowPlacement(self.hwnd, &open_status.placement).context("failed to set window placement")? };
...
impl WindowsWindow {
	pub(crate) fn new(handle: AnyWindowHandle, params: WindowParams, creation_info: WindowCreationInfo) -> Result<Self> {
		...
		let hide_title_bar = params.titlebar.as_ref().map(|titlebar| titlebar.appears_transparent).unwrap_or(true);
		...
		let placement = retrieve_window_placement(hwnd, display, params.bounds, this.state.scale_factor.get(), &this.state.border_offset)?;
		if params.show {
...
impl ClickState {
	...
	pub fn system_update(&self, wparam: usize) {
		match wparam {
			...
			29 => self.double_click_spatial_tolerance_width.set(unsafe { GetSystemMetrics(SM_CXDOUBLECLK) }),
...
fn dwm_set_window_composition_attribute(hwnd: HWND, backdrop_type: u32) {
	...
	unsafe {
		let result =
			DwmSetWindowAttribute(hwnd, DWMWA_SYSTEMBACKDROP_TYPE, &backdrop_type as *const _ as *const _, std::mem::size_of_val(&backdrop_type) as u32);
...
fn set_window_composition_attribute(hwnd: HWND, color: Option<Color>, state: u32) {
	...
	unsafe {
		type SetWindowCompositionAttributeType = unsafe extern "system" fn(HWND, *mut WINDOWCOMPOSITIONATTRIBDATA) -> BOOL;
		...
		if let Some(user32) = GetModuleHandleA(module_name).context("Unable to get user32.dll handle").log_err() {
			...
			let set_window_composition_attribute: SetWindowCompositionAttributeType = std::mem::transmute(GetProcAddress(user32, func_name));
