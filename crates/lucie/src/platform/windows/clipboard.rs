...
use windows::Win32::{
	...
	System::{
		DataExchange::{
			CloseClipboard, CountClipboardFormats, EmptyClipboard, EnumClipboardFormats, GetClipboardData, GetClipboardFormatNameW, IsClipboardFormatAvailable,
...
// Clipboard formats
static CLIPBOARD_HASH_FORMAT: LazyLock<u32> = LazyLock::new(|| register_clipboard_format(windows::core::w!("Lucie internal text hash")));
static CLIPBOARD_METADATA_FORMAT: LazyLock<u32> = LazyLock::new(|| register_clipboard_format(windows::core::w!("Lucie internal metadata")));
static CLIPBOARD_SVG_FORMAT: LazyLock<u32> = LazyLock::new(|| register_clipboard_format(windows::core::w!("image/svg+xml")));
...
static FORMATS_MAP: LazyLock<RapidHashMap<u32, ClipboardFormatType>> = LazyLock::new(|| {
	let mut formats_map = RapidHashMap::default();
...
static IMAGE_FORMATS_MAP: LazyLock<RapidHashMap<u32, ImageFormat>> = LazyLock::new(|| {
	let mut formats_map = RapidHashMap::default();
...
fn read_string_from_clipboard() -> Option<ClipboardEntry> {
	...
	if hash == ClipboardString::text_hash(&text) {
		Some(ClipboardEntry::String(ClipboardString { text, metadata: Some(metadata) }))
	} else {
