...
pub(crate) const WM_LUCIE_CURSOR_STYLE_CHANGED: u32 = WM_USER + 1;
pub(crate) const WM_LUCIE_CLOSE_ONE_WINDOW: u32 = WM_USER + 2;
pub(crate) const WM_LUCIE_TASK_DISPATCHED_ON_MAIN_THREAD: u32 = WM_USER + 3;
pub(crate) const WM_LUCIE_DOCK_MENU_ACTION: u32 = WM_USER + 4;
pub(crate) const WM_LUCIE_FORCE_UPDATE_WINDOW: u32 = WM_USER + 5;
pub(crate) const WM_LUCIE_KEYBOARD_LAYOUT_CHANGED: u32 = WM_USER + 6;
pub(crate) const WM_LUCIE_GPU_DEVICE_LOST: u32 = WM_USER + 7;
pub(crate) const WM_LUCIE_KEYDOWN: u32 = WM_USER + 8;
...
impl WindowsWindowInner {
	pub(crate) fn handle_msg(self: &Rc<Self>, handle: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
		let handled = match msg {
			...
			WM_NCLBUTTONDBLCLK | WM_NCLBUTTONDOWN => self.handle_nc_mouse_down_msg(handle, MouseButton::Left, wparam, lparam),
			...
			WM_LUCIE_KEYDOWN => self.handle_keydown_msg(wparam, lparam),
			...
			WM_LUCIE_CURSOR_STYLE_CHANGED => self.handle_cursor_changed(lparam),
			WM_LUCIE_FORCE_UPDATE_WINDOW => self.draw_window(handle, true),
	...
	fn handle_move_msg(&self, handle: HWND, lparam: LPARAM) -> Option<isize> {
		let origin = logical_point(lparam.signed_loword() as f32, lparam.signed_hiword() as f32, self.state.scale_factor.get());
			...
			|| center_y > monitor_bounds.bottom().0
		{
			...
			if !monitor.is_invalid() && self.state.display.get().handle != monitor {
				...
				self.state.display.set(WindowsDisplay::new_with_handle(monitor).log_err()?);
			}
		}
	...
	fn handle_get_min_max_info_msg(&self, lparam: LPARAM) -> Option<isize> {
		...
		unsafe {
			...
			minmax_info.ptMinTrackSize.x = min_size.width.scale(scale_factor).0 as i32 + boarder_offset.width_offset.get();
			minmax_info.ptMinTrackSize.y = min_size.height.scale(scale_factor).0 as i32 + boarder_offset.height_offset.get();
		}
		Some(0)
	}
	...
	fn handle_size_msg(&self, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
		...
		if wparam.0 == SIZE_MINIMIZED as usize {
			self.state.restore_from_minimized.set(self.state.callbacks.request_frame.take());
			return Some(0);
		}
	...
	fn handle_destroy_msg(&self, handle: HWND) -> Option<isize> {
		...
		unsafe {
			PostMessageW(Some(self.platform_window_handle), WM_LUCIE_CLOSE_ONE_WINDOW, WPARAM(self.validation_number), LPARAM(handle.0 as isize)).log_err();
		}
		Some(0)
	}
	...
	fn handle_syskeyup_msg(&self, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
		let input = handle_key_event(wparam, lparam, &self.state, |keystroke, _| PlatformInput::KeyUp(KeyUpEvent { keystroke }))?;
	...
	fn handle_keyup_msg(&self, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
		let Some(input) = handle_key_event(wparam, lparam, &self.state, |keystroke, _| PlatformInput::KeyUp(KeyUpEvent { keystroke })) else {
			return Some(1);
		};
	...
	fn handle_mouse_horizontal_wheel_msg(&self, handle: HWND, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
		...
		let event = PlatformInput::ScrollWheel(ScrollWheelEvent {
			position: logical_point(cursor_point.x as f32, cursor_point.y as f32, scale_factor),
			delta: ScrollDelta::Lines(Point { x: wheel_distance, y: 0.0 }),
			modifiers: current_modifiers(),
			touch_phase: TouchPhase::Moved
		});
		...
		if lparam == 0 {
			...
			if lparam & GCS_COMPSTR.0 > 0 {
				...
				let caret_pos = (!comp_string.is_empty() && lparam & GCS_CURSORPOS.0 > 0).then(|| {
					...
					let pos = if should_use_ime_cursor_position(ctx, cursor_pos) { cursor_pos } else { comp_string.len() };
					pos..pos
				});
				self.with_input_handler(|input_handler| {
					input_handler.replace_and_mark_text_in_range(None, &String::from_utf16_lossy(&comp_string), caret_pos);
				})?;
			}
	...
	fn handle_dpi_changed_msg(&self, handle: HWND, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
		...
		if is_maximized {
			...
			if unsafe { GetMonitorInfoW(monitor, &mut monitor_info) }.as_bool() {
				...
				unsafe {
					SetWindowPos(handle, None, work_area.left, work_area.top, width, height, SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED)
			...
			unsafe {
				SetWindowPos(handle, None, rect.left, rect.top, width, height, SWP_NOZORDER | SWP_NOACTIVATE)
	...
	fn handle_hit_test_msg(&self, handle: HWND, lparam: LPARAM) -> Option<isize> {
		...
		if !self.state.is_maximized() && 0 <= cursor_point.y && cursor_point.y <= frame_y {
			...
			return Some(if cursor_point.x <= 0 {
				...
				if right - 2 * frame_x <= cursor_point.x { HTTOPRIGHT } else { HTTOP }
			} as _);
		}
	...
	fn handle_nc_mouse_down_msg(&self, handle: HWND, button: MouseButton, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
	...
	fn handle_nc_mouse_up_msg(&self, handle: HWND, button: MouseButton, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
		if let Some(mut func) = self.state.callbacks.input.take() {
			let scale_factor = self.state.scale_factor.get();
	...
	fn handle_cursor_changed(&self, lparam: LPARAM) -> Option<isize> {
		...
		self.state
			.current_cursor
			.set(if lparam.0 == 0 { None } else { Some(HCURSOR(lparam.0 as _)) });
	...
	fn handle_set_cursor(&self, handle: HWND, lparam: LPARAM) -> Option<isize> {
		if unsafe { !IsWindowEnabled(handle).as_bool() }
			|| matches!(lparam.loword() as u32, HTLEFT | HTRIGHT | HTTOP | HTTOPLEFT | HTTOPRIGHT | HTBOTTOM | HTBOTTOMLEFT | HTBOTTOMRIGHT)
		{
			return None;
		}
		unsafe {
	...
	fn handle_input_language_changed(&self) -> Option<isize> {
		unsafe {
			PostMessageW(Some(self.platform_window_handle), WM_LUCIE_KEYBOARD_LAYOUT_CHANGED, WPARAM(self.validation_number), LPARAM(0)).log_err();
		}
		Some(0)
	}
	...
	fn draw_window(&self, handle: HWND, force_render: bool) -> Option<isize> {
		...
		// we are instructing lucie to force render a frame, this will
	...
	fn parse_char_message(&self, wparam: WPARAM) -> Option<String> {
		...
		match code_point {
			...
			_ => {
				...
				char::from_u32(code_point as u32).filter(|c| !c.is_control()).map(|c| c.to_string())
			}
		}
	}
	...
	F: FnOnce(Keystroke, bool) -> PlatformInput
{
	...
	match virtual_key {
		...
		VK_CAPITAL => {
			...
			if state.last_reported_capslock.get().is_some_and(|prev_capslock| prev_capslock == capslock) {
				return None;
			}
			state.last_reported_capslock.set(Some(capslock));
			Some(PlatformInput::ModifiersChanged(ModifiersChangedEvent { modifiers, capslock }))
		}
		vkey => {
...
fn parse_normal_key(vkey: VIRTUAL_KEY, lparam: LPARAM, mut modifiers: Modifiers) -> Option<(Keystroke, bool)> {
	...
	Some((Keystroke { modifiers, key, key_char }, prefer_character_input))
}
...
fn process_key(vkey: VIRTUAL_KEY, scan_code: u16) -> (Option<String>, bool) {
	...
	let result_c = unsafe { ToUnicode(vkey.0 as u32, scan_code as u32, Some(&keyboard_state), &mut buffer_c, 0x4) };
	...
	let result_c_no_modifiers = unsafe { ToUnicode(vkey.0 as u32, scan_code as u32, Some(&state_no_modifiers), &mut buffer_c_no_modifiers, 0x4) };
...
fn should_use_ime_cursor_position(ctx: HIMC, cursor_pos: usize) -> bool {
	...
	let result = unsafe { ImmGetCompositionStringW(ctx, GCS_COMPATTR, Some(attrs.as_mut_ptr() as *mut _), attrs_size as u32) };
	if result <= 0 {
		return false;
	}
