...
use wayland_client::{Proxy, WEnum, protocol::wl_surface};
...
use crate::{
	AnyWindowHandle, Bounds, Capslock, Decorations, Globals, GpuSpecs, Modifiers, Output, Pixels, PlatformDisplay, PlatformInput, Point, PromptButton,
	PromptLevel, RequestFrameOptions, ResizeEdge, Size, Tiling, WaylandClientStatePtr, WindowAppearance, WindowBackgroundAppearance, WindowBounds,
...
impl WaylandSurfaceState {
	fn new(surface: &wl_surface::WlSurface, globals: &Globals, params: &WindowParams, parent: Option<XdgToplevel>) -> anyhow::Result<Self> {
		...
		if let WindowKind::LayerShell(options) = &params.kind {
			...
			let layer_surface = layer_shell.get_layer_surface(&surface, None, options.layer.into(), options.namespace.clone(), &globals.qh, surface.id());
			...
			if let Some(margin) = options.margin {
				layer_surface.set_margin(margin.0.0 as i32, margin.1.0 as i32, margin.2.0 as i32, margin.3.0 as i32)
			}
			...
			return Ok(WaylandSurfaceState::LayerShell(WaylandLayerSurfaceState { layer_surface }));
		}
		...
		Ok(WaylandSurfaceState::Xdg(WaylandXdgSurfaceState { xdg_surface, toplevel, decoration }))
	}
}
...
impl WaylandWindowState {
	...
	pub fn is_transparent(&self) -> bool {
		self.decorations == WindowDecorations::Client || self.background_appearance != WindowBackgroundAppearance::Opaque
	}
...
impl WaylandWindowStatePtr {
	...
	pub fn handle_xdg_surface_event(&self, event: xdg_surface::Event) {
		if let xdg_surface::Event::Configure { serial } = event {
				...
				if let Some(mut configure) = state.in_progress_configure.take() {
					...
					if !configure.fullscreen && !configure.maximized {
						...
						if let Some(size) = configure.size {
							state.window_bounds = Bounds { origin: Point::default(), size };
						}
					}
					drop(state);
					if let Some(size) = configure.size {
			...
			state
				.surface_state
				.set_geometry(window_geometry.origin.x, window_geometry.origin.y, window_geometry.size.width, window_geometry.size.height);
	...
	pub fn handle_toplevel_event(&self, event: xdg_toplevel::Event) -> bool {
		match event {
			xdg_toplevel::Event::Configure { width, height, states } => {
				let mut size = if width == 0 || height == 0 { None } else { Some(size(px(width as f32), px(height as f32))) };
	...
	pub fn handle_surface_event(&self, event: wl_surface::Event, outputs: RapidHashMap<ObjectId, Output>) {
		let mut state = self.state.borrow_mut();
...
impl rwh::HasDisplayHandle for WaylandWindow {
	fn display_handle(&self) -> Result<rwh::DisplayHandle<'_>, rwh::HandleError> {
		let display = self.0.surface().backend().upgrade().ok_or(rwh::HandleError::Unavailable)?.display_ptr() as *mut libc::c_void;
...
impl PlatformWindow for WaylandWindow {
	...
	fn resize(&mut self, size: Size<Pixels>) {
		...
		state
			.surface_state
			.set_geometry(state.bounds.origin.x.0 as i32, state.bounds.origin.y.0 as i32, dp_size.width.0, dp_size.height.0);

		state.globals.executor.spawn(async move { state_ptr.resize(size) }).detach();
	}
	...
	fn mouse_position(&self) -> Point<Pixels> {
		self.borrow().client.get_client().borrow().mouse_location.unwrap_or_default()
	}
	...
	fn prompt(&self, _level: PromptLevel, _msg: &str, _detail: Option<&str>, _answers: &[PromptButton]) -> Option<Receiver<usize>> {
		None
	}
	...
	fn start_window_resize(&self, edge: crate::ResizeEdge) {
		let state = self.borrow();
		if let Some(toplevel) = state.surface_state.toplevel() {
			toplevel.resize(&state.globals.seat, state.client.get_serial(SerialKind::MousePress), edge.to_xdg())
		}
	}
...
fn update_window(mut state: RefMut<WaylandWindowState>) {
	...
	region.add(opaque_area.origin.x, opaque_area.origin.y, opaque_area.size.width, opaque_area.size.height);
	...
	if state.background_appearance == WindowBackgroundAppearance::Opaque && state.decorations == WindowDecorations::Server {
