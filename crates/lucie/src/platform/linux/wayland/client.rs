...
impl Globals {
	fn new(globals: GlobalList, executor: ForegroundExecutor, qh: QueueHandle<WaylandClientStatePtr>, seat: wl_seat::WlSeat) -> Self {
		Globals {
			activation: globals.bind(&qh, 1..=1, ()).ok(),
			compositor: globals
				.bind(&qh, wl_surface::REQ_SET_BUFFER_SCALE_SINCE..=wl_surface::EVT_PREFERRED_BUFFER_SCALE_SINCE, ())
				.unwrap(),
...
pub(crate) struct WaylandClientState {
	...
	windows: RapidHashMap<ObjectId, WaylandWindowStatePtr>,
	// Output to scale mapping
	outputs: RapidHashMap<ObjectId, Output>,
	in_progress_outputs: RapidHashMap<ObjectId, InProgressOutput>,
...
impl WaylandClientStatePtr {
	pub fn get_client(&self) -> Rc<RefCell<WaylandClientState>> {
		self.0.upgrade().expect("The pointer should always be valid when dispatching in wayland")
	}
	...
	pub fn enable_ime(&self) {
		...
		if let Some(window) = state.keyboard_focused_window.clone() {
			...
			if let Some(area) = window.get_ime_area() {
				text_input.set_cursor_rectangle(area.origin.x.0 as i32, area.origin.y.0 as i32, area.size.width.0 as i32, area.size.height.0 as i32);
			}
	...
	pub fn update_ime_position(&self, bounds: Bounds<Pixels>) {
		...
		text_input.set_cursor_rectangle(bounds.origin.x.0 as i32, bounds.origin.y.0 as i32, bounds.size.width.0 as i32, bounds.size.height.0 as i32);
		text_input.commit();
	}
...
fn wl_output_version(version: u32) -> u32 {
	...
	if version < WL_OUTPUT_MIN_VERSION {
		panic!("wl_output below required version: {} < {}", version, WL_OUTPUT_MIN_VERSION);
	}
...
impl WaylandClient {
	pub(crate) fn new() -> Self {
		...
		let mut in_progress_outputs = RapidHashMap::default();
				...
				match &global.interface[..] {
					"wl_seat" => {
						seat = Some(
							globals
								.registry()
								.bind::<wl_seat::WlSeat, _, _>(global.name, wl_seat_version(global.version), &qh, ())
						);
					}
					"wl_output" => {
						let output = globals
							.registry()
							.bind::<wl_output::WlOutput, _, _>(global.name, wl_output_version(global.version), &qh, ());
...
impl LinuxClient for WaylandClient {
	...
	fn run(&self) {
		...
		event_loop.run(None, &mut WaylandClientStatePtr(Rc::downgrade(&self.0)), |_| {}).log_err();
	}

	fn write_to_primary(&self, item: crate::ClipboardItem) {
		let mut state = self.0.borrow_mut();
		let (Some(primary_selection_manager), Some(primary_selection)) = (state.globals.primary_selection_manager.clone(), state.primary_selection.clone())
		else {
			return;
		};
	...
	fn write_to_clipboard(&self, item: crate::ClipboardItem) {
		let mut state = self.0.borrow_mut();
		let (Some(data_device_manager), Some(data_device)) = (state.globals.data_device_manager.clone(), state.data_device.clone()) else {
			return;
		};
		if state.mouse_focused_window.is_some() || state.keyboard_focused_window.is_some() {
	...
	fn active_window(&self) -> Option<AnyWindowHandle> {
		self.0.borrow_mut().keyboard_focused_window.as_ref().map(|window| window.handle())
	}
...
impl Dispatch<wl_registry::WlRegistry, GlobalListContents> for WaylandClientStatePtr {
	fn event(this: &mut Self, registry: &wl_registry::WlRegistry, event: wl_registry::Event, _: &GlobalListContents, _: &Connection, qh: &QueueHandle<Self>) {
		...
		match event {
			wl_registry::Event::Global { name, interface, version } => match &interface[..] {
				...
				"wl_output" => {
					...
					state.in_progress_outputs.insert(output.id(), InProgressOutput::default());
				}
				_ => {}
			},
...
impl Dispatch<wl_surface::WlSurface, ()> for WaylandClientStatePtr {
	fn event(this: &mut Self, surface: &wl_surface::WlSurface, event: <wl_surface::WlSurface as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		let mut client = this.get_client();
		let mut state = client.borrow_mut();
...
impl Dispatch<wl_output::WlOutput, ()> for WaylandClientStatePtr {
	fn event(this: &mut Self, output: &wl_output::WlOutput, event: <wl_output::WlOutput as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		let mut client = this.get_client();
		let mut state = client.borrow_mut();
		...
		match event {
			...
			wl_output::Event::Geometry { x, y, .. } => in_progress_output.position = Some(point(DevicePixels(x), DevicePixels(y))),
			wl_output::Event::Mode { width, height, .. } => in_progress_output.size = Some(size(DevicePixels(width), DevicePixels(height))),
...
impl Dispatch<xdg_surface::XdgSurface, ObjectId> for WaylandClientStatePtr {
	fn event(state: &mut Self, _: &xdg_surface::XdgSurface, event: xdg_surface::Event, surface_id: &ObjectId, _: &Connection, _: &QueueHandle<Self>) {
		let client = state.get_client();
		let mut state = client.borrow_mut();
		let Some(window) = get_window(&mut state, surface_id) else {
			return;
...
impl Dispatch<xdg_wm_base::XdgWmBase, ()> for WaylandClientStatePtr {
	fn event(_: &mut Self, wm_base: &xdg_wm_base::XdgWmBase, event: <xdg_wm_base::XdgWmBase as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
...
impl Dispatch<wl_keyboard::WlKeyboard, ()> for WaylandClientStatePtr {
	fn event(this: &mut Self, _: &wl_keyboard::WlKeyboard, event: wl_keyboard::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		let mut client = this.get_client();
		let mut state = client.borrow_mut();
		match event {
			...
			wl_keyboard::Event::Keymap {
				...
				let keymap = unsafe {
					xkb::Keymap::new_from_fd(&xkb_context, fd, size as usize, XKB_KEYMAP_FORMAT_TEXT_V1, KEYMAP_COMPILE_NO_FLAGS)
						.log_err()
						.flatten()
						...
						state
							...
							.insert_source(Timer::from_duration(state.repeat.delay), {
								...
								move |event_timestamp, _metadata, this| {
									...
									let mut state = client.borrow_mut();
									let is_repeating =
										id == state.repeat.current_id && state.repeat.current_keycode.is_some() && state.keyboard_focused_window.is_some();
...
fn linux_button_to_lucie(button: u32) -> Option<MouseButton> {
...
impl Dispatch<wl_pointer::WlPointer, ()> for WaylandClientStatePtr {
	fn event(this: &mut Self, wl_pointer: &wl_pointer::WlPointer, event: wl_pointer::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		let mut client = this.get_client();
		let mut state = client.borrow_mut();
		...
		match event {
			...
			wl_pointer::Event::Button {
				...
				let button = linux_button_to_lucie(button);
					...
					wl_pointer::ButtonState::Pressed => {
						if let Some(window) = state.keyboard_focused_window.clone() {
							if state.composing && state.text_input.is_some() {
								...
								state = client.borrow_mut();
							} else if let (Some(text), Some(compose)) = (state.pre_edit_text.take(), state.compose_state.as_mut()) {
								compose.reset();
								drop(state);
						...
						if click_elapsed < DOUBLE_CLICK_INTERVAL
							&& state.click.last_mouse_button.is_some_and(|prev_button| prev_button == button)
			...
			wl_pointer::Event::Axis { axis: WEnum::Value(axis), value, .. } => {
				...
				let axis = if state.modifiers.shift { wl_pointer::Axis::HorizontalScroll } else { axis };
...
impl Dispatch<wl_data_device::WlDataDevice, ()> for WaylandClientStatePtr {
	fn event(this: &mut Self, _: &wl_data_device::WlDataDevice, event: wl_data_device::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		let client = this.get_client();
		let mut state = client.borrow_mut();
		...
		match event {
			...
			wl_data_device::Event::Selection { id: data_offer } => {
				if let Some(offer) = data_offer {
					let offer = state.data_offers.iter().find(|wrapper| wrapper.inner.id() == offer.id());
			...
			wl_data_device::Event::Enter {
				...
				if let Some(data_offer) = data_offer {
					...
					data_offer.receive(FILE_LIST_MIME_TYPE.to_string(), unsafe { BorrowedFd::borrow_raw(pipe.write.as_raw_fd()) });
					let fd = pipe.read;
					drop(pipe.write);
			...
			wl_data_device::Event::Drop => {
				...
				let input = PlatformInput::FileDrop(FileDropEvent::Submit { position: state.drag.position });
				drop(state);
				drag_window.handle_input(input);
			}
			_ => {}
...
impl Dispatch<wl_data_offer::WlDataOffer, ()> for WaylandClientStatePtr {
	fn event(this: &mut Self, data_offer: &wl_data_offer::WlDataOffer, event: wl_data_offer::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		let client = this.get_client();
		let mut state = client.borrow_mut();
		...
		if let wl_data_offer::Event::Offer { mime_type } = event {
			...
			// Clipboard
			if let Some(offer) = state.data_offers.iter_mut().find(|wrapper| wrapper.inner.id() == data_offer.id()) {
...
impl Dispatch<wl_data_source::WlDataSource, ()> for WaylandClientStatePtr {
	fn event(this: &mut Self, data_source: &wl_data_source::WlDataSource, event: wl_data_source::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
		let client = this.get_client();
		let mut state = client.borrow_mut();
