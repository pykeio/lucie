...
use crate::{ClipboardEntry, ClipboardItem, Image, ImageFormat, WaylandClientStatePtr, hash, platform::linux::platform::read_fd};
...
impl<T: ReceiveData> DataOffer<T> {
	pub fn new(offer: T) -> Self {
		Self { inner: offer, mime_types: Vec::new() }
	}
	...
	fn read_bytes(&self, connection: &Connection, mime_type: &str) -> Option<Vec<u8>> {
		...
		self.inner
			.receive_data(mime_type.to_string(), unsafe { BorrowedFd::borrow_raw(pipe.write.as_raw_fd()) });
		let fd = pipe.read;
		drop(pipe.write);
		...
		match unsafe { read_fd(fd) } {
			Ok(bytes) => Some(bytes),
			Err(err) => {
				tracing::error!("error reading clipboard pipe: {err:?}");
				None
			}
		}
	}
	...
	fn read_text(&self, connection: &Connection) -> Option<ClipboardItem> {
		...
		let text_content = match String::from_utf8(bytes) {
			Ok(content) => content,
			Err(e) => {
				tracing::error!("Failed to convert clipboard content to UTF-8: {}", e);
				return None;
			}
		};
...
impl Clipboard {
	...
	pub fn read(&mut self) -> Option<ClipboardItem> {
		...
		let item = offer.read_text(&self.connection).or_else(|| offer.read_image(&self.connection))?;
	...
	fn send_internal(&self, fd: OwnedFd, bytes: Vec<u8>) {
		let mut written = 0;
		self.loop_handle
			.insert_source(calloop::generic::Generic::new(File::from(fd), calloop::Interest::WRITE, calloop::Mode::Level), move |_, file, _| {
