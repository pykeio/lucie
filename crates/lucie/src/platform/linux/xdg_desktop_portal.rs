		...
		executor
			.spawn(async move {
				...
				if let Ok(initial_appearance) = settings.color_scheme().await {
					sender.send(Event::WindowAppearance(WindowAppearance::from_native(initial_appearance)))?;
				}
				if let Ok(initial_theme) = settings.read::<String>("org.gnome.desktop.interface", "cursor-theme").await {
...
impl EventSource for XDPEventSource {
	...
	fn process_events<F>(&mut self, readiness: Readiness, token: Token, mut callback: F) -> Result<PostAction, Self::Error>
	where
