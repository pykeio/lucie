...
use x11rb::{
	...
	protocol::{
		...
		xproto,
		xproto::{AtomEnum, ChangeWindowAttributesAux, ClientMessageData, ClientMessageEvent, ConnectionExt as _, EventMask, Visibility}
	},
...
use super::{
	ButtonOrScroll, ScrollDirection, X11Display, X11WindowStatePtr, XcbAtoms, XimCallbackEvent, XimHandler, button_or_scroll_from_event_detail, check_reply,
	clipboard::{self, Clipboard},
	get_reply, get_valuator_axis_index, handle_connection_error, modifiers_from_state, pressed_button_from_mask
};
use crate::{
	AnyWindowHandle, Bounds, Capslock, ClipboardItem, CursorStyle, DisplayId, FileDropEvent, Keystroke, LinuxKeyboardLayout, Modifiers, ModifiersChangedEvent,
	MouseButton, Pixels, Platform, PlatformDisplay, PlatformInput, PlatformKeyboardLayout, Point, RequestFrameOptions, RunnableVariant, ScrollDelta, Size,
	TaskTiming, TouchPhase, WindowParams, X11Window, modifiers_from_xinput_info,
	platform::{
		LinuxCommon, PlatformWindow,
		blade::BladeContext,
		linux::{
			DEFAULT_CURSOR_ICON_NAME, LinuxClient, get_xkb_compose_state, is_within_click_distance, log_cursor_icon_warning,
...
const LUCIE_X11_SCALE_FACTOR_ENV: &str = "LUCIE_X11_SCALE_FACTOR";
...
enum RefreshState {
	Hidden { refresh_rate: Duration },
	PeriodicRefresh { refresh_rate: Duration, event_loop_token: RegistrationToken }
}
...
struct ScrollAxisState {
	...
	/// The logic errs on the side of invalidating this, since the consequence is just skipping the delta of one scroll
	/// event. The consequence of not invalidating it can be large invalid deltas, which are much more user visible.
...
pub struct X11ClientState {
	...
	pub(crate) windows: RapidHashMap<xproto::Window, WindowRef>,
	...
	pub(crate) cursor_styles: RapidHashMap<xproto::Window, CursorStyle>,
	pub(crate) cursor_cache: RapidHashMap<CursorStyle, Option<xproto::Cursor>>,
...
impl X11ClientStatePtr {
	...
	pub fn update_ime_position(&self, bounds: Bounds<Pixels>) {
		...
		let ic_attributes = ximc
			...
			.build();
		let _ = ximc.set_ic_values(xim_handler.im_id, xim_handler.ic_id, ic_attributes).log_err();
		state.ximc = Some(ximc);
...
impl X11Client {
	pub(crate) fn new() -> anyhow::Result<Self> {
						...
						handle.insert_idle(|_| {
							...
							let mut timing = match runnable {
								RunnableVariant::Meta(runnable) => {
									...
									let timing = TaskTiming { location, start, end: None };
		...
		let xinput_version = get_reply(|| "XInput XiQueryVersion failed", xcb_connection.xinput_xi_query_version(2, 1))?;
		assert!(xinput_version.major_version >= 2, "XInput version >= 2 required.");

		let pointer_device_states = current_pointer_device_states(&xcb_connection, &BTreeMap::new()).unwrap_or_default();
		...
		let xkb_state = {
			let xkb_keymap = xkbc::x11::keymap_new_from_device(&xkb_context, &xcb_connection, xkb_device_id, xkbc::KEYMAP_COMPILE_NO_FLAGS);
		...
		let resource_database = x11rb::resource_manager::new_from_default(&xcb_connection).context("Failed to create resource database")?;
		...
		handle
			.insert_source(Generic::new_with_error::<EventHandlerError>(fd, calloop::Interest::READ, calloop::Mode::Level), {
		...
		Ok(X11Client(Rc::new(RefCell::new(X11ClientState {
			...
			cursor_handle,
			cursor_styles: RapidHashMap::default(),
			cursor_cache: RapidHashMap::default(),
						...
						match event {
							...
							Event::KeyPress(key_press) => {
								...
								if let Some(Event::KeyRelease(key_release)) = last_key_release.take() {
									...
									if key_release.detail != key_press.detail || key_press.time.saturating_sub(key_release.time) > 20 {
		...
		match event {
			...
			Event::ClientMessage(event) => {
				...
				if atom == state.atoms.WM_DELETE_WINDOW {
					...
					window.state.borrow_mut().last_sync_counter = Some(x11rb::protocol::sync::Int64 { lo: arg2, hi: arg3 as i32 })
				}

				if event.type_ == state.atoms.XdndEnter {
					state.xdnd_state.other_window = atom;
					if (arg1 & 0x1) == 0x1 {
						state.xdnd_state.drag_type = xdnd_get_supported_atom(&state.xcb_connection, &state.atoms, state.xdnd_state.other_window);
					} else {
						if let Some(atom) = [arg2, arg3, arg4].into_iter().find(|atom| xdnd_is_atom_supported(*atom, &state.atoms)) {
					...
					if let Ok(pos) = get_reply(|| "Failed to query pointer position", state.xcb_connection.query_pointer(event.window)) {
					...
					xdnd_send_status(&state.xcb_connection, &state.atoms, event.window, state.xdnd_state.other_window, arg4);
					let position = state.xdnd_state.position;
					drop(state);
					window.handle_input(PlatformInput::FileDrop(FileDropEvent::Pending { position }));
				} else if event.type_ == state.atoms.XdndDrop {
					xdnd_send_finished(&state.xcb_connection, &state.atoms, event.window, state.xdnd_state.other_window);
					let position = state.xdnd_state.position;
					drop(state);
			...
			Event::SelectionNotify(event) => {
				...
				let reply = get_reply(
					...
					state
						.xcb_connection
						.get_property(false, event.requestor, state.atoms.XDND_DATA, AtomEnum::ANY, 0, 1024)
				)
				.log_err();
				...
				if let Ok(file_list) = str::from_utf8(&reply.value) {
					let paths: SmallVec<[_; 2]> = file_list.lines().filter_map(file_url_to_path).collect();
			...
			Event::ConfigureNotify(event) => {
				let bounds = Bounds {
					origin: Point { x: event.x.into(), y: event.y.into() },
					size: Size {
				...
				let window = self.get_window(event.window)?;
				window.set_bounds(bounds).context("X11: Failed to set window bounds").log_err();
			}
			Event::PropertyNotify(event) => {
				let window = self.get_window(event.window)?;
				window.property_notify(event).context("X11: Failed to handle property notify").log_err();
			}
			Event::FocusIn(event) => {
			...
			Event::XkbNewKeyboardNotify(_) | Event::XkbMapNotify(_) => {
				...
				let xkb_state = {
					let xkb_keymap =
						xkbc::x11::keymap_new_from_device(&state.xkb_context, &state.xcb_connection, state.xkb_device_id, xkbc::KEYMAP_COMPILE_NO_FLAGS);
			...
			Event::XkbStateNotify(event) => {
				...
				if state.last_modifiers_changed_event == modifiers && state.last_capslock_changed_event == capslock {
					...
					focused_window.handle_input(PlatformInput::ModifiersChanged(ModifiersChangedEvent { modifiers, capslock }));
				}
					...
					if let Some(mut compose_state) = state.compose_state.take() {
						...
						match compose_state.status() {
							...
							xkbc::Status::Composing => {
								...
								state.pre_edit_text = compose_state.utf8().or(crate::Keystroke::underlying_dead_key(keysym));
			...
			Event::XinputButtonPress(event) => {
				...
				let position =
					point(px(event.event_x as f32 / u16::MAX as f32 / state.scale_factor), px(event.event_y as f32 / u16::MAX as f32 / state.scale_factor));
				...
				match button_or_scroll_from_event_detail(event.detail) {
					...
					Some(ButtonOrScroll::Scroll(direction)) => {
						...
						if !event.flags.contains(xinput::PointerEventFlags::POINTER_EMULATED) {
							...
							window.handle_input(PlatformInput::ScrollWheel(make_scroll_wheel_event(position, scroll_delta, modifiers)));
						}
					}
					None => {
			...
			Event::XinputMotion(event) => {
				...
				if event.valuator_mask[0] & 3 != 0 {
					window.handle_input(PlatformInput::MouseMove(crate::MouseMoveEvent { position, pressed_button, modifiers }));
				}
			...
			Event::XinputLeave(event) if event.mode == xinput::NotifyMode::NORMAL => {
				...
				// Set last scroll values to `None` so that a large delta isn't created if scrolling is done outside the window (the
				// valuator is global)
				...
				let window = self.get_window(event.event)?;
				window.handle_input(PlatformInput::MouseExited(crate::MouseExitEvent { pressed_button, position, modifiers }));
			...
			Event::XinputHierarchy(event) => {
				...
				if let Some(pointer_device_states) = current_pointer_device_states(&state.xcb_connection, &state.pointer_device_states) {
	...
	fn xim_handle_event(&self, event: Event) -> Option<()> {
		match event {
			Event::KeyPress(event) | Event::KeyRelease(event) => {
				...
				ximc.forward_event(xim_handler.im_id, xim_handler.ic_id, xim::ForwardEventFlag::empty(), &event)
	...
	fn xim_handle_preedit(&self, window: xproto::Window, text: String) -> Option<()> {
		...
		if let Some(scaled_area) = window.get_ime_area() {
			let ic_attributes = ximc
				...
				.build();
			ximc.set_ic_values(xim_handler.im_id, xim_handler.ic_id, ic_attributes).ok();
		}
		let mut state = self.0.borrow_mut();
		state.restore_xim(ximc, xim_handler);
		drop(state);
...
impl LinuxClient for X11Client {
	...
	fn displays(&self) -> Vec<Rc<dyn PlatformDisplay>> {
		...
		setup
			...
			.iter()
			.enumerate()
			.filter_map(|(root_id, _)| Some(Rc::new(X11Display::new(&state.xcb_connection, state.scale_factor, root_id).ok()?) as Rc<dyn PlatformDisplay>))
			.collect()
	}
	...
	fn display(&self, id: DisplayId) -> Option<Rc<dyn PlatformDisplay>> {
		...
		Some(Rc::new(X11Display::new(&state.xcb_connection, state.scale_factor, id.0 as usize).ok()?))
	}
	...
	fn open_window(&self, handle: AnyWindowHandle, params: WindowParams) -> anyhow::Result<Box<dyn PlatformWindow>> {
		...
		let x_window = state.xcb_connection.generate_id().context("X11: Failed to generate window ID")?;
		...
		check_reply(
			...
			state
				.xcb_connection
				.change_property32(xproto::PropMode::REPLACE, x_window, state.atoms.XdndAware, state.atoms.XA_ATOM, &[5])
		)
		.log_err();
	...
	fn window_stack(&self) -> Option<Vec<AnyWindowHandle>> {
		...
		let reply = state
			.xcb_connection
			.get_property(false, root, state.atoms._NET_CLIENT_LIST_STACKING, xproto::AtomEnum::WINDOW, 0, u32::MAX)
			.ok()?
			.reply()
			.ok()?;
...
impl X11ClientState {
	...
	fn take_xim(&mut self) -> Option<(X11rbClient<Rc<XCBConnection>>, XimHandler)> {
		let ximc = self.ximc.take().ok_or(anyhow!("bug: XIM connection not set")).log_err()?;
	...
	fn update_refresh_loop(&mut self, x_window: xproto::Window) {
		...
		match (is_visible, window_ref.refresh_state.take()) {
			...
			(false, Some(RefreshState::PeriodicRefresh { refresh_rate, event_loop_token })) => {
			...
			(true, Some(RefreshState::Hidden { refresh_rate })) => {
				...
				window_ref.refresh_state = Some(RefreshState::PeriodicRefresh { refresh_rate, event_loop_token });
			}
			(true, None) => {
				let Some(screen_resources) =
					get_reply(|| "Failed to get screen resources", self.xcb_connection.randr_get_screen_resources_current(x_window)).log_err()
				else {
					return;
				};
				...
				let mode_info = screen_resources.crtcs.iter().find_map(|crtc| {
					let crtc_info = self.xcb_connection.randr_get_crtc_info(*crtc, x11rb::CURRENT_TIME).ok()?.reply().ok()?;
	...
	fn get_cursor_icon(&mut self, style: CursorStyle) -> Option<xproto::Cursor> {
		...
		match style {
			...
			_ => 'outer: {
				...
				if errors.is_empty() {
					result = Err(anyhow!("errors while loading cursor icons {:?}:\n{}", cursor_icon_names, errors));
				} else {
		...
		let cursor = match result {
			...
			Err(err) => match self.cursor_handle.load_cursor(&self.xcb_connection, DEFAULT_CURSOR_ICON_NAME) {
				Ok(default) => {
					log_cursor_icon_warning(
						err.context(format!("X11: error loading cursor icon, falling back on default icon '{}'", DEFAULT_CURSOR_ICON_NAME))
					);
					Some(default)
				}
				Err(default_err) => {
					log_cursor_icon_warning(
						err.context(default_err)
							.context(format!("X11: error loading default cursor fallback '{}'", DEFAULT_CURSOR_ICON_NAME))
					);
					None
				}
			}
...
fn xdnd_send_finished(xcb_connection: &XCBConnection, atoms: &XcbAtoms, source: xproto::Window, target: xproto::Window) {
	...
	check_reply(|| "Failed to send XDnD finished event", xcb_connection.send_event(false, target, EventMask::default(), message)).log_err();
	xcb_connection.flush().log_err();
}

fn xdnd_send_status(xcb_connection: &XCBConnection, atoms: &XcbAtoms, source: xproto::Window, target: xproto::Window, action: u32) {
	let message = ClientMessageEvent {
		format: 32,
		window: target,
	...
	check_reply(|| "Failed to send XDnD status event", xcb_connection.send_event(false, target, EventMask::default(), message)).log_err();
	xcb_connection.flush().log_err();
}
...
fn current_pointer_device_states(
	...
	let devices_query_result = get_reply(|| "Failed to query XInput devices", xcb_connection.xinput_xi_query_device(XINPUT_ALL_DEVICES)).log_err()?;
...
fn scroll_data_to_axis_state(data: &xinput::DeviceClassDataScroll, old_axis_state_with_valid_scroll_value: Option<&ScrollAxisState>) -> ScrollAxisState {
	ScrollAxisState {
...
fn reset_all_pointer_device_scroll_positions(pointer_device_states: &mut BTreeMap<xinput::DeviceId, PointerDeviceState>) {
	pointer_device_states
		.iter_mut()
...
fn get_scroll_delta_and_update_state(pointer: &mut PointerDeviceState, event: &xinput::MotionEvent) -> Option<Point<f32>> {
...
fn get_axis_scroll_delta_and_update_state(event: &xinput::MotionEvent, axis: &mut ScrollAxisState) -> Option<f32> {
	...
	if let Some(axis_value) = event.axisvalues.get(axis_index) {
		...
		let delta_scroll = axis.scroll_value.map(|old_scroll| (old_scroll - new_scroll) * axis.multiplier);
...
fn make_scroll_wheel_event(position: Point<Pixels>, scroll_delta: Point<f32>, modifiers: Modifiers) -> crate::ScrollWheelEvent {
	// When shift is held down, vertical scrolling turns into horizontal scrolling.
	let delta = if modifiers.shift { Point { x: scroll_delta.y, y: 0.0 } } else { scroll_delta };
...
fn create_invisible_cursor(connection: &XCBConnection) -> anyhow::Result<crate::platform::linux::x11::client::xproto::Cursor> {
...
fn get_scale_factor(connection: &XCBConnection, resource_database: &Database, screen_index: usize) -> f32 {
	let env_dpi = std::env::var(LUCIE_X11_SCALE_FACTOR_ENV)
		.ok()
		.map(|var| {
			if var.to_lowercase() == "randr" {
				...
				if valid_scale_factor(scale) {
					...
					panic!("`{}` must be a positive normal number or `randr`. Got `{}`", LUCIE_X11_SCALE_FACTOR_ENV, var);
				}
			} else if var.is_empty() {
				DpiMode::NotSet
			} else {
				panic!("`{}` must be a positive number or `randr`. Got `{}`", LUCIE_X11_SCALE_FACTOR_ENV, var);
			}
		})
...
fn get_randr_scale_factor(connection: &XCBConnection, screen_index: usize) -> Option<f32> {
	...
	for monitor in monitors_reply.monitors {
		...
		let scale_factor =
			get_dpi_factor((monitor.width as u32, monitor.height as u32), (monitor.width_in_millimeters as u64, monitor.height_in_millimeters as u64));
		if monitor.primary {
...
fn legacy_get_randr_scale_factor(connection: &XCBConnection, root: u32) -> Option<f32> {
	...
	let primary_output_cookie = connection.randr_get_output_info(primary_output, x11rb::CURRENT_TIME).ok()?;
	...
	{
		let crtc_cookie = connection.randr_get_crtc_info(primary_output_info.crtc, x11rb::CURRENT_TIME).ok()?;
		let crtc_info = crtc_cookie.reply().ok()?;

		if crtc_info.width > 0 && crtc_info.height > 0 {
			let scale_factor =
				get_dpi_factor((crtc_info.width as u32, crtc_info.height as u32), (primary_output_info.mm_width as u64, primary_output_info.mm_height as u64));
			return Some(scale_factor);
		}
	}
	...
	let mut crtc_infos: RapidHashMap<randr::Crtc, randr::GetCrtcInfoReply> = RapidHashMap::default();
	...
	let mut output_infos: RapidHashMap<randr::Output, randr::GetOutputInfoReply> = RapidHashMap::default();
	...
	for crtc_info in crtc_infos.values() {
		for &output in &crtc_info.outputs {
			if let Some(output_info) = output_infos.get(&output) {
				...
				let scale_factor =
					get_dpi_factor((crtc_info.width as u32, crtc_info.height as u32), (output_info.mm_width as u64, output_info.mm_height as u64));
