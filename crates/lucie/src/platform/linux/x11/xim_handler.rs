...
impl<C: Client<XEvent = xproto::KeyPressEvent>> ClientHandler<C> for XimHandler {
	...
	fn handle_get_im_values(&mut self, client: &mut C, input_method_id: u16, _attributes: AHashMap<AttributeName, Vec<u8>>) -> Result<(), ClientError> {
	...
	fn handle_create_ic(&mut self, _client: &mut C, _input_method_id: u16, input_context_id: u16) -> Result<(), ClientError> {
		self.connected = true;
		self.ic_id = input_context_id;
		Ok(())
	}

	fn handle_commit(&mut self, _client: &mut C, _input_method_id: u16, _input_context_id: u16, text: &str) -> Result<(), ClientError> {
	...
	fn handle_preedit_draw(
		...
		self.last_callback_event = Some(XimCallbackEvent::XimPreeditEvent(self.window, String::from(preedit_string)));
		Ok(())
	}
}
