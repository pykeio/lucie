...
#[track_caller]
pub fn uniform_list<R>(id: impl Into<ElementId>, item_count: usize, f: impl 'static + Fn(Range<usize>, &mut Window, &mut App) -> Vec<R>) -> UniformList
where
	R: IntoElement
{
	let id = id.into();
	...
	let render_range =
		move |range: Range<usize>, window: &mut Window, cx: &mut App| f(range, window, cx).into_iter().map(|component| component.into_any_element()).collect();
...
impl Element for UniformList {
	...
	fn request_layout(&mut self, global_id: Option<&GlobalElementId>, window: &mut Window, cx: &mut App) -> (LayoutId, Self::RequestLayoutState) {
		...
		let layout_id = self
			.interactivity
			.request_layout(global_id, window, cx, |style, window, cx| match self.sizing_behavior {
				...
				ListSizingBehavior::Auto => window.with_text_style(style.text_style().cloned(), |window| window.request_layout(style, None, cx))
			});
	...
	fn prepaint(
		...
		let can_scroll_horizontally = matches!(self.horizontal_sizing_behavior, ListHorizontalSizingBehavior::Unconstrained);
		...
		self.interactivity
			.prepaint(global_id, bounds, content_size, window, cx, |_style, mut scroll_offset, hitbox, window, cx| {
				...
				if self.item_count > 0 {
					...
					}) = shared_scroll_to_item
					{
						...
						if scroll_strict || is_above || is_below {
							...
							match strategy {
								ScrollStrategy::Top => {
									updated_scroll_offset.y = -(item_top - offset_pixels).clamp(Pixels::ZERO, max_scroll_offset);
								}
								...
								ScrollStrategy::Bottom => {
									updated_scroll_offset.y = -(item_bottom - list_height).clamp(Pixels::ZERO, max_scroll_offset);
								}
					...
					let last_visible_element_ix = ((-scroll_offset.y + padded_bounds.size.height) / item_height).ceil() as usize;

					let visible_range = first_visible_element_ix..cmp::min(last_visible_element_ix, self.item_count);

					let items = if y_flipped {
						let flipped_range = self.item_count.saturating_sub(visible_range.end)..self.item_count.saturating_sub(visible_range.start);
					...
					window.with_content_mask(Some(content_mask), |window| {
						for (mut item, ix) in items.into_iter().zip(visible_range.clone()) {
							let item_origin = padded_bounds.origin + scroll_offset + point(Pixels::ZERO, item_height * ix);
							...
							let available_space = size(AvailableSpace::Definite(available_width), AvailableSpace::Definite(item_height));
						...
						for decoration in &self.decorations {
							let mut decoration =
								decoration
									.as_ref()
									.compute(visible_range.clone(), bounds, scroll_offset, item_height, self.item_count, window, cx);
							let available_space = size(AvailableSpace::Definite(bounds.size.width), AvailableSpace::Definite(bounds.size.height));
	...
	fn paint(
		...
		self.interactivity.paint(global_id, bounds, hitbox.as_ref(), window, cx, |_, window, cx| {
...
impl<T: UniformListDecoration + 'static> UniformListDecoration for Entity<T> {
	fn compute(
		...
		self.update(cx, |inner, cx| inner.compute(visible_range, bounds, scroll_offset, item_height, item_count, window, cx))
	}
}
...
impl UniformList {
	...
	fn measure_item(&self, list_width: Option<Pixels>, window: &mut Window, cx: &mut App) -> Size<Pixels> {
		...
		let available_space = size(list_width.map_or(AvailableSpace::MinContent, |width| AvailableSpace::Definite(width)), AvailableSpace::MinContent);
...
mod test {
	...
	use crate::{self as lucie, Context, FocusHandle, ScrollStrategy, TestAppContext, UniformListScrollHandle, Window, div, prelude::*, uniform_list};
					...
					.child(
						uniform_list(
							...
							cx.processor(|this, range: Range<usize>, _window, _cx| {
								...
								range.map(|ix| div().id(ix).h(px(20.0)).child(format!("Item {ix}"))).collect()
							})
						)
