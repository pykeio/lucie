...
#[derive(Clone)]
enum ListItem {
	Unmeasured { focus_handle: Option<FocusHandle> },
	Measured { size: Size<Pixels>, focus_handle: Option<FocusHandle> }
}

impl ListItem {
	fn size(&self) -> Option<Size<Pixels>> {
		if let ListItem::Measured { size, .. } = self { Some(*size) } else { None }
	}
	...
	fn contains_focused(&self, window: &Window, cx: &App) -> bool {
		match self {
			ListItem::Unmeasured { focus_handle } | ListItem::Measured { focus_handle, .. } => {
				focus_handle.as_ref().is_some_and(|handle| handle.contains_focused(window, cx))
			}
		}
	}
}
...
impl ListState {
	...
	pub fn remeasure(&self) {
		...
		let new_items = state.items.iter().map(|item| ListItem::Unmeasured { focus_handle: item.focus_handle() });
	...
	pub fn bounds_for_item(&self, ix: usize) -> Option<Bounds<Pixels>> {
		...
		if let Some(&ListItem::Measured { size, .. }) = cursor.item() {
			...
			if count == ix {
				...
				return Some(Bounds::from_corners(point(bounds.left(), top), point(bounds.right(), top + size.height)));
			}
		}
		None
	}
	...
	pub fn max_offset_for_scrollbar(&self) -> Size<Pixels> {
		...
		let height = state.scrollbar_drag_start_height.unwrap_or_else(|| state.items.summary().height);
...
impl StateInner {
	...
	fn scroll(&mut self, scroll_top: &ListOffset, height: Pixels, delta: Point<Pixels>, current_view: EntityId, window: &mut Window, cx: &mut App) {
		...
		if self.alignment == ListAlignment::Bottom && new_scroll_top == scroll_max {
			...
			let (start, ..) = self.items.find::<ListItemSummary, _>((), &Height(new_scroll_top), Bias::Right);
			let item_ix = start.count;
			let offset_in_item = new_scroll_top - start.height;
			self.logical_scroll_top = Some(ListOffset { item_ix, offset_in_item });
		}
	...
	fn logical_scroll_top(&self) -> ListOffset {
		self.logical_scroll_top.unwrap_or_else(|| match self.alignment {
			ListAlignment::Top => ListOffset { item_ix: 0, offset_in_item: px(0.) },
	...
	fn scroll_top(&self, logical_scroll_top: &ListOffset) -> Pixels {
		let (start, ..) = self.items.find::<ListItemSummary, _>((), &Count(logical_scroll_top.item_ix), Bias::Right);
		start.height + logical_scroll_top.offset_in_item
	}

	fn layout_all_items(&mut self, available_width: Pixels, render_item: &mut RenderItemFn, window: &mut Window, cx: &mut App) {
	...
	fn layout_items(
		...
		let available_item_space =
			size(available_width.map_or(AvailableSpace::MinContent, |width| AvailableSpace::Definite(width)), AvailableSpace::MinContent);
		...
		if rendered_height - scroll_top.offset_in_item < available_height {
			while rendered_height < available_height {
				...
				if let Some(item) = cursor.item() {
					...
					measured_items.push_front(ListItem::Measured { size: element_size, focus_handle });
		...
		if !rendered_focused_item {
			...
			while let Some(item) = cursor.item() {
				if item.contains_focused(window, cx) {
					...
					item_layouts.push_back(ItemLayout { index: item_index, element, size });
					break;
				}
				cursor.next();
			}
	...
	fn prepaint_items(
		...
		window.transact(|window| {
			...
			let mut layout_response = self.layout_items(Some(bounds.size.width), bounds.size.height, &padding, render_item, window, cx);
						...
						if autoscroll_bounds.top() < bounds.top() {
							...
							while height > Pixels::ZERO {
								...
								let size = item.size().unwrap_or_else(|| {
									...
									let item_available_size = size(bounds.size.width.into(), AvailableSpace::MinContent);
	...
	fn set_offset_from_scrollbar(&mut self, point: Point<Pixels>) {
		...
		if self.alignment == ListAlignment::Bottom && new_scroll_top == scroll_max {
			...
			let (start, _, _) = self.items.find::<ListItemSummary, _>((), &Height(new_scroll_top), Bias::Right);
...
impl Element for List {
	...
	fn request_layout(&mut self, _id: Option<&GlobalElementId>, window: &mut Window, cx: &mut App) -> (crate::LayoutId, Self::RequestLayoutState) {
		let layout_id = match self.sizing_behavior {
			ListSizingBehavior::Infer => {
				...
				window.with_text_style(style.text_style().cloned(), |window| {
					...
					let padding = style
						.padding
						.to_pixels(state.last_layout_bounds.unwrap_or_default().size.into(), window.rem_size());

					let layout_response = state.layout_items(None, available_height, &padding, &mut self.render_item, window, cx);
			...
			ListSizingBehavior::Auto => {
				let mut style = Style::default();
				style.refine(&self.style);
				window.with_text_style(style.text_style().cloned(), |window| window.request_layout(style, None, cx))
			}
		};
		(layout_id, ())
	}
	...
	fn prepaint(
		...
		{
			let new_items = SumTree::from_iter(state.items.iter().map(|item| ListItem::Unmeasured { focus_handle: item.focus_handle() }), ());
		...
		let layout = match state.prepaint_items(bounds, padding, true, &mut self.render_item, window, cx) {
			...
			Err(autoscroll_request) => {
				...
				state.prepaint_items(bounds, padding, false, &mut self.render_item, window, cx).unwrap()
			}
		};
...
mod test {
	...
	use crate::{
		self as lucie, AppContext, AvailableSpace, Context, Element, IntoElement, ListState, Render, ScrollDelta, ScrollWheelEvent, TestAppContext, Window,
		div, list
	};
	...
	fn test_reset_after_paint_before_scroll(cx: &mut TestAppContext) {
		...
		state.scroll_to(lucie::ListOffset { item_ix: 0, offset_in_item: px(0.0) });

		struct TestView(ListState);
		impl Render for TestView {
			fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
				list(self.0.clone(), |_, _, _| div().h(px(10.)).w_full().into_any()).w_full().h_full()
			}
		}
	...
	fn test_scroll_by_positive_and_negative_distance(cx: &mut TestAppContext) {
		...
		impl Render for TestView {
			fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
				list(self.0.clone(), |_, _, _| div().h(px(20.)).w_full().into_any()).w_full().h_full()
			}
		}
	...
	fn test_remeasure(cx: &mut TestAppContext) {
		...
		impl Render for TestView {
			fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
				...
				list(self.state.clone(), move |_, _, _| div().h(px(height as f32)).w_full().into_any())
					.w_full()
					.h_full()
			}
		}
