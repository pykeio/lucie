...
/// An element that applies an animation to another element
...
impl<E: IntoElement + 'static> Element for AnimationElement<E> {
	...
	fn request_layout(&mut self, global_id: Option<&GlobalElementId>, window: &mut Window, cx: &mut App) -> (crate::LayoutId, Self::RequestLayoutState) {
...
mod easing {
	...
	pub fn bounce(easing: impl Fn(f32) -> f32) -> impl Fn(f32) -> f32 {
		move |delta| {
			if delta < 0.5 { easing(delta * 2.0) } else { easing((1.0 - delta) * 2.0) }
		}
	}
