...
impl Img {
	...
	pub fn extensions() -> &'static [&'static str] {
		...
		&[
			"avif", "jpg", "jpeg", "png", "gif", "webp", "tif", "tiff", "tga", "dds", "bmp", "ico", "hdr", "exr", "pbm", "pam", "ppm", "pgm", "ff", "farbfeld",
			"qoi", "svg"
		]
	}
	...
	///
	/// 1. Checking if any ancestor node of the current node contains an `ImageCacheElement`, If such a node exists, the
	///    image cache specified by that ancestor will be used.
...
impl Element for Img {
	...
	fn request_layout(&mut self, global_id: Option<&GlobalElementId>, window: &mut Window, cx: &mut App) -> (LayoutId, Self::RequestLayoutState) {
		let mut layout_state = ImgLayoutState { frame_index: 0, replacement: None };
		...
		window.with_optional_element_state(global_id, |state, window| {
			...
			let layout_id = self.interactivity.request_layout(global_id, window, cx, |mut style, window, cx| {
				let mut replacement_id = None;

				match self
					.source
					.use_data(self.image_cache.clone().or_else(|| window.image_cache_stack.last().cloned()), window, cx)
				{
					Some(Ok(data)) => {
						...
						if let Length::Auto = style.size.width {
							style.size.width = match style.size.height {
								Length::Definite(DefiniteLength::Absolute(abs_length)) => {
									...
									Length::Definite(px(image_size.width.0 * height_px.0 / image_size.height.0).into())
								}
						...
						if let Length::Auto = style.size.height {
							style.size.height = match style.size.width {
								Length::Definite(DefiniteLength::Absolute(abs_length)) => {
									...
									Length::Definite(px(image_size.height.0 * width_px.0 / image_size.width.0).into())
								}
	...
	fn prepaint(
		...
		self.interactivity
			.prepaint(global_id, bounds, bounds.size, window, cx, |_, _, hitbox, window, cx| {
	...
	fn paint(
		...
		self.interactivity
			.paint(global_id, bounds, hitbox.as_ref(), window, cx, |style, window, cx| {
				if let Some(Ok(data)) = source.use_data(self.image_cache.clone().or_else(|| window.image_cache_stack.last().cloned()), window, cx) {
					let new_bounds = self.style.object_fit.get_bounds(bounds, data.size(layout_state.frame_index));
					let corner_radii = style.corner_radii.to_pixels(window.rem_size()).clamp_radii_for_quad_size(new_bounds.size);
					window
						.paint_image(new_bounds, corner_radii, data, layout_state.frame_index, self.style.grayscale)
						.log_err();
...
impl ImageSource {
	pub(crate) fn use_data(&self, cache: Option<AnyImageCache>, window: &mut Window, cx: &mut App) -> Option<Result<Arc<RenderImage>, ImageCacheError>> {
		match self {
	...
	pub(crate) fn get_data(&self, cache: Option<AnyImageCache>, window: &mut Window, cx: &mut App) -> Option<Result<Arc<RenderImage>, ImageCacheError>> {
		match self {
			ImageSource::Resource(resource) => {
				if let Some(cache) = cache {
					cache.load(resource, window, cx)
...
/// An image loader for the asset system
#[derive(Clone)]
			...
			let bytes = match source.clone() {
				...
				Resource::Uri(uri) => {
					...
					if !response.status().is_success() {
						...
						return Err(ImageCacheError::BadStatus { uri, status: response.status(), body });
					}
					body
				}
				Resource::Embedded(path) => {
					...
					if let Some(data) = data {
						...
						return Err(ImageCacheError::Asset(format!("Embedded resource not found: {}", path).into()));
					}
				}
			};
