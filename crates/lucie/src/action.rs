...
macro_rules! actions {
    ($namespace:path, [ $( $(#[$attr:meta])* $name:ident),* $(,)? ]) => {
        $(
            #[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug, lucie::Action)]
...
/// ```
///
/// The derive macro for `Action` requires that the type implement `Clone` and `PartialEq`.
///
...
///
/// - `builder = xyz::builder_fn` specifies the path to a function that will be used as the action builder. See
///   [`Action::build`]. This is required for actions with data (i.e. `#[derive]`d non-unit structs).
///
/// - `no_register` skips registering the action. This is useful for implementing the `Action` trait while not
///   supporting invocation by name or JSON deserialization.
///
/// - `deprecated_aliases = ["editor::SomeAction"]` specifies deprecated old names for the action. These action names
///   should *not* correspond to any actions that are registered. These old names can then still be used to refer to
///   invoke this action. In Zed, the keymap JSON schema will accept these old names and provide warnings.
///
/// - `deprecated = "Message about why this action is deprecation"` specifies a deprecation message. In Zed, the keymap
///   JSON schema will cause this to be displayed as a warning.
///
...
///
/// ```
/// use lucie::{SharedString, register_action};
/// #[derive(Clone, PartialEq, Eq)]
/// pub struct Paste {
/// 	pub content: SharedString
/// }
///
/// impl lucie::Action for Paste {
///     # fn boxed_clone(&self) -> Box<dyn lucie::Action> { unimplemented!()}
///     # fn partial_eq(&self, other: &dyn lucie::Action) -> bool { unimplemented!() }
///     # fn name(&self) -> &'static str { "Paste" }
///     # fn name_for_type() -> &'static str { "Paste" }
///     # fn build(value: Option<&mut [u8]>) -> anyhow::Result<Box<dyn lucie::Action>> {
///     #     unimplemented!()
///     # }
/// }
///
...
pub trait Action: Any + Send {
	...
	/// Build this action from a serialized form. This is used to construct actions from the keymap.
	fn build(value: Option<&mut [u8]>) -> Result<Box<dyn Action>>
	where
		Self: Sized;
...
type ActionBuilder = fn(data: Option<&mut [u8]>) -> anyhow::Result<Box<dyn Action>>;

pub(crate) struct ActionRegistry {
	by_name: RapidHashMap<&'static str, ActionData>,
	names_by_type_id: RapidHashMap<TypeId, &'static str>,
	all_names: Vec<&'static str>,                                   // So we can return a static slice.
	deprecated_aliases: RapidHashMap<&'static str, &'static str>,   // deprecated name -> preferred name
	deprecation_messages: RapidHashMap<&'static str, &'static str>, // action name -> deprecation message
	documentation: RapidHashMap<&'static str, &'static str>         // action name -> documentation
}
...
impl ActionRegistry {
	...
	fn insert_action(&mut self, action: MacroActionData) {
		...
		self.by_name.insert(name, ActionData { build: action.build });
		for &alias in action.deprecated_aliases {
			...
			self.by_name.insert(alias, ActionData { build: action.build });
	...
	pub fn build_action(&self, name: &str, data: Option<&mut [u8]>) -> std::result::Result<Box<dyn Action>, ActionBuildError> {
		let build_action = self
			...
			.build;
		(build_action)(data).map_err(|e| ActionBuildError::BuildError { name: name.to_owned(), error: e })
	}
	...
	pub fn deprecated_aliases(&self) -> &RapidHashMap<&'static str, &'static str> {
		&self.deprecated_aliases
	}

	pub fn deprecation_messages(&self) -> &RapidHashMap<&'static str, &'static str> {
		&self.deprecation_messages
	}

	pub fn documentation(&self) -> &RapidHashMap<&'static str, &'static str> {
		&self.documentation
	}
}
...
pub fn generate_list_of_all_registered_actions() -> impl Iterator<Item = MacroActionData> {
	inventory::iter::<MacroActionBuilder>.into_iter().map(|builder| builder.0())
}
...
mod no_action {
	...
	pub fn is_no_action(action: &dyn lucie::Action) -> bool {
