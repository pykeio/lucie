#![allow(clippy::type_complexity)] // Not useful, we make heavy use of callbacks
...
/// Do not touch, here be dragons for use by lucie_macros and such.
#[doc(hidden)]
pub mod private {
	pub use anyhow;
	pub use inventory;
...
mod seal {
	/// A mechanism for restricting implementations of a trait to only those in Lucie.
...
/// The context trait, allows the different contexts in Lucie to be used
/// interchangeably for certain operations.
pub trait AppContext {
	/// Create a new entity in the app context.
	#[expect(clippy::wrong_self_convention, reason = "`App::new` is an ubiquitous function for creating entities")]
...
/// This trait is used for the different visual contexts in Lucie that
...
pub trait VisualContext: AppContext {
	...
	fn update_window_entity<T: 'static, R>(&mut self, entity: &Entity<T>, update: impl FnOnce(&mut T, &mut Window, &mut Context<T>) -> R) -> Self::Result<R>;
...
/// A trait for tying together the types of a Lucie entity and the events it can
/// emit.
...
/// Information about the GPU Lucie is running on.
