use lucie::{App, Application, Context, Half, Hsla, Pixels, Point, Window, WindowBounds, WindowOptions, div, prelude::*, px, rgb, size};
...
impl Render for DragInfo {
	fn render(&mut self, _: &mut Window, _: &mut Context<'_, Self>) -> impl IntoElement {
		let size = lucie::size(px(120.), px(50.));
...
impl Render for DragDrop {
	fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
		let items = [lucie::blue(), lucie::red(), lucie::green()];
				...
				div()
					...
					.children(items.into_iter().enumerate().map(|(ix, color)| {
						...
						div()
							...
							.on_drag(drag_info, |info: &DragInfo, position, _, cx| cx.new(|_| info.position(position)))
					}))
			)
			.child(
				div()
					...
					.items_center()
					.border_3()
					.border_color(self.drop_on.map(|info| info.color).unwrap_or(lucie::black()))
