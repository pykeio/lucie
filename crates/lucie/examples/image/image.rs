...
use anyhow::Result;
use lucie::{
	App, AppContext, Application, AssetSource, Bounds, Context, ImageSource, KeyBinding, Menu, MenuItem, Point, SharedString, SharedUri, TitlebarOptions,
...
impl ImageContainer {
	pub fn new(text: impl Into<SharedString>, src: impl Into<ImageSource>) -> Self {
		Self { text: text.into(), src: src.into() }
	}
}

impl RenderOnce for ImageContainer {
	fn render(self, _window: &mut Window, _: &mut App) -> impl IntoElement {
		div().child(div().flex_row().size_full().gap_4().child(self.text).child(img(self.src).size(px(256.0))))
	}
}
...
impl Render for ImageShowcase {
	fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
		div().id("main").bg(lucie::white()).overflow_y_scroll().p_5().size_full().child(
			div()
				.flex()
				.flex_col()
				.justify_center()
				.items_center()
				.gap_8()
				.child(img("https://github.com/zed-industries/zed/actions/workflows/ci.yml/badge.svg"))
				.child(
					div()
						.flex()
						.flex_row()
...
fn main() {
	...
	Application::new()
		.with_assets(Assets { base: manifest_dir.join("examples") })
		.run(move |cx: &mut App| {
