...
use lucie::{
	App, Application, Bounds, Context, MouseDownEvent, MouseMoveEvent, MouseUpEvent, Pixels, Point, Render, SharedString, UniformListScrollHandle, Window,
	WindowBounds, WindowOptions, canvas, div, point, prelude::*, px, rgb, size, uniform_list
};
...
impl Quote {
	pub fn random() -> Self {
		...
		let prev_close = 100.0 + rng.f64() * 200.0;
		let change = (rng.f64() * 10.0) - 5.0;
		let last_done = prev_close + change;
		let open = prev_close + (rng.f64() * 6.0) - 3.0;
		let high = (prev_close + rng.f64() * 10.0).max(open);
		let low = (prev_close - rng.f64() * 10.0).min(open);
		let timestamp = Duration::from_secs(rng.u64(0..86400));
		let volume = rng.i64(1_000_000..100_000_000);
		let turnover = last_done * volume as f64;
		let symbol = {
			let mut ticker = String::new();
			if rng.bool() {
				ticker.push_str(&format!("{:03}.{}", rng.usize(100..1000), rng.usize(0..10)));
			} else {
				ticker.push_str(&format!("{}{}", rng.char('A'..='Z'), rng.char('A'..='Z')));
			}
			ticker.push_str(&format!(".{}", rng.char('A'..='Z')));
			ticker
		};
		let name = format!("{} {} - #{}", symbol, rng.usize(1..100), rng.usize(10000..100000));
		let ttm = rng.f64() * 10.0;
		let market_cap = 1_000_000.0 + rng.f64() * 10_000_000.0;
		let float_cap = market_cap + 1_000.0 + rng.f64() * 9_000.0;
		let shares = 100.0 + rng.f64() * 900.0;
		let pb = market_cap / shares;
		let pe = market_cap / shares;
		let eps = market_cap / shares;
		let dividend = rng.f64() * 10.0;
		let dividend_yield = rng.f64() * 10.0;
		let dividend_per_share = rng.f64() * 10.0;
		let dividend_date = SharedString::new(format!("{}-{}-{}", rng.usize(2000..2023), rng.usize(1..12), rng.usize(1..28)));
		let dividend_payment = rng.f64() * 10.0;
	...
	fn change_color(&self) -> lucie::Hsla {
		if self.change() > 0.0 { lucie::green() } else { lucie::red() }
	}
...
impl TableRow {
	...
	fn render_cell(&self, key: &str, width: Pixels, color: lucie::Hsla) -> impl IntoElement {
		div().whitespace_nowrap().truncate().w(width).px_1().child(match key {
			...
			"timestamp" => div().text_color(color).child(format!("{:?}", self.quote.timestamp.as_secs())),
...
impl RenderOnce for TableRow {
	fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
		...
		div()
			...
			.bg(if self.ix.is_multiple_of(2) { rgb(0xFFFFFF) } else { rgb(0xFAFAFA) })
			.py_0p5()
			.px_2()
...
impl DataTable {
	...
	fn scroll_height(&self) -> Pixels {
		self.scroll_handle.0.borrow().last_item_size.unwrap_or_default().contents.height
	}
	...
	fn render_scrollbar(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
		...
		let offset_top = (table_height * percentage).clamp(px(4.), (table_height - SCROLLBAR_THUMB_HEIGHT - px(4.)).max(px(4.)));
				...
				canvas(
					...
					move |thumb_bounds, _, window, _| {
						...
						window.on_mouse_event(move |ev: &MouseMoveEvent, _, _, cx| {
							...
							let percentage = ((ev.position.y - table_bounds.origin.y + inside_offset) / (table_bounds.size.height)).clamp(0., 1.);

							let offset_y = ((scroll_height - table_bounds.size.height) * percentage).clamp(px(0.), scroll_height - SCROLLBAR_THUMB_HEIGHT);
...
impl Render for DataTable {
	fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
		div()
			...
			.flex()
			.flex_col()
			.child(format!("Total {} items, visible range: {:?}", self.quotes.len(), self.visible_range))
			.child(
				div()
					.flex()
					.flex_col()
...
fn main() {
	Application::new().run(|cx: &mut App| {
		cx.open_window(
			WindowOptions {
				focus: true,
				window_bounds: Some(WindowBounds::centered(size(px(1280.0), px(1000.0)), cx)),
				..Default::default()
			},
			|_, cx| {
				cx.new(|_| {
