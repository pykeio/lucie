//! Example demonstrating Lucie's testing infrastructure.
//!
//! When run normally, this displays an interactive counter window.
//! The tests below demonstrate various Lucie testing patterns.
//!
...
struct Counter {
	...
	_subscription: lucie::Subscription
}

/// Event emitted by Counter
struct CounterEvent;

impl lucie::EventEmitter<CounterEvent> for Counter {}
...
impl Render for Counter {
	fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
		div()
			...
			.justify_center()
			.items_center()
			.child(div().text_3xl().text_color(rgb(0xcdd6f4)).child(format!("{}", self.count)))
			.child(
				div()
					.flex()
					.gap_2()
					.child(
						div()
							...
							.on_click(cx.listener(|this, _, window, cx| this.decrement(&Decrement, window, cx)))
							.child("−")
					)
					.child(
						div()
							...
							.cursor_pointer()
							.text_color(rgb(0xcdd6f4))
							.on_click(cx.listener(|this, _, window, cx| this.increment(&Increment, window, cx)))
							.child("+")
					)
			)
			.child(
			...
			.child(div().text_sm().text_color(rgb(0x6c7086)).child("Press ↑/↓ or click buttons"))
	}
}

fn main() {
	Application::new().run(|cx: &mut App| {
		cx.bind_keys([lucie::KeyBinding::new("up", Increment, Some("Counter")), lucie::KeyBinding::new("down", Decrement, Some("Counter"))]);
...
mod tests {
	...
	/// Here's a basic test. Just add the macro and take a TestAppContext as an argument!
	///
	...
	fn basic_testing(cx: &mut TestAppContext) {
		...
		assert_eq!(count_after_update, 999, "Side effects should run after update completes");
	}
	...
	fn test_counter_in_window(cx: &mut TestAppContext) {
		...
		assert_eq!(count_after, 1, "Action dispatched via focus handle should increment");
	}
	...
	fn test_counter_random_operations(cx: &mut TestAppContext, mut rng: StdRng) {
		...
		for _ in 0..100 {
			if rng.random_bool(0.5) {
				...
				expected -= 1;
				counter.update_in(&mut cx, |counter, window, cx| counter.decrement(&Decrement, window, cx));
			}
		}

		let actual = counter.read_with(&cx, |counter, _| counter.count);
		assert_eq!(actual, expected, "Counter should match expected after random ops");
	}

	/// Now, all of those tests are good, but Lucie also provides strong support for testing distributed systems.
	...
	mod distributed_systems {
		...
		impl MockNetwork {
			...
			fn a_client(&self) -> NetworkClient {
				NetworkClient { network: self.clone(), is_a: true }
			}

			fn b_client(&self) -> NetworkClient {
				NetworkClient { network: self.clone(), is_a: false }
			}
		}
		...
		impl NetworkClient {
			...
			fn receive_all(&self) -> Vec<i32> {
				let mut network = self.network.state.lock().unwrap();
				if self.is_a { network.a_to_b.drain(..).collect() } else { network.b_to_a.drain(..).collect() }
			}
		}
		...
		fn test_random_interleaving(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext, mut rng: StdRng) {
			...
			let diff: Vec<_> = original_order
				.iter()
				.zip(actual.iter())
				.map(|(o, a)| if o == a { " ".to_string() } else { "^".to_string() })
				.collect();
