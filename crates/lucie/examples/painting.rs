use lucie::{
	Application, Background, Bounds, ColorSpace, Context, MouseDownEvent, Path, PathBuilder, PathStyle, Pixels, Point, Render, StrokeOptions, Window,
	WindowOptions, canvas, div, linear_color_stop, linear_gradient, point, prelude::*, px, quad, rgb, size
};
...
impl PaintingViewer {
	fn new(_window: &mut Window, _cx: &mut Context<Self>) -> Self {
		...
		builder.scale(0.9);
		let path = builder.build().unwrap();
		lines.push((path, lucie::black().into()));

		// draw a lightening bolt ⚡
		let mut builder = PathBuilder::fill();
		builder.add_polygon(&[point(px(150.), px(300.)), point(px(200.), px(225.)), point(px(200.), px(275.)), point(px(250.), px(200.))], false);
		let path = builder.build().unwrap();
		...
		let path = builder.build().unwrap();
		lines.push((path, linear_gradient(180., linear_color_stop(rgb(0xFACC15), 0.7), linear_color_stop(rgb(0xD56D0C), 1.)).color_space(ColorSpace::Oklab)));
		...
		builder.curve_to(square_bounds.origin + point(horizontal_offset, vertical_offset), square_bounds.origin + point(px(0.0), vertical_offset));
		builder.line_to(square_bounds.top_right() + point(-horizontal_offset, vertical_offset));
		builder.curve_to(square_bounds.bottom_right(), square_bounds.top_right() + point(px(0.0), vertical_offset));
		builder.line_to(square_bounds.bottom_left());
		let path = builder.build().unwrap();
		lines.push((path, linear_gradient(180., linear_color_stop(lucie::blue(), 0.4), linear_color_stop(lucie::red(), 1.))));
		...
		// draw a wave
		let options = StrokeOptions::default().with_line_width(1.).with_line_join(lyon::path::LineJoin::Bevel);
		let mut builder = PathBuilder::stroke(px(1.)).with_style(PathStyle::Stroke(options));
		builder.move_to(point(px(40.), px(420.)));
		for i in 1..50 {
			builder.line_to(point(px(40.0 + i as f32 * 10.0), px(420.0 + (i as f32 * 10.0).sin() * 40.0)));
		}
		let path = builder.build().unwrap();
		lines.push((path, lucie::green().into()));
		...
		div()
				...
				div()
					...
					.child(
						div()
							.flex()
							.gap_x_2()
							.child(button(if dashed { "Solid" } else { "Dashed" }, cx, move |this, _| this.dashed = !dashed))
			...
			.child(
				div()
						...
						canvas(
							...
							move |_, _, window, _| {
								...
								for (bounds, color) in background_quads.iter() {
									window.paint_quad(quad(*bounds, px(0.), *color, px(0.), lucie::transparent_black(), Default::default()));
								}
								...
								for points in lines {
									...
									if let Ok(path) = builder.build() {
										window.paint_path(path, lucie::black());
									}
								}
							}
						)
					...
					.on_mouse_move(cx.listener(|this, ev: &lucie::MouseMoveEvent, _, cx| {
						if !this._painting {
							return;
						}
...
fn main() {
	Application::new().run(|cx| {
		cx.open_window(WindowOptions { focus: true, ..Default::default() }, |window, cx| cx.new(|cx| PaintingViewer::new(window, cx)))
			.unwrap();
