...
impl ImageGallery {
	fn on_next_image(&mut self, _: &ClickEvent, window: &mut Window, cx: &mut Context<Self>) {
		self.image_cache.update(cx, |image_cache, cx| image_cache.clear(window, cx));
				...
				div()
					...
					.child(
						div()
							...
							.flex_row()
							.justify_between()
							.child(format!("Example to show images and test memory usage (Rendered: {} images).", self.total_count))
							.child(
								div()
									.id("btn")
									.py_1()
...
fn simple_lru_cache(id: impl Into<ElementId>, max_items: usize) -> SimpleLruCacheProvider {
	SimpleLruCacheProvider { id: id.into(), max_items }
}
...
impl ImageCacheProvider for SimpleLruCacheProvider {
	fn provide(&mut self, window: &mut Window, cx: &mut App) -> lucie::AnyImageCache {
		window
			.with_global_id(self.id.clone(), |global_id, window| {
				window.with_element_state::<Entity<SimpleLruCache>, _>(global_id, |lru_cache, _window| {
					let mut lru_cache = lru_cache.unwrap_or_else(|| cx.new(|cx| SimpleLruCache::new(self.max_items, cx)));
...
struct SimpleLruCache {
	max_items: usize,
	usages: Vec<u64>,
	cache: HashMap<u64, lucie::ImageCacheItem>
}
...
impl ImageCache for SimpleLruCache {
	fn load(&mut self, resource: &lucie::Resource, window: &mut Window, cx: &mut App) -> Option<Result<Arc<lucie::RenderImage>, lucie::ImageCacheError>> {
		...
		self.cache.insert(hash, lucie::ImageCacheItem::Loading(task.clone()));
		self.usages.insert(0, hash);
