...
use anyhow::Result;
use lucie::{
	App, Application, AssetSource, BoxShadow, ClickEvent, Context, SharedString, Task, Window, WindowBounds, WindowOptions, div, hsla, img, point, prelude::*,
	px, rgb, size, svg
};
		...
		div()
			...
			.child(
				div()
					...
					.bg(lucie::white())
					.border_3()
					.border_color(lucie::red())
					.text_color(lucie::yellow())
					.child(
						div()
							.flex()
							.flex_col()
							...
							.child(
								div()
									...
									.text_decoration_color(lucie::red())
									.child(format!("opacity: {:.1}", self.opacity))
							)
							.child(svg().path("image/arrow_circle.svg").text_color(lucie::black()).text_2xl().size_8())
							.child(
								div().flex().children(
									["🎊", "✈️", "🎉", "🎈", "🎁", "🎂"].map(|emoji| div().child(emoji.to_string()).hover(|style| style.opacity(0.5)))
								)
							)
...
fn main() {
	Application::new()
		...
		.run(|cx: &mut App| {
			cx.open_window(
				WindowOptions {
					window_bounds: Some(WindowBounds::centered(size(px(500.0), px(500.0)), cx)),
					..Default::default()
				},
