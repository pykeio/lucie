use lucie::{App, Application, Context, TextOverflow, Window, WindowBounds, WindowOptions, div, prelude::*, px, size};
		...
		div()
			...
			.child(
				div()
					...
					.child(div().flex().border_1().border_color(lucie::red()).text_ellipsis().child("short flex"))
					.child(
						div()
							.overflow_hidden()
							.border_1()
			...
			.child(
				div()
					...
					.border_1()
					.border_color(lucie::blue())
					.child(text.to_owned() + "<- ELLIPSIS START")
			)
			.child(
				div()
					.flex_shrink_0()
