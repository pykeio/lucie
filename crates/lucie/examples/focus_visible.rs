...
impl Example {
	fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
		let items = vec![
			(cx.focus_handle().tab_index(1).tab_stop(true), "Button with .focus() - always shows border when focused"),
			(cx.focus_handle().tab_index(2).tab_stop(true), "Button with .focus_visible() - only shows border with keyboard"),
			(cx.focus_handle().tab_index(3).tab_stop(true), "Button with both .focus() and .focus_visible()"),
		];
	...
	fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
		...
		fn button_base(id: impl Into<ElementId>, label: &'static str) -> Stateful<Div> {
			div()
				...
				.justify_center()
				.items_center()
				.bg(lucie::rgb(0x2563eb))
				.text_color(lucie::white())
				.rounded_md()
				.cursor_pointer()
				.hover(|style| style.bg(lucie::rgb(0x1d4ed8)))
				.child(label)
		}
		...
		div()
			...
			.p_8()
			.gap_6()
			.bg(lucie::rgb(0xf3f4f6))
			.child(
				div()
					.text_2xl()
					.font_weight(lucie::FontWeight::BOLD)
					.text_color(lucie::rgb(0x111827))
			...
			.child(
				div()
					.p_4()
					.rounded_md()
					.bg(lucie::rgb(0xdbeafe))
					.text_color(lucie::rgb(0x1e3a8a))
						...
						div()
							...
							.child(
								button_base("button2", self.items[1].1)
									...
									.focus_visible(|style| style.border_4().border_color(lucie::rgb(0x10b981)))
									.on_click(cx.listener(|this, _, _, cx| {
