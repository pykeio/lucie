		...
		for meta in metas {
			...
			match (&meta, ident.as_str()) {
				...
				(Meta::NameValue(meta), "on_failure") => {
					let Expr::Lit(ExprLit { lit: Lit::Str(name), .. }) = &meta.value else {
						return Err(syn::Error::new(meta.value.span(), "on_failure argument must be a string"));
					};
					let segments = name
						.value()
						.split("::")
...
fn parse_usize_from_expr(expr: &Expr) -> Result<usize, syn::Error> {
	...
	int.base10_parse().map_err(|_| syn::Error::new(int.span(), "failed to parse integer"))
}
