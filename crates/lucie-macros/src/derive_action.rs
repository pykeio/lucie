...
use quote::quote;
use syn::{Data, DeriveInput, LitStr, Path, Token, parse::ParseStream};
...
pub(crate) fn derive_action(input: TokenStream) -> TokenStream {
	...
	for attr in &input.attrs {
		if attr.path().is_ident("action") {
			attr.parse_nested_meta(|meta| {
				if meta.path.is_ident("name") {
					...
					if builder_fn.is_some() {
						return Err(meta.error("'builder' argument specified multiple times"));
					}
					meta.input.parse::<Token![=]>()?;
					let path: Path = meta.input.parse()?;
					builder_fn = Some(path);
					...
					return Err(meta.error(format!(
                        ...
                        'namespace', 'builder', 'no_register, 'deprecated_aliases', or 'deprecated'",
						meta.path
					)));
				}
				Ok(())
			...
			if let Meta::NameValue(MetaNameValue {
				value: Lit(ExprLit { lit: Str(ref lit_str), .. }),
				..
			}) = attr.meta
			{
	...
	let full_name = if let Some(namespace) = namespace { format!("{namespace}::{name}") } else { name };

	let is_unit_struct = matches!(&input.data, Data::Struct(data) if data.fields.is_empty());

	let build_fn_body = if let Some(builder_fn) = builder_fn {
		quote! { Ok(Box::new(#builder_fn(_data)?)) }
