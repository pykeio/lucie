...
impl PlatformDispatcher for LinuxDispatcher {
	...
	fn dispatch_on_main_thread(&self, runnable: RunnableVariant, priority: Priority) {
		self.main_sender.send(priority, runnable).unwrap_or_else(|runnable| {
...
impl<T> PriorityQueueCalloopReceiver<T> {
	pub fn new() -> (PriorityQueueCalloopSender<T>, Self) {
		...
		(PriorityQueueCalloopSender::new(tx, ping.clone()), Self { receiver: rx, source, ping })
	}
}
...
impl<T> calloop::EventSource for PriorityQueueCalloopReceiver<T> {
	...
	fn process_events<F>(&mut self, readiness: calloop::Readiness, token: calloop::Token, mut callback: F) -> Result<calloop::PostAction, Self::Error>
	where
...
mod tests {
	...
	fn calloop_works() {
		...
		let mut data = Data { got_msg: false, got_closed: false };
		...
		event_loop.dispatch(Some(::std::time::Duration::ZERO), &mut data).unwrap();
...
// thread 'platform::linux::dispatcher::tests::tomato' (478301) panicked at
// crates/gpui/src/platform/linux/dispatcher.rs:515:9: assertion failed: data.got_closed
