...
struct CosmicTextSystemState {
	...
	font_ids_by_family_cache: RapidHashMap<FontKey, SmallVec<[FontId; 4]>>
}
...
impl CosmicTextSystem {
	pub(crate) fn new() -> Self {
		...
		Self(RwLock::new(CosmicTextSystemState {
			...
			font_ids_by_family_cache: RapidHashMap::default()
		}))
	}
}
...
impl PlatformTextSystem for CosmicTextSystem {
	...
	fn typographic_bounds(&self, font_id: FontId, glyph_id: GlyphId) -> Result<Bounds<f32>> {
		...
		Ok(Bounds {
			origin: point(0.0, 0.0),
			size: size(glyph_metrics.advance_width(glyph_id), glyph_metrics.advance_height(glyph_id))
		})
	}
...
impl CosmicTextSystemState {
	...
	fn load_family(&mut self, name: &str, features: &FontFeatures) -> Result<SmallVec<[FontId; 4]>> {
		...
		for (font_id, postscript_name) in families {
			let font = self.font_system.get_font(font_id).context("Could not load font")?;
	...
	fn glyph_for_char(&self, font_id: FontId, ch: char) -> Option<GlyphId> {
		...
		if glyph_id == 0 { None } else { Some(GlyphId(glyph_id.into())) }
	}
	...
	#[profiling::function]
	fn rasterize_glyph(&mut self, params: &RenderGlyphParams, glyph_bounds: Bounds<DevicePixels>) -> Result<(Size<DevicePixels>, Vec<u8>)> {
	...
	fn font_id_for_cosmic_id(&mut self, id: cosmic_text::fontdb::ID) -> FontId {
		if let Some(ix) = self.loaded_fonts.iter().position(|loaded_font| loaded_font.font.id() == id) {
			FontId(ix)
		} else {
	...
	fn layout_line(&mut self, text: &str, font_size: Pixels, font_runs: &[FontRun]) -> LineLayout {
		...
		let line = ShapeLine::new(&mut self.font_system, text, &attrs_list, cosmic_text::Shaping::Advanced, 4);
		...
		for glyph in &layout.glyphs {
			...
			if let Some(last_run) = runs.last_mut().filter(|last_run| last_run.font_id == font_id) {
				...
				runs.push(ShapedRun { font_id, glyphs: vec![shaped_glyph] });
			}
		}
...
impl TryFrom<&FontFeatures> for CosmicFontFeatures {
	...
	fn try_from(features: &FontFeatures) -> Result<Self> {
		...
		for feature in features.0.iter() {
			let name_bytes: [u8; 4] = feature.0.as_bytes().try_into().context("Incorrect feature flag format")?;
