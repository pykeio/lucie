use std::{cell::RefCell, ffi::c_void, fmt::Display, num::NonZeroU32, ops::Div, ptr::NonNull, rc::Rc, sync::Arc};
...
fn query_render_extent(xcb: &Rc<XCBConnection>, x_window: xproto::Window) -> anyhow::Result<gpu::Extent> {
...
pub(crate) fn xcb_flush(xcb: &XCBConnection) {
	xcb.flush().map_err(handle_connection_error).context("X11 flush failed").log_err();
}

pub(crate) fn check_reply<E, F, C>(failure_context: F, result: Result<VoidCookie<'_, C>, ConnectionError>) -> anyhow::Result<()>
where
...
pub(crate) fn get_reply<E, F, C, O>(failure_context: F, result: Result<Cookie<'_, C, O>, ConnectionError>) -> anyhow::Result<O>
where
	E: Display + Send + Sync + 'static,
	F: FnOnce() -> E,
	C: RequestConnection,
	...
	pub fn new(
		...
		let setup_result = trys!({
			...
			check_reply(
				...
				xcb.change_property32(xproto::PropMode::REPLACE, x_window, atoms._NET_WM_PID, xproto::AtomEnum::CARDINAL, &[pid])
			)?;
			...
			if let Some(size) = params.window_min_size {
				...
				check_reply(|| format!("X11 change of WM_SIZE_HINTS failed. min_size: {:?}", min_size), size_hints.set_normal_hints(xcb, x_window))?;
			}
				...
				&& let Some(title) = titlebar.title
			{
				check_reply(
					...
					xcb.change_property8(xproto::PropMode::REPLACE, x_window, xproto::AtomEnum::WM_NAME, xproto::AtomEnum::STRING, title.as_bytes())
				)?;
			}
			...
			if params.kind == WindowKind::Floating {
				if let Some(parent_window) = parent_window {
					...
					check_reply(
						...
						xcb.change_property32(xproto::PropMode::REPLACE, x_window, atoms.WM_TRANSIENT_FOR, xproto::AtomEnum::WINDOW, &[parent_window])
					)?;
				}
			...
			check_reply(|| "X11 sync CreateCounter failed.", sync::create_counter(xcb, sync_request_counter, sync::Int64 { lo: 0, hi: 0 }))?;
		...
		if setup_result.is_err() {
			check_reply(|| "X11 DestroyWindow failed while cleaning it up after setup failure.", xcb.destroy_window(x_window))?;
			xcb_flush(xcb);
		}
...
impl X11Window {
	...
	fn set_wm_hints<C: Display + Send + Sync + 'static, F: FnOnce() -> C>(
		...
		let message = ClientMessageEvent::new(32, self.0.x_window, state.atoms._NET_WM_STATE, [wm_hint_property_state as u32, prop1, prop2, 1, 0]);
		check_reply(
			failure_context,
			self.0
				.xcb
				.send_event(false, state.x_root_window, xproto::EventMask::SUBSTRUCTURE_REDIRECT | xproto::EventMask::SUBSTRUCTURE_NOTIFY, message)
		)?;
		xcb_flush(&self.0.xcb);
		Ok(())
	}
	...
	fn send_moveresize(&self, flag: u32) -> anyhow::Result<()> {
		...
		check_reply(|| "X11 UngrabPointer before move/resize of window failed.", self.0.xcb.ungrab_pointer(x11rb::CURRENT_TIME))?;

		let pointer = get_reply(|| "X11 QueryPointer before move/resize of window failed.", self.0.xcb.query_pointer(self.0.x_window))?;
...
impl X11WindowStatePtr {
	...
	fn set_edge_constraints(&self, mut state: std::cell::RefMut<X11WindowState>) -> anyhow::Result<()> {
		let reply = get_reply(
			...
			self.xcb
				.get_property(false, self.x_window, state.atoms._GTK_EDGE_CONSTRAINTS, xproto::AtomEnum::CARDINAL, 0, 4)
		)?;
	...
	fn set_wm_properties(&self, mut state: std::cell::RefMut<X11WindowState>) -> anyhow::Result<()> {
		let reply = get_reply(
			...
			self.xcb
				.get_property(false, self.x_window, state.atoms._NET_WM_STATE, xproto::AtomEnum::ATOM, 0, u32::MAX)
		)?;
	...
	pub fn set_bounds(&self, bounds: Bounds<i32>) -> anyhow::Result<()> {
		...
		let is_resize;
		{
			...
			if true {
				state
					.renderer
					.update_drawable_size(size(DevicePixels(gpu_size.width as i32), DevicePixels(gpu_size.height as i32)));
				resize_args = Some((state.content_size(), state.scale_factor));
			}
			if let Some(value) = state.last_sync_counter.take() {
				check_reply(|| "X11 sync SetCounter failed.", sync::set_counter(&self.xcb, state.counter_id, value))?;
			}
		}
...
impl PlatformWindow for X11Window {
	...
	fn content_size(&self) -> Size<Pixels> {
		...
		let state = self.0.state.borrow();
		state.content_size().map(|size| size.div(state.scale_factor))
	}
	...
	fn resize(&mut self, size: Size<Pixels>) {
		...
		check_reply(
			...
			self.0
				.xcb
				.configure_window(self.0.x_window, &xproto::ConfigureWindowAux::new().width(width).height(height))
		)
		.log_err();
		xcb_flush(&self.0.xcb);
	}
	...
	fn mouse_position(&self) -> Point<Pixels> {
		get_reply(|| "X11 QueryPointer failed.", self.0.xcb.query_pointer(self.0.x_window))
			.log_err()
			.map_or(Point::new(Pixels::ZERO, Pixels::ZERO), |reply| Point::new((reply.root_x as u32).into(), (reply.root_y as u32).into()))
	}
	...
	fn activate(&self) {
		...
		let message = xproto::ClientMessageEvent::new(32, self.0.x_window, self.0.state.borrow().atoms._NET_ACTIVE_WINDOW, data);
		self.0
			.xcb
			.send_event(false, self.0.state.borrow().x_root_window, xproto::EventMask::SUBSTRUCTURE_REDIRECT | xproto::EventMask::SUBSTRUCTURE_NOTIFY, message)
			.log_err();
		self.0
			.xcb
			.set_input_focus(xproto::InputFocus::POINTER_ROOT, self.0.x_window, xproto::Time::CURRENT_TIME)
			.log_err();
		xcb_flush(&self.0.xcb);
	}
	...
	fn set_title(&mut self, title: &str) {
		check_reply(
			...
			self.0
				.xcb
				.change_property8(xproto::PropMode::REPLACE, self.0.x_window, xproto::AtomEnum::WM_NAME, xproto::AtomEnum::STRING, title.as_bytes())
		)
		.log_err();
	...
	fn set_app_id(&mut self, app_id: &str) {
		...
		check_reply(
			...
			self.0
				.xcb
				.change_property8(xproto::PropMode::REPLACE, self.0.x_window, xproto::AtomEnum::WM_CLASS, xproto::AtomEnum::STRING, &data)
		)
		.log_err();
	}
	...
	fn minimize(&self) {
		...
		let message = ClientMessageEvent::new(32, self.0.x_window, state.atoms.WM_CHANGE_STATE, [WINDOW_ICONIC_STATE, 0, 0, 0, 0]);
		check_reply(
	...
	fn show_window_menu(&self, position: Point<Pixels>) {
		...
		check_reply(|| "X11 UngrabPointer failed.", self.0.xcb.ungrab_pointer(x11rb::CURRENT_TIME)).log_err();
		...
		let message = ClientMessageEvent::new(
			...
			[XINPUT_ALL_DEVICE_GROUPS as u32, coords.dst_x as u32, coords.dst_y as u32, 0, 0]
		);
		check_reply(
