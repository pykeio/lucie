		...
		.map(|var| {
			if var.to_lowercase() == "randr" {
				...
				if valid_scale_factor(scale) {
					...
					panic!("`{}` must be a positive normal number or `randr`. Got `{}`", GPUI_X11_SCALE_FACTOR_ENV, var);
				}
			} else if var.is_empty() {
				DpiMode::NotSet
			} else {
				panic!("`{}` must be a positive number or `randr`. Got `{}`", GPUI_X11_SCALE_FACTOR_ENV, var);
			}
		})
	...
	match env_dpi {
		...
		DpiMode::Randr => {
			if let Some(scale) = get_randr_scale_factor(connection, screen_index) {
				log::info!("Using RandR scale factor from {}=randr: {}", GPUI_X11_SCALE_FACTOR_ENV, scale);
				return scale;
			}
