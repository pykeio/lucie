...
impl WindowsPlatform {
	...
	fn update_jump_list(&self, menus: Vec<MenuItem>, entries: Vec<SmallVec<[PathBuf; 2]>>) -> Vec<SmallVec<[PathBuf; 2]>> {
		let mut actions = Vec::new();
		menus.into_iter().for_each(|menu| {
			if let Some(dock_menu) = DockMenuItem::new(menu).log_err() {
				actions.push(dock_menu);
...
impl Platform for WindowsPlatform {
	...
	fn set_cursor_style(&self, style: CursorStyle) {
		...
		if self.inner.state.current_cursor.get().map(|c| c.0) != hcursor.map(|c| c.0) {
			self.post_message(WM_GPUI_CURSOR_STYLE_CHANGED, WPARAM(0), LPARAM(hcursor.map_or(0, |c| c.0 as isize)));
	...
	fn perform_dock_menu_action(&self, action: usize) {
		unsafe {
			PostMessageW(Some(self.handle), WM_GPUI_DOCK_MENU_ACTION, WPARAM(self.inner.validation_number), LPARAM(action as isize)).log_err();
		}
	}
		...
		'tasks: loop {
			'timeout_loop: loop {
				if start.elapsed().as_millis() >= MAIN_TASK_TIMEOUT {
					...
					// we spent our budget on gpui tasks, we likely have a lot of work queued so drain system events first to stay
					// responsive then quit out of foreground work to allow us to process other gpui events first before returning back
	...
	*directx_devices = try_to_recover_from_device_lost(|| DirectXDevices::new().context("Failed to recreate new DirectX devices after device lost"))?;
	...
	if let Some(all_windows) = all_windows.upgrade() {
		for window in all_windows.read().iter() {
			unsafe {
				SendMessageW(window.as_raw(), WM_GPUI_GPU_DEVICE_LOST, Some(WPARAM(validation_number)), Some(lparam));
			}
		}
		std::thread::sleep(std::time::Duration::from_millis(200));
		for window in all_windows.read().iter() {
			unsafe {
				SendMessageW(window.as_raw(), WM_GPUI_FORCE_UPDATE_WINDOW, Some(WPARAM(validation_number)), None);
			}
		}
	}
	Ok(())
