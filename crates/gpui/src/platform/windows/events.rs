...
impl WindowsWindowInner {
	...
	fn handle_destroy_msg(&self, handle: HWND) -> Option<isize> {
		...
		unsafe {
			PostMessageW(Some(self.platform_window_handle), WM_GPUI_CLOSE_ONE_WINDOW, WPARAM(self.validation_number), LPARAM(handle.0 as isize)).log_err();
		}
		Some(0)
	}
	...
	fn handle_input_language_changed(&self) -> Option<isize> {
		unsafe {
			PostMessageW(Some(self.platform_window_handle), WM_GPUI_KEYBOARD_LAYOUT_CHANGED, WPARAM(self.validation_number), LPARAM(0)).log_err();
		}
		Some(0)
	}
