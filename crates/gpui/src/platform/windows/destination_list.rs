	...
	pub(crate) fn new(item: MenuItem) -> anyhow::Result<Self> {
		match item {
			MenuItem::Action { name, action, .. } => Ok(Self {
				...
				description: if name == "New Window" { "Opens a new window".to_string() } else { name.into() },
				action
			}),
...
pub(crate) fn update_jump_list(jump_list: &JumpList) -> anyhow::Result<Vec<SmallVec<[PathBuf; 2]>>> {
...
fn add_recent_folders(list: &ICustomDestinationList, entries: &[SmallVec<[PathBuf; 2]>], removed: &Vec<SmallVec<[PathBuf; 2]>>) -> anyhow::Result<()> {
	unsafe {
		let tasks: IObjectCollection = CoCreateInstance(&EnumerableObjectCollection, None, CLSCTX_INPROC_SERVER)?;

		for folder_path in entries.iter().filter(|path| !removed.contains(path)) {
			let argument = HSTRING::from(folder_path.iter().map(|path| format!("\"{}\"", path.display())).join(" "));

			let description = HSTRING::from(folder_path.iter().map(|path| path.to_string_lossy()).collect::<Vec<_>>().join("\n"));
			...
			let display = folder_path
				.iter()
				.map(|p| p.file_name().map(|name| name.to_string_lossy()).unwrap_or_else(|| p.to_string_lossy()))
				.join(", ");
...
fn create_shell_link(argument: HSTRING, description: HSTRING, icon: Option<HSTRING>, display: &str) -> anyhow::Result<IShellLinkW> {
	unsafe {
