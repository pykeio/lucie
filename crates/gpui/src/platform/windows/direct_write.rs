...
use windows::{
	Win32::{
		...
		Graphics::{Direct3D::D3D_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP, Direct3D11::*, DirectWrite::*, Dxgi::Common::*, Gdi::LOGFONTW},
...
struct DirectWriteState {
	...
	fonts: Vec<FontInfo>,
	font_selections: RapidHashMap<Font, FontId>,
	font_id_by_identifier: RapidHashMap<FontIdentifier, FontId>
}
...
impl GPUState {
	fn new(directx_devices: &DirectXDevices) -> Result<Self> {
		...
		let vertex_shader = {
			let source = shader_resources::RawShaderBytes::new(shader_resources::ShaderModule::EmojiRasterization, shader_resources::ShaderTarget::Vertex)?;
			let mut shader = None;
		...
		let pixel_shader = {
			let source = shader_resources::RawShaderBytes::new(shader_resources::ShaderModule::EmojiRasterization, shader_resources::ShaderTarget::Fragment)?;
			let mut shader = None;
...
impl DirectWriteTextSystem {
	pub(crate) fn new(directx_devices: &DirectXDevices) -> Result<Self> {
		...
		Ok(Self(RwLock::new(DirectWriteState {
			...
			fonts: Vec::new(),
			font_selections: RapidHashMap::default(),
			font_id_by_identifier: RapidHashMap::default()
		})))
	}
...
impl PlatformTextSystem for DirectWriteTextSystem {
	...
	fn font_id(&self, font: &Font) -> Result<FontId> {
		...
		if let Some(font_id) = lock.font_selections.get(font) {
			...
			let font_id = lock.select_font(font).with_context(|| format!("Failed to select font: {:?}", font))?;
	...
	fn rasterize_glyph(&self, params: &RenderGlyphParams, raster_bounds: Bounds<DevicePixels>) -> anyhow::Result<(Size<DevicePixels>, Vec<u8>)> {
	...
	fn layout_line(&self, text: &str, font_size: Pixels, runs: &[FontRun]) -> LineLayout {
		self.0
			...
			.log_err()
			.unwrap_or(LineLayout { font_size, ..Default::default() })
	}
}
...
impl DirectWriteState {
		...
		unsafe {
			...
			for family_name in fallbacks.fallback_list() {
				let Some(fonts) = font_set
					.GetMatchingFonts(&HSTRING::from(family_name), DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STRETCH_NORMAL, DWRITE_FONT_STYLE_NORMAL)
					.log_err()
				else {
					continue;
				};
	...
	unsafe fn get_font_id_from_font_collection(
		...
		let collection = if is_system_font { &self.system_font_collection } else { &self.custom_font_collection };
		...
		for index in 0..total_number {
			...
			let fallbacks = font_fallbacks.and_then(|fallbacks| self.generate_font_fallbacks(fallbacks).log_err().flatten());
	...
	fn select_font(&mut self, target_font: &Font) -> Option<FontId> {
		unsafe {
			if target_font.family == ".SystemUIFont" {
				...
				self.find_font_id(family.as_ref(), target_font.weight, target_font.style, &target_font.features, target_font.fallbacks.as_ref())
			} else {
				let family = self.system_ui_font_name.clone();
				self.find_font_id(
	...
	unsafe fn find_font_id(
		...
		unsafe {
			self.get_font_id_from_font_collection(family_name, weight, style, features, fallbacks, false)
				.or_else(|| self.get_font_id_from_font_collection(family_name, weight, style, features, fallbacks, true))
				.or_else(|| {
	...
	fn layout_line(&mut self, text: &str, font_size: Pixels, font_runs: &[FontRun]) -> Result<LineLayout> {
		if font_runs.is_empty() {
			return Ok(LineLayout { font_size, ..Default::default() });
		}
		unsafe {
			...
			for run in font_runs {
				...
				let font_size = if break_ligatures { font_size.0.next_up() } else { font_size.0 };
	...
	fn rasterize_glyph(&self, params: &RenderGlyphParams, glyph_bounds: Bounds<DevicePixels>) -> Result<(Size<DevicePixels>, Vec<u8>)> {
		...
		let bitmap_data = if params.is_emoji {
			if let Ok(color) = self.rasterize_color(params, glyph_bounds) {
				...
				monochrome.into_iter().flat_map(|pixel| [0, 0, 0, pixel]).collect::<Vec<_>>()
			}
		} else {
	...
	fn rasterize_color(&self, params: &RenderGlyphParams, glyph_bounds: Bounds<DevicePixels>) -> Result<Vec<u8>> {
		...
		loop {
			...
			if image_format == DWRITE_GLYPH_IMAGE_FORMATS_COLR {
				...
				let color_size = size(color_bounds.right - color_bounds.left, color_bounds.bottom - color_bounds.top);
				if color_size.width > 0 && color_size.height > 0 {
					let mut alpha_data = vec![0u8; (color_size.width * color_size.height) as usize];
					unsafe { color_analysis.CreateAlphaTexture(DWRITE_TEXTURE_ALIASED_1x1, &color_bounds, &mut alpha_data) }?;
					...
					glyph_layers.push(GlyphLayerTexture::new(&self.components.gpu_state, run_color, bounds, &alpha_data)?);
				}
			}

			let has_next = unsafe { color_enumerator.MoveNext() }.map(|e| e.as_bool()).unwrap_or(false);
			if !has_next {
				break;
			}
		}
		...
		for layer in glyph_layers {
			...
			unsafe {
				...
				gpu_state
					.device_context
					.Map(params_buffer[0].as_ref().unwrap(), 0, D3D11_MAP_WRITE_DISCARD, 0, Some(&mut dest))?;
	...
	fn all_font_names(&self) -> Vec<String> {
		...
		result.extend(get_font_names_from_collection(&self.custom_font_collection, &self.components.locale));
		result
	}

	fn handle_gpu_lost(&mut self, directx_devices: &DirectXDevices) -> Result<()> {
		try_to_recover_from_device_lost(|| GPUState::new(directx_devices).context("Recreating GPU state for DirectWrite"))
...
impl Drop for DirectWriteState {
	fn drop(&mut self) {
		unsafe {
			let _ = self.components.factory.UnregisterFontFileLoader(&self.components.in_memory_loader);
		}
	}
}
...
impl GlyphLayerTexture {
	pub fn new(gpu_state: &GPUState, run_color: Rgba, bounds: Bounds<i32>, alpha_data: &[u8]) -> Result<Self> {
		...
		let texture_view = {
			...
			unsafe { gpu_state.device.CreateShaderResourceView(&texture, None, Some(&mut view))? };
			view.unwrap()
		};
		...
		unsafe {
			gpu_state
				.device_context
				.UpdateSubresource(&texture, 0, None, alpha_data.as_ptr() as _, texture_size.width as u32, 0)
		};
...
impl TextRenderer {
	pub fn new(locale_str: &str) -> Self {
		TextRenderer { locale: locale_str.to_owned() }
	}
}
...
impl IDWritePixelSnapping_Impl for TextRenderer_Impl {
	...
	fn GetCurrentTransform(&self, _clientdrawingcontext: *const ::core::ffi::c_void, transform: *mut DWRITE_MATRIX) -> windows::core::Result<()> {
		unsafe {
...
impl IDWriteTextRenderer_Impl for TextRenderer_Impl {
	fn DrawGlyphRun(
		...
		let Some((font_identifier, font_struct, color_font)) = get_font_identifier_and_font_struct(font_face, &self.locale) else {
			return Ok(());
		};
		...
		for (cluster_utf16_len, cluster_glyph_count) in cluster_analyzer {
			...
			for (cluster_glyph_idx, glyph_id) in glyph_ids[glyph_idx..(glyph_idx + cluster_glyph_count)].iter().enumerate() {
				...
				glyphs.push(ShapedGlyph {
					id,
					position: point(px(context.width + glyph_offsets[this_glyph_idx].advanceOffset), px(-glyph_offsets[this_glyph_idx].ascenderOffset)),
...
impl<'a> StringIndexConverter<'a> {
	fn new(text: &'a str) -> Self {
		Self { text, utf8_ix: 0, utf16_ix: 0 }
	}
...
fn get_font_identifier_and_font_struct(font_face: &IDWriteFontFace3, locale: &str) -> Option<(FontIdentifier, Font, bool)> {
...
fn get_font_identifier(font_face: &IDWriteFontFace3, locale: &str) -> Option<FontIdentifier> {
	...
	get_postscript_name(font_face, locale)
		.log_err()
		.map(|postscript_name| FontIdentifier { postscript_name, weight, style })
}
...
const fn make_direct_write_feature(feature_name: &str, parameter: u32) -> DWRITE_FONT_FEATURE {
	...
	DWRITE_FONT_FEATURE { nameTag: tag, parameter }
}
