...
use windows::Win32::UI::{
	Input::KeyboardAndMouse::{
		GetKeyboardLayoutNameW, MAPVK_VK_TO_CHAR, MAPVK_VK_TO_VSC, MapVirtualKeyW, ToUnicode, VIRTUAL_KEY, VK_0, VK_1, VK_2, VK_3, VK_4, VK_5, VK_6, VK_7,
		VK_8, VK_9, VK_ABNT_C1, VK_CONTROL, VK_MENU, VK_OEM_1, VK_OEM_2, VK_OEM_3, VK_OEM_4, VK_OEM_5, VK_OEM_6, VK_OEM_7, VK_OEM_8, VK_OEM_102, VK_OEM_COMMA,
		VK_OEM_MINUS, VK_OEM_PERIOD, VK_OEM_PLUS, VK_SHIFT
	},
...
pub(crate) struct WindowsKeyboardMapper {
	key_to_vkey: RapidHashMap<String, (u16, bool)>,
	vkey_to_key: RapidHashMap<u16, String>,
	vkey_to_shifted: RapidHashMap<u16, String>
}
...
impl PlatformKeyboardMapper for WindowsKeyboardMapper {
	fn map_key_equivalent(&self, mut keystroke: Keystroke, use_key_equivalents: bool) -> KeybindingKeystroke {
		...
		let modifiers = Modifiers { shift, ..keystroke.modifiers };
...
impl WindowsKeyboardLayout {
	pub(crate) fn new() -> Result<Self> {
		...
		let entry = windows_registry::LOCAL_MACHINE.open(format!("System\\CurrentControlSet\\Control\\Keyboard Layouts\\{id}"))?;
...
impl WindowsKeyboardMapper {
	pub(crate) fn new() -> Self {
		let mut key_to_vkey = RapidHashMap::default();
		let mut vkey_to_key = RapidHashMap::default();
		let mut vkey_to_shifted = RapidHashMap::default();
...
pub(crate) fn generate_key_char(vkey: VIRTUAL_KEY, scan_code: u32, control: bool, shift: bool, alt: bool) -> Option<String> {
	let mut state = [0; 256];
	if control {
