...
impl DisplayLink {
	pub fn new(display_id: CGDirectDisplayID, data: *mut c_void, callback: unsafe extern "C" fn(*mut c_void)) -> Result<DisplayLink> {
	...
	pub fn start(&mut self) -> Result<()> {
		unsafe {
			dispatch_resume(crate::dispatch_sys::dispatch_object_t { _ds: self.frame_requests });
	...
	pub fn stop(&mut self) -> Result<()> {
		unsafe {
			dispatch_suspend(crate::dispatch_sys::dispatch_object_t { _ds: self.frame_requests });
...
mod sys {
	...
	unsafe extern "C" {
		...
		pub fn CVDisplayLinkSetOutputCallback(display_link: &mut DisplayLinkRef, callback: CVDisplayLinkOutputCallback, user_info: *mut c_void) -> i32;
