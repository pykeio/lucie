use cocoa::{base::id, foundation::NSRange};
...
#[cfg(test)]
mod tests {
	use cocoa::{appkit::NSImage, base::nil, foundation::NSString};
	...
	fn test_nsattributed_string() {
		...
		unsafe {
			...
			let hello_attr_string = NSAttributedString::alloc(nil).init_attributed_string(hello_string);
			...
			let another_attr_string = NSAttributedString::alloc(nil).init_attributed_string(another_string);
			...
			let rtfd_data = attr_string.RTFDFromRange_documentAttributes_(NSRange::new(0, msg_send![attr_string, length]), nil);
