...
use mach2::{
	...
	thread_policy::{
		THREAD_EXTENDED_POLICY, THREAD_EXTENDED_POLICY_COUNT, THREAD_PRECEDENCE_POLICY, THREAD_PRECEDENCE_POLICY_COUNT, THREAD_TIME_CONSTRAINT_POLICY,
		THREAD_TIME_CONSTRAINT_POLICY_COUNT, thread_extended_policy_data_t, thread_precedence_policy_data_t, thread_time_constraint_policy_data_t
	}
};
use objc::{
	class, msg_send,
...
impl PlatformDispatcher for MacDispatcher {
	...
	fn dispatch(&self, runnable: RunnableVariant, _: Option<TaskLabel>, priority: Priority) {
		let (context, trampoline) = match runnable {
			RunnableVariant::Meta(runnable) => (runnable.into_raw().as_ptr() as *mut c_void, Some(trampoline as unsafe extern "C" fn(*mut c_void))),
			RunnableVariant::Compat(runnable) => (runnable.into_raw().as_ptr() as *mut c_void, Some(trampoline_compat as unsafe extern "C" fn(*mut c_void)))
		};
		...
		unsafe {
			dispatch_async_f(dispatch_get_global_queue(queue_priority, 0), context, trampoline);
		}
	}
...
fn set_audio_thread_priority() -> anyhow::Result<()> {
	...
	let result =
		unsafe { mach2::thread_policy::thread_policy_set(thread_id, THREAD_EXTENDED_POLICY, &mut policy as *mut _ as *mut _, THREAD_EXTENDED_POLICY_COUNT) };
	...
	let result = unsafe {
		mach2::thread_policy::thread_policy_set(thread_id, THREAD_PRECEDENCE_POLICY, &mut precedence as *mut _ as *mut _, THREAD_PRECEDENCE_POLICY_COUNT)
	};
