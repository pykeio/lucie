...
impl BladeAtlas {
	...
	pub fn get_texture_info(&self, id: AtlasTextureId) -> BladeTextureInfo {
		...
		BladeTextureInfo { raw_view: texture.raw_view }
	}
}
...
impl BladeAtlasState {
	...
	fn push_texture(&mut self, min_size: Size<DevicePixels>, kind: AtlasTextureKind) -> &mut BladeAtlasTexture {
		...
		for upload in self.uploads.drain(..) {
			...
			transfers.copy_buffer_to_texture(
				...
				gpu::TexturePiece {
					...
					mip_level: 0,
					array_layer: 0,
					origin: [upload.bounds.origin.x.into(), upload.bounds.origin.y.into(), 0]
				},
				gpu::Extent {
