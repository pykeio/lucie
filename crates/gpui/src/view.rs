...
struct AnyViewState {
	...
	accessed_entities: RapidHashSet<EntityId>
}
...
impl<V: Render> Element for Entity<V> {
	...
	fn prepaint(&mut self, _id: Option<&GlobalElementId>, _: Bounds<Pixels>, element: &mut Self::RequestLayoutState, window: &mut Window, cx: &mut App) {
...
impl AnyView {
	...
	/// [Context::notify] has not been called since it was rendered. The one exception is when [Window::refresh] is
...
impl Element for AnyView {
	...
	fn request_layout(&mut self, _id: Option<&GlobalElementId>, window: &mut Window, cx: &mut App) -> (LayoutId, Self::RequestLayoutState) {
		window.with_rendered_view(self.entity_id(), |window| match self.cached_style.as_ref() {
			Some(style) => {
			...
			window.with_element_state::<AnyViewState, _>(global_id.unwrap(), |element_state, window| {
				...
				(
					...
					AnyViewState {
						...
						cache_key: ViewCacheKey { bounds, content_mask, text_style }
					}
				)
			})
		})
