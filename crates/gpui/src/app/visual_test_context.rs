...
use crate::{
	...
	TextSystem, Window, WindowBounds, WindowHandle, WindowOptions, app::GpuiMode, current_platform, http::FakeHttpClient
};
...
/// `VisualTestAppContext` uses the real OS `Platform` to produce actual rendered output.
///
...
impl VisualTestAppContext {
	...
	///
	/// This initializes the real platform (not the test platform), which means:
	/// - Windows are actually rendered by the compositor
	...
	pub fn simulate_keystrokes(&mut self, window: AnyWindowHandle, keystrokes: &str) {
		for keystroke_text in keystrokes.split_whitespace() {
			let keystroke = Keystroke::parse(keystroke_text).unwrap_or_else(|_| panic!("Invalid keystroke: {}", keystroke_text));
	...
	pub fn simulate_mouse_move(&mut self, window: AnyWindowHandle, position: Point<Pixels>, button: impl Into<Option<MouseButton>>, modifiers: Modifiers) {
		self.simulate_event(
			window,
			MouseMoveEvent {
				position,
	...
	pub fn simulate_mouse_down(&mut self, window: AnyWindowHandle, position: Point<Pixels>, button: MouseButton, modifiers: Modifiers) {
		self.simulate_event(
			window,
			MouseDownEvent {
				position,
	...
	pub fn simulate_mouse_up(&mut self, window: AnyWindowHandle, position: Point<Pixels>, button: MouseButton, modifiers: Modifiers) {
		self.simulate_event(
			window,
			MouseUpEvent {
				position,
	...
	pub fn simulate_click(&mut self, window: AnyWindowHandle, position: Point<Pixels>, modifiers: Modifiers) {
	...
	pub async fn wait_for<T: 'static>(&mut self, entity: &Entity<T>, predicate: impl Fn(&T) -> bool, timeout: Duration) -> Result<()> {
