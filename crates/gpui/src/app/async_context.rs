...
/// Internally, this holds a weak reference to an `App`, so its methods are fallible to protect against cases where the
/// [App] is dropped.
#[derive(Clone)]
pub struct AsyncApp {
...
impl AppContext for AsyncApp {
	...
	fn insert_entity<T: 'static>(&mut self, reservation: Reservation<T>, build_entity: impl FnOnce(&mut Context<T>) -> T) -> Result<Entity<T>> {
		let app = self.app.upgrade().context("app was released")?;
		let mut app = app.borrow_mut();
	...
	where
		T: 'static
	{
		Err(anyhow!("Cannot as_mut with an async context. Try calling update() first"))
	}
...
impl AsyncApp {
	...
	pub fn subscribe<T, Event>(&mut self, entity: &Entity<T>, mut on_event: impl FnMut(Entity<T>, &Event, &mut App) + 'static) -> Result<Subscription>
	where
	...
	pub fn open_window<V>(&self, options: crate::WindowOptions, build_root_view: impl FnOnce(&mut Window, &mut App) -> Entity<V>) -> Result<WindowHandle<V>>
	where
		V: 'static + Render
	{
		let app = self.app.upgrade().context("app was released")?;
...
impl AsyncWindowContext {
	...
	pub fn on_next_frame(&mut self, f: impl FnOnce(&mut Window, &mut App) + 'static) {
		self.app.update_window(self.window, |_, window, _| window.on_next_frame(f)).ok();
	}

	/// A convenience method for [`App::global`].
	pub fn read_global<G: Global, R>(&mut self, read: impl FnOnce(&G, &Window, &App) -> R) -> Result<R> {
		self.app.update_window(self.window, |_, window, cx| read(cx.global(), window, cx))
	}
	...
	{
		self.app
			.update_window(self.window, |_, window, cx| cx.update_global(|global, cx| update(global, window, cx)))
	}
	...
	pub fn prompt<T>(&mut self, level: PromptLevel, message: &str, detail: Option<&str>, answers: &[T]) -> oneshot::Receiver<usize>
	where
		T: Clone + Into<PromptButton>
	{
		self.app
			.update_window(self.window, |_, window, cx| window.prompt(level, message, detail, answers, cx))
...
impl AppContext for AsyncWindowContext {
	...
	fn update_entity<T: 'static, R>(&mut self, handle: &Entity<T>, update: impl FnOnce(&mut T, &mut Context<T>) -> R) -> Result<R> {
		self.app.update_window(self.window, |_, _, cx| cx.update_entity(handle, update))
	}
...
impl VisualContext for AsyncWindowContext {
	...
	fn new_window_entity<T: 'static>(&mut self, build_entity: impl FnOnce(&mut Window, &mut Context<T>) -> T) -> Self::Result<Entity<T>> {
		self.app.update_window(self.window, |_, window, cx| cx.new(|cx| build_entity(window, cx)))
	}

	fn update_window_entity<T: 'static, R>(&mut self, view: &Entity<T>, update: impl FnOnce(&mut T, &mut Window, &mut Context<T>) -> R) -> Self::Result<R> {
		self.app
			.update_window(self.window, |_, window, cx| view.update(cx, |entity, cx| update(entity, window, cx)))
	}

	fn replace_root_view<V>(&mut self, build_view: impl FnOnce(&mut Window, &mut Context<V>) -> V) -> Self::Result<Entity<V>>
	where
		V: 'static + Render
	{
		self.app.update_window(self.window, |_, window, cx| window.replace_root(cx, build_view))
	}
