...
impl<T> PriorityQueueState<T> {
	...
	fn try_recv<'a>(&'a self) -> Result<Option<parking_lot::MutexGuard<'a, PriorityQueues<T>>>, RecvError> {
		let mut queues = self.queues.lock();
		...
		if queues.is_empty() { Ok(None) } else { Ok(Some(queues)) }
	}
}
...
impl<T> Drop for PriorityQueueSender<T> {
	fn drop(&mut self) {
		self.state.sender_count.fetch_sub(1, std::sync::atomic::Ordering::AcqRel);
	}
}
...
impl<T> PriorityQueueReceiver<T> {
	...
	pub(crate) fn try_iter(self) -> TryIter<T> {
		TryIter { receiver: self, ended: false }
	}
...
impl<T> Drop for PriorityQueueReceiver<T> {
	fn drop(&mut self) {
		self.state.receiver_count.fetch_sub(1, std::sync::atomic::Ordering::AcqRel);
	}
}
