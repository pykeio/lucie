...
impl LineLayout {
	...
	fn compute_wrap_boundaries(&self, text: &str, wrap_width: Pixels, max_lines: Option<usize>) -> SmallVec<[WrapBoundary; 1]> {
...
#[derive(Default)]
struct FrameCache {
	lines: RapidHashMap<Arc<CacheKey>, Arc<LineLayout>>,
	wrapped_lines: RapidHashMap<Arc<CacheKey>, Arc<WrappedLineLayout>>,
...
impl LineLayoutCache {
	...
	pub fn layout_line<Text>(&self, text: Text, font_size: Pixels, runs: &[FontRun], force_width: Option<Pixels>) -> Arc<LineLayout>
	where
		Text: AsRef<str>,
		SharedString: From<Text>
	{
