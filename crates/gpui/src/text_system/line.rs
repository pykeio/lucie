...
use crate::{
	App, Bounds, Half, Hsla, LineLayout, Pixels, Point, Result, SharedString, StrikethroughStyle, TextAlign, UnderlineStyle, Window, WrapBoundary,
		...
		for (run_ix, run) in layout.runs.iter().enumerate() {
			...
			for (glyph_ix, glyph) in run.glyphs.iter().enumerate() {
				...
				if wraps.peek() == Some(&&WrapBoundary { run_ix, glyph_ix }) {
					...
					glyph_origin.x = aligned_origin_x(origin, align_width.unwrap_or(layout.width), glyph.position.x, &align, layout, wraps.peek());
					...
					if let Some(style_run) = style_run {
						...
						if let Some(run_underline) = style_run.underline.as_ref() {
							current_underline.get_or_insert((
								point(glyph_origin.x, glyph_origin.y + baseline_offset.y + (layout.descent * 0.618)),
								UnderlineStyle {
						...
						if let Some(run_strikethrough) = style_run.strikethrough.as_ref() {
							current_strikethrough.get_or_insert((
								point(glyph_origin.x, glyph_origin.y + (((layout.ascent * 0.5) + baseline_offset.y) * 0.5)),
								StrikethroughStyle {
