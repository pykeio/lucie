...
pub struct Keymap {
	...
	binding_indices_by_action_id: RapidHashMap<TypeId, SmallVec<[usize; 3]>>,
...
impl Keymap {
	...
	pub fn add_bindings<T: IntoIterator<Item = KeyBinding>>(&mut self, bindings: T) {
		for binding in bindings {
			...
			if is_no_action(&*binding.action) {
				...
				self.binding_indices_by_action_id.entry(action_id).or_default().push(self.bindings.len());
			}
	...
	pub fn bindings_for_input(&self, input: &[impl AsKeystroke], context_stack: &[KeyContext]) -> (SmallVec<[KeyBinding; 1]>, bool) {
		...
		let mut pending = RapidHashSet::default();
	...
	pub fn possible_next_bindings_for_input(&self, input: &[Keystroke], context_stack: &[KeyContext]) -> Vec<KeyBinding> {
		...
		bindings.into_iter().map(|(_, _, binding)| binding.clone()).collect::<Vec<_>>()
	}
}
...
mod tests {
	...
	fn test_keymap() {
		...
		assert_eq!(keymap.binding_enabled(&bindings[0], &[KeyContext::parse("terminal").unwrap()]), Some(1));

		// contextual bindings are enabled in contexts that match their predicate
		assert_eq!(keymap.binding_enabled(&bindings[1], &[KeyContext::parse("barf x=y").unwrap()]), None);
		assert_eq!(keymap.binding_enabled(&bindings[1], &[KeyContext::parse("pane x=y").unwrap()]), Some(1));

		assert_eq!(keymap.binding_enabled(&bindings[2], &[KeyContext::parse("editor").unwrap()]), None);
		assert_eq!(keymap.binding_enabled(&bindings[2], &[KeyContext::parse("editor mode=full").unwrap()]), Some(1));
	}

	#[test]
	fn test_depth_precedence() {
		let bindings = [KeyBinding::new("ctrl-a", ActionBeta {}, Some("pane")), KeyBinding::new("ctrl-a", ActionGamma {}, Some("editor"))];

		let mut keymap = Keymap::default();
		keymap.add_bindings(bindings);

		let (result, pending) =
			keymap.bindings_for_input(&[Keystroke::parse("ctrl-a").unwrap()], &[KeyContext::parse("pane").unwrap(), KeyContext::parse("editor").unwrap()]);
	...
	fn test_keymap_disabled() {
		...
		assert!(
			keymap
				.bindings_for_input(&[Keystroke::parse("ctrl-a").unwrap()], &[KeyContext::parse("barf").unwrap()],)
				.0
				.is_empty()
		);
		assert!(
			!keymap
				.bindings_for_input(&[Keystroke::parse("ctrl-a").unwrap()], &[KeyContext::parse("editor").unwrap()],)
				.0
				.is_empty()
		);
		...
		assert!(
			keymap
				.bindings_for_input(&[Keystroke::parse("ctrl-a").unwrap()], &[KeyContext::parse("editor mode=full").unwrap()],)
				.0
				.is_empty()
		);
		...
		assert!(
			keymap
				.bindings_for_input(&[Keystroke::parse("ctrl-b").unwrap()], &[KeyContext::parse("barf").unwrap()],)
				.0
				.is_empty()
		);
	}
	...
	fn test_multiple_keystroke_binding_disabled() {
		...
		let editor_workspace_context = || [KeyContext::parse("workspace").unwrap(), KeyContext::parse("editor").unwrap()];
	...
	fn test_override_multikey() {
		let bindings = [KeyBinding::new("ctrl-w left", ActionAlpha {}, Some("editor")), KeyBinding::new("ctrl-w", NoAction {}, Some("editor"))];

		let mut keymap = Keymap::default();
		keymap.add_bindings(bindings);

		// Ensure `space` results in pending input on the workspace, but not editor
		let (result, pending) = keymap.bindings_for_input(&[Keystroke::parse("ctrl-w").unwrap()], &[KeyContext::parse("editor").unwrap()]);
		assert!(result.is_empty());
		assert!(pending);

		let bindings = [KeyBinding::new("ctrl-w left", ActionAlpha {}, Some("editor")), KeyBinding::new("ctrl-w", ActionBeta {}, Some("editor"))];
	...
	fn test_simple_disable() {
		let bindings = [KeyBinding::new("ctrl-x", ActionAlpha {}, Some("editor")), KeyBinding::new("ctrl-x", NoAction {}, Some("editor"))];

		let mut keymap = Keymap::default();
		keymap.add_bindings(bindings);

		// Ensure `space` results in pending input on the workspace, but not editor
		let (result, pending) = keymap.bindings_for_input(&[Keystroke::parse("ctrl-x").unwrap()], &[KeyContext::parse("editor").unwrap()]);
		assert!(result.is_empty());
		assert!(!pending);
	}
	...
	fn test_fail_to_disable() {
		...
		let bindings = [KeyBinding::new("ctrl-x", ActionAlpha {}, Some("editor")), KeyBinding::new("ctrl-x", NoAction {}, Some("workspace"))];
		...
		// Ensure `space` results in pending input on the workspace, but not editor
		let (result, pending) =
			keymap.bindings_for_input(&[Keystroke::parse("ctrl-x").unwrap()], &[KeyContext::parse("workspace").unwrap(), KeyContext::parse("editor").unwrap()]);
		assert_eq!(result.len(), 1);
		assert!(!pending);
	}
	...
	fn test_disable_deeper() {
		let bindings = [KeyBinding::new("ctrl-x", ActionAlpha {}, Some("workspace")), KeyBinding::new("ctrl-x", NoAction {}, Some("editor"))];
	...
	fn test_pending_match_enabled() {
		...
		let matched = keymap.bindings_for_input(
			...
			&[KeyContext::parse("Workspace"), KeyContext::parse("Pane"), KeyContext::parse("Editor vim_mode=normal")].map(Result::unwrap)
		);
	...
	fn test_context_precedence_with_same_source() {
		...
		let bindings = [KeyBinding::new("cmd-r", ActionAlpha {}, Some("Workspace")), KeyBinding::new("cmd-r", ActionBeta {}, Some("Editor"))];
		...
		let (result, _) =
			keymap.bindings_for_input(&[Keystroke::parse("cmd-r").unwrap()], &[KeyContext::parse("Workspace").unwrap(), KeyContext::parse("Editor").unwrap()]);
	...
	fn test_source_precedence_sorting() {
		...
		let (result, _) = keymap.bindings_for_input(&[Keystroke::parse("cmd-r").unwrap()], &[KeyContext::parse("Editor").unwrap()]);
