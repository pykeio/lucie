...
use crate::{
	...
	IntoElement, LayoutId, ParentElement, Pixels, Refineable, RenderImage, Resource, Style, StyleRefinement, Styled, Task, Window, hash
};
...
pub struct AnyImageCache {
	...
	load_fn: fn(image_cache: &AnyEntity, resource: &Resource, window: &mut Window, cx: &mut App) -> Option<Result<Arc<RenderImage>, ImageCacheError>>
}
...
impl AnyImageCache {
	...
	pub fn load(&self, resource: &Resource, window: &mut Window, cx: &mut App) -> Option<Result<Arc<RenderImage>, ImageCacheError>> {
...
impl fmt::Debug for RetainAllImageCache {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("HashMapImageCache").field("num_images", &self.0.len()).finish()
	}
}
...
impl RetainAllImageCache {
	...
	pub fn load(&mut self, source: &Resource, window: &mut Window, cx: &mut App) -> Option<Result<Arc<RenderImage>, ImageCacheError>> {
