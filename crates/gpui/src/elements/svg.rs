...
impl Element for Svg {
	...
	fn request_layout(&mut self, global_id: Option<&GlobalElementId>, window: &mut Window, cx: &mut App) -> (LayoutId, Self::RequestLayoutState) {
		let layout_id = self
			.interactivity
			.request_layout(global_id, window, cx, |style, window, cx| window.request_layout(style, None, cx));
		(layout_id, ())
	}
	...
	fn prepaint(
		...
		self.interactivity
			.prepaint(global_id, bounds, bounds.size, window, cx, |_, _, hitbox, _, _| hitbox)
	}
