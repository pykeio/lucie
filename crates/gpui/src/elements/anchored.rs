...
impl Element for Anchored {
	...
	fn prepaint(
		...
		let (origin, mut desired) = self
			.position_mode
			.get_position_and_bounds(self.anchor_position, self.anchor_corner, size, bounds, self.offset);
		...
		if self.fit_mode == AnchoredFitMode::SwitchAnchor {
			...
			if desired.left() < limits.left() || desired.right() > limits.right() {
				let switched = Bounds::from_corner_and_size(anchor_corner.other_side_corner_along(Axis::Horizontal), origin, size);
			...
			if desired.top() < limits.top() || desired.bottom() > limits.bottom() {
				let switched = Bounds::from_corner_and_size(anchor_corner.other_side_corner_along(Axis::Vertical), origin, size);
	...
	fn get_position_and_bounds(
		...
		match self {
			...
			AnchoredPositionMode::Local => {
				...
				let bounds = Bounds::from_corner_and_size(anchor_corner, bounds.origin + anchor_position + offset, size);
				(anchor_position, bounds)
			}
		}
	}
