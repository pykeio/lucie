use gpui::{App, Application, Bounds, Context, Hsla, Window, WindowBounds, WindowOptions, div, prelude::*, px, rgb, size};
...
impl Render for HolyGrailExample {
	fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
		...
		div()
			...
			.child(block(gpui::blue()).col_span(1).row_span(3).child("AD :(").text_color(gpui::white()))
			.child(block(gpui::black()).row_span(1).col_span_full().text_color(gpui::white()).child("Footer"))
	}
}
