use gpui::{
	App, Application, BoxShadow, Context, Div, SharedString, Window, WindowBounds, WindowOptions, div, hsla, point, prelude::*, px, relative, rgb, size
};
...
impl Shadow {
	...
	fn square() -> Div {
		div().size_16().bg(rgb(0xffffff)).border_1().border_color(hsla(0.0, 0.0, 0.0, 0.1))
	}
...
fn example(label: impl Into<SharedString>, example: impl IntoElement) -> impl IntoElement {
	...
	div()
		...
		.child(div().flex().items_center().justify_center().flex_1().py_12().child(example))
		.child(
			div()
				.w_full()
				.border_t_1()
	...
	fn render(&mut self, _window: &mut Window, _cx: &mut Context<Self>) -> impl IntoElement {
		div()
			...
			.child(div().flex().flex_col().w_full().children(vec![
				div().border_b_1().border_color(hsla(0.0, 0.0, 0.0, 1.0)).flex().flex_row().children(vec![
					example(
						"Square",
					...
					example(
						"Rounded 4",
						Shadow::rounded_small().shadow(vec![BoxShadow {
							color: hsla(0.0, 0.5, 0.5, 0.3),
							offset: point(px(0.), px(8.)),
							blur_radius: px(8.),
							spread_radius: px(0.)
					...
					example(
						"Rounded 8",
						Shadow::rounded_medium().shadow(vec![BoxShadow {
							color: hsla(0.0, 0.5, 0.5, 0.3),
							offset: point(px(0.), px(8.)),
							blur_radius: px(8.),
							spread_radius: px(0.)
				...
				div().border_b_1().border_color(hsla(0.0, 0.0, 0.0, 1.0)).flex().w_full().children(vec![
				...
				div().border_b_1().border_color(hsla(0.0, 0.0, 0.0, 1.0)).flex().children(vec![
					example(
						"Blur 0",
						Shadow::base().shadow(vec![BoxShadow {
...
fn main() {
	Application::new().run(|cx: &mut App| {
		cx.open_window(
			WindowOptions {
				window_bounds: Some(WindowBounds::centered(size(px(1000.0), px(800.0)), cx)),
				..Default::default()
			},
