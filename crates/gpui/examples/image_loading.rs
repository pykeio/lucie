...
impl ImageLoadingExample {
	fn loading_element() -> impl IntoElement {
		div().size_full().flex_none().p_0p5().rounded_xs().child(div().size_full().with_animation(
			"loading-bg",
			Animation::new(Duration::from_secs(3)).repeat().with_easing(pulsating_between(0.04, 0.24)),
			...
			div().flex().flex_row().w_full().justify_around().child(
				div()
					...
					.child({
						...
						img(move |window: &mut Window, cx: &mut App| window.use_asset::<LoadImageWithParameters>(&image_source, cx))
							.id("image-1")
							.border_1()
							.size_12()
					...
					.child({
						...
						let image_source = Path::new("this/file/really/shouldn't/exist/or/won't/be/an/image/I/hope").to_path_buf();
