...
use gpui::{App, Application, Context, FocusHandle, Focusable, Render, Task, Window, WindowBounds, WindowOptions, actions, div, prelude::*, px, rgb, size};
...
fn main() {
	Application::new().run(|cx: &mut App| {
		cx.bind_keys([gpui::KeyBinding::new("up", Increment, Some("Counter")), gpui::KeyBinding::new("down", Decrement, Some("Counter"))]);
	...
	mod distributed_systems {
		...
		impl MockNetwork {
			...
			fn a_client(network: &Arc<Mutex<Self>>) -> NetworkClient {
				NetworkClient { network: network.clone(), is_a: true }
			}
		...
		impl NetworkedCounter {
			...
			fn increment_tracked(&mut self, delta: i32, cx: &mut Context<Self>, order: Arc<Mutex<Vec<i32>>>) {
				self.count += delta;
		...
		fn test_random_interleaving(cx_a: &mut TestAppContext, cx_b: &mut TestAppContext, mut rng: StdRng) {
			...
			for i in 0..num_operations {
				...
				if which {
					b.update(cx_b, |b, cx| b.increment_tracked(id, cx, actual_order.clone()));
				} else {
					a.update(cx_a, |a, cx| a.increment_tracked(id, cx, actual_order.clone()));
				}
			}
