...
pub mod core_media {
	...
	pub use crate::bindings::{
		CMItemIndex, CMSampleTimingInfo, CMTime, CMTimeMake, CMVideoCodecType, kCMSampleAttachmentKey_NotSync, kCMTimeInvalid, kCMVideoCodecType_H264
	};
	...
	impl CMSampleBuffer {
		pub fn attachments(&self) -> Vec<CFDictionary<CFString>> {
			unsafe {
				let attachments = CMSampleBufferGetSampleAttachmentsArray(self.as_concrete_TypeRef(), true);
				CFArray::<CFDictionary>::wrap_under_get_rule(attachments)
					.into_iter()
					.map(|attachments| CFDictionary::wrap_under_get_rule(attachments.as_concrete_TypeRef()))
					.collect()
			}
		}
		...
		pub fn image_buffer(&self) -> Option<CVImageBuffer> {
			unsafe {
				...
				if ptr.is_null() { None } else { Some(CVImageBuffer::wrap_under_get_rule(ptr)) }
			}
		}
		...
		pub fn sample_timing_info(&self, index: usize) -> Result<CMSampleTimingInfo> {
			unsafe {
				...
				let result = CMSampleBufferGetSampleTimingInfo(self.as_concrete_TypeRef(), index as CMItemIndex, &mut timing_info);
		...
		pub fn format_description(&self) -> CMFormatDescription {
			unsafe { CMFormatDescription::wrap_under_get_rule(CMSampleBufferGetFormatDescription(self.as_concrete_TypeRef())) }
		}

		pub fn data(&self) -> CMBlockBuffer {
			unsafe { CMBlockBuffer::wrap_under_get_rule(CMSampleBufferGetDataBuffer(self.as_concrete_TypeRef())) }
		}
	}
	...
	unsafe extern "C" {
		...
		fn CMSampleBufferGetSampleAttachmentsArray(buffer: CMSampleBufferRef, create_if_necessary: bool) -> CFArrayRef;
		fn CMSampleBufferGetImageBuffer(buffer: CMSampleBufferRef) -> CVImageBufferRef;
		fn CMSampleBufferGetSampleTimingInfo(buffer: CMSampleBufferRef, index: CMItemIndex, timing_info_out: *mut CMSampleTimingInfo) -> OSStatus;
	...
	impl CMBlockBuffer {
		pub fn bytes(&self) -> &[u8] {
			unsafe {
				let mut bytes = ptr::null();
				let mut len = 0;
				let result = CMBlockBufferGetDataPointer(self.as_concrete_TypeRef(), 0, &mut 0, &mut len, &mut bytes);
...
pub mod core_video {
	...
	use anyhow::Result;
	use core_foundation::{base::kCFAllocatorDefault, dictionary::CFDictionaryRef, mach_port::CFAllocatorRef};
	#[cfg(target_os = "macos")]
	use core_foundation::{
	...
	pub use crate::bindings::{
		kCVPixelFormatType_32BGRA, kCVPixelFormatType_420YpCbCr8BiPlanarFullRange, kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange,
	...
	impl CVMetalTextureCache {
		...
		pub unsafe fn new(metal_device: *mut MTLDevice) -> Result<Self> {
			...
			let result = unsafe { CVMetalTextureCacheCreate(kCFAllocatorDefault, ptr::null(), metal_device, ptr::null(), &mut this) };
			anyhow::ensure!(result == kCVReturnSuccess, "could not create texture cache, code: {result}");
		...
		pub unsafe fn create_texture_from_image(
			...
			anyhow::ensure!(result == kCVReturnSuccess, "could not create texture, code: {result}");
