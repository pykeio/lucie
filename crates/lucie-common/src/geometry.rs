...
impl<T: Clone + fmt::Debug + Default + PartialEq> Along for Point<T> {
	type Unit = T;
...
impl<T: Clone + fmt::Debug + Default + PartialEq + Negate> Negate for Point<T> {
	fn negate(self) -> Self {
...
where
	T: Mul<S, Output = T> + Clone + fmt::Debug + Default + PartialEq,
	S: Clone
{
...
where
	T: Div<S, Output = T> + Clone + fmt::Debug + Default + PartialEq,
	S: Clone
{
	type Output = Self;
	...
	T: PartialOrd + Clone + fmt::Debug + Default + PartialEq
{
	...
	pub fn max(&self, other: &Self) -> Self {
		Point {
			x: if self.x > other.x { self.x.clone() } else { other.x.clone() },
			y: if self.y > other.y { self.y.clone() } else { other.y.clone() }
		}
	}
...
impl<T: Clone + fmt::Debug + Default + PartialEq> Clone for Point<T> {
	fn clone(&self) -> Self {
...
impl<T> Size<T>
where
	T: Clone + fmt::Debug + Default + PartialEq + Half
{
	...
	T: PartialOrd + Clone + fmt::Debug + Default + PartialEq
{
	...
	pub fn max(&self, other: &Self) -> Self {
		Size {
			width: if self.width >= other.width { self.width.clone() } else { other.width.clone() },
			height: if self.height >= other.height { self.height.clone() } else { other.height.clone() }
		}
	}
	...
	pub fn min(&self, other: &Self) -> Self {
		Size {
			width: if self.width >= other.width { other.width.clone() } else { self.width.clone() },
			height: if self.height >= other.height { other.height.clone() } else { self.height.clone() }
		}
	}
}
...
where
	T: Sub<Output = T> + Clone + fmt::Debug + Default + PartialEq
{
	type Output = Size<T>;
...
where
	T: Add<Output = T> + Clone + fmt::Debug + Default + PartialEq
{
	type Output = Size<T>;
...
where
	T: Mul<Rhs, Output = Rhs> + Clone + fmt::Debug + Default + PartialEq,
...
impl<T> Eq for Size<T> where T: Eq + Clone + fmt::Debug + Default + PartialEq {}
...
impl Size<Length> {
	...
	/// `Length::Definite(DefiniteLength::Fraction(1.0))`, which represents 100% of the available space in both
	/// dimensions.
	///
	/// # Returns
	///
...
impl<T> Bounds<T>
where
	T: Sub<T, Output = T> + Half + Clone + fmt::Debug + Default + PartialEq
{
...
impl<T> Bounds<T>
where
	T: PartialOrd + Add<T, Output = T> + Clone + fmt::Debug + Default + PartialEq
{
	...
	pub fn intersects(&self, other: &Bounds<T>) -> bool {
		...
		self.origin.x < their_lower_right.x && my_lower_right.x > other.origin.x && self.origin.y < their_lower_right.y && my_lower_right.y > other.origin.y
	}
}

impl<T> Bounds<T>
where
	T: Add<T, Output = T> + Half + Clone + fmt::Debug + Default + PartialEq
{
...
impl<T> Bounds<T>
where
	T: Add<T, Output = T> + Clone + fmt::Debug + Default + PartialEq
{
...
impl<T> Bounds<T>
where
	T: Add<T, Output = T> + Sub<Output = T> + Clone + fmt::Debug + Default + PartialEq
{
	...
	pub fn extend(&self, amount: Edges<T>) -> Bounds<T> {
		Bounds {
			...
			size: self.size.clone() + size(amount.left.clone() + amount.right.clone(), amount.top.clone() + amount.bottom)
		}
	}
}
...
impl<T> Bounds<T>
where
	T: Add<T, Output = T> + Sub<T, Output = T> + Neg<Output = T> + Clone + fmt::Debug + Default + PartialEq
{
...
impl<T: PartialOrd + Add<T, Output = T> + Sub<Output = T> + Clone + fmt::Debug + Default + PartialEq> Bounds<T> {
...
impl<T> Bounds<T>
where
	T: Add<T, Output = T> + Sub<T, Output = T> + Clone + fmt::Debug + Default + PartialEq
{
...
impl<T> Bounds<T>
where
	T: Add<T, Output = T> + PartialOrd + Clone + fmt::Debug + Default + PartialEq
{
...
impl<T> Bounds<T>
where
	T: Add<T, Output = T> + Sub<T, Output = T> + PartialOrd + Clone + fmt::Debug + Default + PartialEq
{
...
/// # Returns
///
/// Returns `true` if either the width or the height of the bounds is less than or equal to zero, indicating an empty
/// area.
impl<T: PartialOrd + Clone + fmt::Debug + Default + PartialEq> Bounds<T> {
	/// Checks if the bounds represent an empty area.
	///
	/// # Returns
	///
	/// Returns `true` if either the width or the height of the bounds is less than or equal to zero, indicating an
	/// empty area.
	#[must_use]
...
impl<T: Clone + fmt::Debug + Default + PartialEq + fmt::Display + Add<T, Output = T>> fmt::Display for Bounds<T> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
impl Size<DevicePixels> {
	...
	pub fn to_pixels(self, scale_factor: f32) -> Size<Pixels> {
		size(px(self.width.0 as f32 / scale_factor), px(self.height.0 as f32 / scale_factor))
	}
}
...
impl Size<Pixels> {
	...
	pub fn to_device_pixels(self, scale_factor: f32) -> Size<DevicePixels> {
		size(DevicePixels((self.width.0 * scale_factor).round() as i32), DevicePixels((self.height.0 * scale_factor).round() as i32))
	}
}
...
impl Bounds<Pixels> {
	...
	pub fn to_device_pixels(self, factor: f32) -> Bounds<DevicePixels> {
		Bounds {
			origin: point(DevicePixels((self.origin.x.0 * factor).round() as i32), DevicePixels((self.origin.y.0 * factor).round() as i32)),
...
impl Bounds<DevicePixels> {
	...
	pub fn to_pixels(self, scale_factor: f32) -> Bounds<Pixels> {
		Bounds {
			origin: point(px(self.origin.x.0 as f32 / scale_factor), px(self.origin.y.0 as f32 / scale_factor)),
...
where
	T: Mul<Output = T> + Clone + fmt::Debug + Default + PartialEq
{
	type Output = Self;
...
impl<T: Clone + fmt::Debug + Default + PartialEq> Edges<T> {
	...
	///
	/// This method applies a predicate function to each field of the `Edges` and returns `true` if any field satisfies
	/// the predicate.
	///
	/// # Arguments
	///
...
impl Edges<Length> {
	/// Sets the edges of the `Edges` struct to `auto`, which is a special value that allows the layout engine to
	/// automatically determine the size of the edges.
	///
	/// This is typically used in layout contexts where the exact size of the edges is not important, or when the size
	/// should be calculated based on the content or container.
	///
	/// # Returns
	///
...
impl Edges<DefiniteLength> {
	...
	/// the size of the parent element (for percentage-based lengths) and the size of a rem unit (for rem-based
	/// lengths).
	///
	/// # Arguments
	///
...
impl Edges<Pixels> {
	...
	/// # Returns
	///
	/// Returns a new `Edges<ScaledPixels>` where each edge is the result of scaling the original edge by the given
	/// factor.
	///
	/// # Examples
	///
...
///
/// Each field represents the size of the corner on one side of the box: `top_left`, `top_right`, `bottom_right`, and
/// `bottom_left`.
#[derive(Refineable, Clone, Default, Debug, Eq, PartialEq)]
#[refineable(Debug, PartialEq)]
#[repr(C)]
	...
	T: Clone + Copy + fmt::Debug + Default + PartialEq
{
	...
	///
	/// This function creates a `Corners` instance with the `top_left`, `top_right`, `bottom_right`, and `bottom_left`
	/// fields all initialized to the same value provided as an argument. This is useful when you want to have uniform
	/// corners around a box, such as a uniform border radius on a rectangle.
	///
	/// # Arguments
	///
...
impl Corners<Pixels> {
	...
	/// # Returns
	///
	/// Returns a new `Corners<ScaledPixels>` where each corner is the result of scaling the original corner by the
	/// given factor.
	///
	/// # Examples
	///
...
impl<T: Div<f32, Output = T> + Ord + Clone + fmt::Debug + Default + PartialEq> Corners<T> {
...
impl<T> Copy for Corners<T> where T: Copy + Clone + fmt::Debug + Default + PartialEq {}
...
#[derive(Clone, Copy, Default, Add, AddAssign, Sub, SubAssign, Neg, Div, DivAssign, PartialEq, Debug)]
#[repr(transparent)]
...
/// ```
#[derive(Clone, Copy, Default, Add, AddAssign, Sub, SubAssign, Neg, Div, DivAssign, PartialEq)]
#[repr(transparent)]
...
impl fmt::Display for PixelsParseError {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::MissingSuffix => f.write_str("expected 'px' suffix"),
			Self::Float(e) => e.fmt(f)
		}
	}
}

impl std::error::Error for PixelsParseError {}

impl TryFrom<&'_ str> for Pixels {
	type Error = PixelsParseError;

	fn try_from(value: &'_ str) -> Result<Self, Self::Error> {
		value
			.strip_suffix("px")
			.ok_or(PixelsParseError::MissingSuffix)
			.and_then(|number| Ok(number.parse().map_err(PixelsParseError::Float)?))
			.map(Self)
	}
}
...
/// display.
#[derive(Add, AddAssign, Clone, Copy, Default, Div, Eq, Hash, Ord, PartialEq, PartialOrd, Sub, SubAssign)]
#[repr(transparent)]
...
/// The value of `1rem` is typically equal to the font-size of the root element (often the `<html>` element in
/// browsers), making it a flexible unit that adapts to the user's text size preferences. In this framework, `rems`
/// serve a similar purpose, allowing for scalable and accessible design that can adjust to different display settings
/// or user preferences.
///
/// For example, if the root element's font-size is `16px`, then `1rem` equals `16px`. A length of `2rems` would then be
/// `32px`.
///
...
impl fmt::Display for RemsParseError {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::MissingSuffix => f.write_str("expected 'rem' suffix"),
			Self::Float(e) => e.fmt(f)
		}
	}
}

impl std::error::Error for RemsParseError {}

impl TryFrom<&'_ str> for Rems {
	type Error = RemsParseError;

	fn try_from(value: &'_ str) -> Result<Self, Self::Error> {
		value
			.strip_suffix("rem")
			.ok_or(RemsParseError::MissingSuffix)
			.and_then(|number| number.parse().map_err(RemsParseError::Float))
			.map(Self)
	}
}
...
#[derive(Debug, Clone)]
pub struct AbsoluteLengthParseError;

impl fmt::Display for AbsoluteLengthParseError {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.write_str("expected a number with a 'px' or 'rem' suffix")
	}
}

impl std::error::Error for AbsoluteLengthParseError {}

impl TryFrom<&'_ str> for AbsoluteLength {
	type Error = AbsoluteLengthParseError;
...
#[derive(Debug, Clone)]
pub enum DefiniteLengthParseError {
	InvalidFormat,
	BadPercent(ParseFloatError)
}

impl fmt::Display for DefiniteLengthParseError {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::InvalidFormat => f.write_str("expected a number with a 'px', 'rem', or '%' suffix"),
			Self::BadPercent(e) => e.fmt(f)
		}
	}
}

impl std::error::Error for DefiniteLengthParseError {}

impl TryFrom<&'_ str> for DefiniteLength {
	type Error = DefiniteLengthParseError;

	fn try_from(value: &'_ str) -> Result<Self, Self::Error> {
		if let Some(percentage) = value.strip_suffix('%') {
			let fraction: f32 = percentage.parse::<f32>().map_err(DefiniteLengthParseError::BadPercent)?;
			...
			Err(DefiniteLengthParseError::InvalidFormat)
		}
	}
}
...
impl fmt::Display for LengthParseError {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		f.write_str("expected 'auto' or a number with a 'px', 'rem', or '%' suffix")
	}
}
...
impl<T: IsZero + Clone + fmt::Debug + Default + PartialEq> IsZero for Point<T> {
	fn is_zero(&self) -> bool {
...
where
	T: IsZero + Clone + fmt::Debug + Default + PartialEq
{
	fn is_zero(&self) -> bool {
...
impl<T: IsZero + Clone + fmt::Debug + Default + PartialEq> IsZero for Bounds<T> {
	fn is_zero(&self) -> bool {
		self.size.is_zero()
	}
}
