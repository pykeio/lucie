...
type CategoryInfo = RapidHashMap<String, (Timings, NonZero<usize>, u8)>;
...
impl Output {
    ...
    fn collapse(self) -> RapidHashMap<Importance, CategoryInfo> {
        let mut categories = RapidHashMap::<Importance, RapidHashMap<String, _>>::default();
        for entry in self.tests {
                ...
                && let Ok(timings) = entry.2
            {
                if let Some(handle) = categories.get_mut(&mdata.importance) {
                    ...
                    let mut new = RapidHashMap::default();
...
pub struct PerfReport {
    ...
    deltas: RapidHashMap<Importance, PerfDelta>,
}
